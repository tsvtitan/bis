...
begin
  ...
  FSStopRemote:='Разорвано соединение. Время => %s';
  FSConnectRemote:='Установлено соединение с ip-адресом %s:%d';
  FSDoCommandGetFail:='Неудачное получение страницы. %s';
  FSRequestPage:='Запрошена страница [%s %s]';
  FSHandlerNotFound:='Не найден обработчик страницы.';
  FSInvalidHandler:='Неверный обработчик страницы.';
  FSAccessDenied:='Доступ запрещен.';
