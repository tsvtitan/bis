...
begin
  if Assigned(Parent) then begin
    TempValue:=Value;
    FColorBox:=TColorBox(DoFindComponent(FColorBoxName));
    if Assigned(FColorBox) then begin
      ...
      FLabelColorBox:=TLabel(DoFindComponent(FLabelName));
...
procedure TBisParamColorBox.GetControls(List: TList);
begin
  inherited GetControls(List);
  if Assigned(List) then
    List.Add(FLabelColorBox);
end;
