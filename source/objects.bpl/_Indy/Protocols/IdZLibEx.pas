...
{**********************************************************************}
{}
{ $Log:  57210: IdZLibEx.pas
{
{   Rev 1.3    9/11/2004 10:58:10 AM  JPMugaas
{ FTP now decompresses output directly to the IOHandler.
}
{
    Rev 1.2    7/18/2004 3:01:48 PM  DSiders
  Added localization comments.
}
{
{   Rev 1.1    2/21/2004 3:32:54 PM  JPMugaas
{ Foxed for Unit rename.
}
{
{   Rev 1.0    2/12/2004 11:26:20 PM  JPMugaas
{ ZLibEx package for inclusion in Indy.  We should be basing Indy partly on
{ this instead of Borland's old ZLib units and objects.
}
{*****************************************************************************
*  ZLibEx.pas                                                                *
*                                                                            *
...
type
  ...
  TZStrategy         = (zsDefault, zsFiltered, zsHuffman, zsRLE);
...
//memory allocation function implementations for ZLib
function zcalloc(opaque: Pointer; items, size: Integer): Pointer;
procedure zcfree(opaque, block: Pointer);

//misc made public for Indy usage

function ZCompressCheck(code: Integer): Integer;
function ZDecompressCheck(code: Integer): Integer;

//ZLib function constants
const
  {** flush constants *******************************************************}
...
const
  ...
  _z_errmsg: array[0..9] of PChar = (
    // Z_NEED_DICT      (2)
    'need dictionary',        {do not localize}
    // Z_STREAM_END     (1)
    'stream end',             {do not localize}
    // Z_OK             (0)
    '',                       {do not localize}
    // Z_ERRNO          (-1)
    'file error',             {do not localize}
    // Z_STREAM_ERROR   (-2)
    'stream error',           {do not localize}
    // Z_DATA_ERROR     (-3)
    'data error',             {do not localize}
    // Z_MEM_ERROR      (-4)
    'insufficient memory',    {do not localize}
    // Z_BUF_ERROR      (-5)
    'buffer error',           {do not localize}
    // Z_VERSION_ERROR  (-6)
    'incompatible version',   {do not localize}
    ''
  );
  ...
  SZInvalid = 'Invalid ZStream operation!'; {do not localize}
