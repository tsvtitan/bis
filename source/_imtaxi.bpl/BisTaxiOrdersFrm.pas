...
uses
  ...
  BisEvents, BisDataSet, BisThread, BisProvider, BisControls;
...
type
  ...
  TBisTaxiOrdersFrame=class;

  TBisTaxiOrdersFrameThread=class(TBisThread)
  private
    FParent: TBisTaxiOrdersFrame;
    FClient: TBisDataSet;
  ...
  public
    ...
    property Parent: TBisTaxiOrdersFrame read FParent write FParent;
  ...
  private
    ...
    FThread: TBisTaxiOrdersFrameThread;
    FPhoneIndex: Integer;
...
{ TBisTaxiOrdersFrameThread }

procedure TBisTaxiOrdersFrameThread.GetAutoRefresh;
begin
  FAutoRefresh:=true;
...
procedure TBisTaxiOrdersFrameThread.GetClient;
var
...
procedure TBisTaxiOrdersFrameThread.RefreshParent;
var
  DateAccept,OrderNum: Variant;
begin
  if Assigned(FServer) and Assigned(FParent) and FParent.CanAutoRefresh then begin
   // FParent.Provider.BeginUpdate;
    try
      DateAccept:=FParent.Provider.FieldByName('DATE_ACCEPT').Value;
      OrderNum:=FParent.Provider.FieldByName('ORDER_NUM').Value;
      ...
      FParent.Provider.Locate('DATE_ACCEPT;ORDER_NUM',VarArrayOf([DateAccept,OrderNum]),[loCaseInsensitive]);
...
procedure TBisTaxiOrdersFrameThread.Execute;
  ...
  var
    ...
    DateAccept: Variant;
    OrderNum: Variant;
    ServerLocked, ClientLocked: Variant;
    ServerActionId, ClientActionId: Variant;
    ServerResultId, ClientResultId: Variant;
    ServerTypeProcess, ClientTypeProcess: Variant;
    ServerDateArrival, ClientDateArrival: TDateTime;
    ServerMinBetween, ClientMinBetween: Integer;
  begin
    Result:=false;
    if FServer.Active then begin
      ...
      if not Result then begin
        FServer.First;
        while not FServer.Eof do begin
          DateAccept:=FServer.FieldByName('DATE_ACCEPT').Value;
          OrderNum:=FServer.FieldByName('ORDER_NUM').Value;
          Found:=FClient.Locate('DATE_ACCEPT;ORDER_NUM',VarArrayOf([DateAccept,OrderNum]),[loCaseInsensitive]);
          if Found then begin
            ServerLocked:=FServer.FieldByName('LOCKED').Value;
            ServerActionId:=FServer.FieldByName('ACTION_ID').Value;
            ServerResultId:=FServer.FieldByName('RESULT_ID').Value;
            ServerTypeProcess:=FServer.FieldByName('TYPE_PROCESS').Value;
            ServerDateArrival:=FServer.FieldByName('DATE_ARRIVAL').AsDateTime;
            ServerMinBetween:=MinutesBetween(Core.ServerDate,ServerDateArrival);
            ClientLocked:=FClient.FieldByName('LOCKED').Value;
            ClientActionId:=FClient.FieldByName('ACTION_ID').Value;
            ClientResultId:=FClient.FieldByName('RESULT_ID').Value;
            ClientDateArrival:=FClient.FieldByName('DATE_ARRIVAL').Value;
            ClientMinBetween:=MinutesBetween(Core.ServerDate,ClientDateArrival);
            Result:=not VarSameValue(ServerLocked,ClientLocked) or
                    not VarSameValue(ServerActionId,ClientActionId) or
                    not VarSameValue(ServerResultId,ClientResultId) or
                    not VarSameValue(ServerTypeProcess,ClientTypeProcess) or
                    (ServerMinBetween<>ClientMinBetween);
            if Result then
              break;
          end else begin
            Result:=true;
            ...
            if not Terminated and FAutoRefresh and Assigned(FParent) and
              ...
              try
                ...
                if not Terminated and FClient.Active then begin
                  ...
                  FServer.Open;
                  if not Terminated and ChangeExists then begin
...
procedure TBisTaxiOrdersFrame.DoEventRefresh;
begin
  if not Assigned(FThread) and CanAutoRefresh then begin
    StartAutoRefresh(true);
  end;
end;

procedure TBisTaxiOrdersFrame.FreeAutoRefresh;
begin
...
procedure TBisTaxiOrdersFrame.ThreadTerminate(Sender: TObject);
begin
  FThread:=nil;
end;

procedure TBisTaxiOrdersFrame.StartAutoRefresh(Once: Boolean);
begin
  StopAutoRefresh;
  if not Assigned(FThread) then begin
    FThread:=TBisTaxiOrdersFrameThread.Create;
...
procedure TBisTaxiOrdersFrame.StopAutoRefresh;
begin
  if Assigned(FThread) then begin
    FThread.Terminate;
  end;
...
begin
  Result:=Null;
  if DataSet.Active and not DataSet.isEmpty and Assigned(Core) then begin
...
begin
  ...
  if Assigned(Data) and Assigned(Data.Values) then begin
    if (Column=1) then begin
      Locked:=not VarIsNull(Data.Values.GetValue('LOCKED'));
    ...
    if (Column=FPhoneIndex) then begin
      TypeAccept:=VarToIntDef(Data.Values.GetValue('TYPE_ACCEPT'),0);
      if TypeAccept=0 then
        ImageIndex:=20
      else ImageIndex:=21;
    end;
  end;
...
begin
  ...
  if Assigned(Data) and Assigned(DataKey) and Assigned(DateKeyFocused) then begin
    ...
    if Flag then begin
      Phone:=Trim(VarToStrDef(Data.Values.GetValue('PHONE'),''));
      L:=Length('+79029232332');
...
begin
  if Assigned(FParentForm) then
    FParentForm.Caption:=GetParentFormCaption;
end;
...
begin
  if not VarIsNull(NewOrderId) and not VarIsNull(OldOrderId) then begin
    ...
    if Provider.Active and not Provider.IsEmpty then begin
      Provider.Locate('ORDER_ID',NewOrderId,[loCaseInsensitive]);
    end;
  end;
end;
  ...
  if CanClientCall then begin
    ...
    if Assigned(AIface) then begin
      AIface.Dial(Provider.FieldByName('PHONE').AsString,
                  Provider.FieldByName('ORDER_ID').Value);
    end;
  end;
end;
