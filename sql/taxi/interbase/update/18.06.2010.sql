...
BEGIN
  ...
  IF (NEXT_ID IS NOT NULL) THEN BEGIN
    ...
    EXECUTE PROCEDURE CREATE_ORDER_HISTORY(OLD_ORDER_ID,NEW_ORDER_ID,ACCOUNT_ID,
...
DROP PROCEDURE I_METHOD

--

DROP PROCEDURE U_METHOD

--

DROP PROCEDURE D_METHOD
...
ALTER TABLE METHODS
ADD IN_MESSAGE INTEGER

--

ALTER TABLE METHODS
ADD IN_QUERY INTEGER
...
ALTER TABLE METHODS
ADD OUTGOING INTEGER

--

CREATE VIEW S_METHODS
AS
SELECT * FROM METHODS
...
BEGIN
  INSERT INTO METHODS (METHOD_ID,NAME,DESCRIPTION,PRIORITY,
                       IN_MESSAGE,IN_QUERY,IN_CALL,OUTGOING)
...
CREATE OR ALTER PROCEDURE D_METHOD
(
  OLD_METHOD_ID VARCHAR(32))
AS
BEGIN
...
CREATE OR ALTER PROCEDURE LOCK_OUT_MESSAGE
(
...
BEGIN
  ...
  IF ((SENDER_ID IS NOT NULL) AND (CODE IS NOT NULL)) THEN BEGIN
    ...
    EXECUTE PROCEDURE TRY_PARK_IN (NULL,ACCOUNT_ID,SENDER_ID,CODE,TYPE_MESSAGE);
  ...
  IF ((CONTACT IS NOT NULL) AND (SENDER_ID IS NOT NULL)) THEN BEGIN
    ...
    IF (CNT>0) THEN BEGIN
      ...
      IF ((MIN_BALANCE IS NULL) OR ((MIN_BALANCE IS NOT NULL) AND (BALANCE>MIN_BALANCE))) THEN BEGIN
        ...
        EXECUTE PROCEDURE QUERY_PARK_STATES (NULL,ACCOUNT_ID,SENDER_ID,CONTACT,TYPE_MESSAGE);
...
BEGIN
  ...
  IF (LOCKED IS NULL) THEN BEGIN
    ...
    EXECUTE PROCEDURE LOCK_ORDER(ORDER_ID,LOCKED,TYPE_PROCESS);
    ...
    EXECUTE PROCEDURE PROCESS_RESULT(ORDER_ID,GET_UNIQUE_ID(),RESULT_ID,ACCOUNT_ID,TYPE_PROCESS)
     RETURNING_VALUES ORDER_ID;

    EXECUTE PROCEDURE UNLOCK_ORDER(ORDER_ID);
    ...
    IF (CNT>0) THEN BEGIN
      ...
      IF ((MIN_BALANCE IS NULL) OR ((MIN_BALANCE IS NOT NULL) AND (BALANCE>MIN_BALANCE))) THEN BEGIN
        ...
        IF (ORDER_ID IS NOT NULL) THEN BEGIN
          ...
          EXECUTE PROCEDURE PROCESS_ORDER(ACCOUNT_ID,ORDER_ID,0);
        ...
        IF (CNT=0) THEN BEGIN
          ...
          IF (((MIN_BALANCE IS NULL) OR ((MIN_BALANCE IS NOT NULL) AND (BALANCE>MIN_BALANCE))) AND
            ...
            INSERT INTO OUT_MESSAGES (OUT_MESSAGE_ID,CREATOR_ID,RECIPIENT_ID,DATE_CREATE,
                                      TEXT_OUT,DATE_OUT,TYPE_MESSAGE,CONTACT,DESCRIPTION,PRIORITY,LOCKED,DATE_BEGIN)
                              VALUES (GET_UNIQUE_ID(),:ACCOUNT_ID,:DRIVER_ID,CURRENT_TIMESTAMP,
                                      :S1,NULL,:TYPE_MESSAGE,:PHONE,NULL,1,NULL,:D);
      ...
      IF (SHIFT_BEGIN IS NOT NULL) THEN BEGIN
        ...
        IF ((CNT=0) AND (D1>PARK_IN) AND (D1>PARK_OUT) AND (D1>SHIFT_BEGIN)) THEN BEGIN
          ...
          IF (CNT=0) THEN BEGIN
            ...
            IF ((D1>DATE_END) AND (TYPE_MESSAGE IS NOT NULL)) THEN BEGIN
              ...
              INSERT INTO OUT_MESSAGES (OUT_MESSAGE_ID,CREATOR_ID,RECIPIENT_ID,DATE_CREATE,
                                        TEXT_OUT,DATE_OUT,TYPE_MESSAGE,CONTACT,DESCRIPTION,PRIORITY,LOCKED,DATE_BEGIN)
                                VALUES (GET_UNIQUE_ID(),:ACCOUNT_ID,:DRIVER_ID,CURRENT_TIMESTAMP,
                                        :S1,NULL,:TYPE_MESSAGE,:PHONE,NULL,2,NULL,:D);
...
BEGIN
  LOCK_COUNT=0;
    ...
    FOR SELECT OUT_MESSAGE_ID, DATE_BEGIN, DATE_END
      ...
      IF (DATE_BEGIN<=D AND
          (DATE_BEGIN+(COMMON_PERIOD*(1e0/24/60/60)))>=D) THEN BEGIN
...
BEGIN
  ...
  IF (S IS NOT NULL) THEN BEGIN
    ...
    INSERT INTO OUT_MESSAGES (OUT_MESSAGE_ID,CREATOR_ID,RECIPIENT_ID,DATE_CREATE,
                              TEXT_OUT,DATE_OUT,TYPE_MESSAGE,CONTACT,DESCRIPTION,PRIORITY,LOCKED,DATE_BEGIN)
                      VALUES (GET_UNIQUE_ID(),:WHO_CREATE_ID,:ACCOUNT_ID,CURRENT_TIMESTAMP,
                              :S,NULL,:TYPE_MESSAGE,:PHONE,NULL,2,NULL,CURRENT_TIMESTAMP);
  END
...
CREATE OR ALTER PROCEDURE I_RESULT
(
  RESULT_ID VARCHAR(32),
  ACTION_ID VARCHAR(32),
  NEXT_ID VARCHAR(32),
...
AS
BEGIN
  INSERT INTO RESULTS (RESULT_ID,ACTION_ID,NEXT_ID,NAME,DESCRIPTION,
                       PROC_DETECT,PROC_PROCESS,FONT_COLOR,BRUSH_COLOR,PRIORITY,VISIBLE)
...
CREATE OR ALTER PROCEDURE U_RESULT
(
  RESULT_ID VARCHAR(32),
  ACTION_ID VARCHAR(32),
  NEXT_ID VARCHAR(32),
