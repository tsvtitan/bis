   ...
   WHERE P.PRESENTATION_ID=PRESENTATION_ID
    ...
    CLOSE C1;		 
	  ...
	  IF TRIM(PARAM_IDS)<>'' THEN
  	  ...
  	  SET @QUERY=CONCAT(@QUERY,'AND O.DATE_BEGIN>=DATE_ADD(CURRENT_TIMESTAMP,INTERVAL -1 MONTH) ');
  	  SET @QUERY=CONCAT(@QUERY,'AND (O.DATE_END IS NULL OR O.DATE_END<=CURRENT_TIMESTAMP) ');
  	  SET @QUERY=CONCAT(@QUERY,'AND O.STATUS=1 ');
		  SET @QUERY=CONCAT(@QUERY,'GROUP BY OP.OBJECT_ID ');
      SET @QUERY=CONCAT(@QUERY,'ORDER BY O.DATE_BEGIN DESC');
