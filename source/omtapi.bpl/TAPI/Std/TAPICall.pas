...
uses
  ...
  TAPI,TAPISystem,TAPIHelpFunc,tapierr,TAPIServices,TAPITon,DevConf,Qos;

{$INCLUDE TAPI.INC}

type
  TLineCallPrivilege = (cpNone,cpOwner,cpMonitor);
  {$NODEFINE TLineCallPrivilege}
  TLineCallPrivileges = set of TLineCallPrivilege;
  {$NODEFINE TLineCallPrivileges}

  (*$HPPEMIT 'namespace Tapicall'*)
  (*$HPPEMIT '{'*)
  (*$HPPEMIT '  enum TLineCallPrivilege {cpNone,cpOwner,cpMonitor};'*)
  (*$HPPEMIT '  typedef Set<TLineCallPrivilege,cpNone,cpMonitor>TLineCallPrivileges;'*)
  (*$HPPEMIT '}'*)
...
{$ENDIF}
  ...
  TLineCallReason = (crDirect,crFWDBusy,crFWDNoAnswer,crFWDUnCond,crPickup,
    ...
    crUnavail,crIntrude
    {$IFDEF TAPI20},crParked,crCampedon,crRouterRequest{$ENDIF});
  ...
  TLineProxyRequest = (lprSetAgentGroup,lprSetAgentState,lprSetAgentActivity,
    ...
    lprGetQueueList,lprSetQueueMeasurementPeriod,lprGetQueueInfo,
    lprGetGroupList,lprSetAgentStateEx{$ENDIF});
  ...
  // Eventproc
  TCallEventOffering=procedure(Sender:TObject;OfferingMode:TLineOfferingModes;Rights:TLineCallPrivileges)of Object;
  TCallEvent=procedure(Sender:TObject;Rights:TLineCallPrivileges)of Object;
  TCallEventConnected=procedure(Sender:TObject;ConnectedMode:TLineConnectedModes;Rights:TLineCallPrivileges)of Object;
  TCallEventReply=procedure(Sender:TObject;AsyncFunc:TAsyncFunc;Error:Dword)of Object;
  TCallEventBusy=procedure(Sender:TObject;BusyMode:TLineBusyMode;Rights:TLineCallPrivileges)of Object;
  TCallEventSpecialInfo=procedure(Sender:TObject;SpecialInfo:TLineSpecialInfo;Rights:TLineCallPrivileges)of Object;
  TCallGatherDigitsEvent=procedure(Sender: TObject;GatherTermination:TLineGatherTerm;TickCount:Dword)of Object;
  TCallEventDialTone=procedure(Sender:TObject;DialTonMode:TLineDialToneMode;Rights:TLineCallPrivileges)of Object;
  TCallEventDisconnected=procedure(Sender:TObject;DisconnectedMode:TLineDisconnectMode;Rights:TLineCallPrivileges)of Object;
  ...
  public
    ...
    property SendingFlowspec:TFlowSpec read FSendingFlowspec write FSendingFlowspec;
    property ReceivingFlowspec:TFlowSpec read FReceivingFlowspec write FReceivingFlowspec;
  published
    property BearerMode:TLineBearerMode read FBearerMode write FBearerMode default bmVoice;                   
  ...
  public
    ...
    property CallPrivileges:TLineCallPrivileges read FCallPrivileges;
...
type
   ...
   public
     ...
     //procedure DeallocateCall;
     procedure ChangePrivilege(NewPriv: TLineCallPrivileges);
   ...
   public
     ...
     property SendingFlowspec:TFLOWSPEC read FSendingFlowspec;
     property ReceivingFlowspec:TFLOWSPEC read FReceivingFlowspec;
     {$ENDIF}
   end;
...
function IntToCallPrivilege(Value:LongWord):TLineCallPrivileges;
function CallPrivilegeToInt(Value:TLineCallPrivileges):LongWord;
...
begin
  ...
  FCallPrivileges:=IntToCallPrivilege(CStatus^.dwCallPrivilege);
...
var CRight:TLineCallPrivileges;
{$IFDEF DEBUG}
var MS:String;
{$ENDIF}
begin
...
procedure TTAPICall.ChangePrivilege(NewPriv: TLineCallPrivileges);
var R:Longint;
begin
...
begin
  ...
  FCallerID:=GetDataFromTAPIStruct(FLineCallInfo,
    FLineCallInfo^.dwCallerIDOffset,FLineCallInfo^.dwCallerIDSize);
  FCallerIDName:=GetDataFromTAPIStruct(FLineCallInfo,
    FLineCallInfo^.dwCallerIDNameOffset,FLineCallInfo^.dwCallerIDNameSize);
  FCalledIDFlags:=IntToCallPartyID(FLineCallInfo^.dwCalledIDFlags);
  FCalledID:=GetDataFromTAPIStruct(FLineCallInfo,
      FLineCallInfo^.dwCalledIDOffset,FLineCallInfo^.dwCalledIDSize);
  ...
  begin
   ...
   FCalledID:=PChar(Dummy);
    ...
    FCalledID:=''; }
  FCalledIDName:=GetDataFromTAPIStruct(FLineCallInfo,
    FLineCallInfo^.dwCalledIDNameOffset,FLineCallInfo^.dwCalledIDNameSize);
  ...
  FComment:=GetDataFromTAPIStruct(FLineCallInfo,FLineCallInfo^.dwCommentOffset,FLineCallInfo^.dwCommentSize);
    //PChar(Dummy);
  //end
 // else
   // FComment:='';
  ...
  begin
    ...
    begin
      Offset:=FLineCallInfo^.dwTerminalModesOffset+DWord(i)*DSize;
      FTerminalModes[i]:=IntToTermModes(DWord(GetDataFromTAPIStructP(FLineCallInfo,Offset,DSize)));
    end;
  end;
  FDevSpecificSize:= FLineCallInfo^.dwDevSpecificSize;
  if FLineCallInfo^.dwDevSpecificSize > 0 then
  begin
    SetLength(FDevSpecific,FDevSpecificSize);
    FDevSpecific:=GetDataFromTAPIStructP(FLineCallInfo,FLineCallInfo^.dwDevSpecificOffset,FDevSpecificSize);
  end;
  {$IFDEF TAPI20}
  FCallTreatment:=FLineCallInfo^.dwCallTreatment;
  FCallData:=GetDataFromTAPIStruct(FLineCallInfo,FLineCallInfo^.dwCallDataSize,FLineCallInfo^.dwCallDataOffset);
  if FLineCallInfo^.dwSendingFlowspecSize > 0 then
    FSendingFlowspec:=TFlowSpec(GetDataFromTAPIStructP(FLineCallInfo,FLineCallInfo^.dwSendingFlowspecOffset,FLineCallInfo^.dwSendingFlowspecSize)^);
  if FLineCallInfo^.dwReceivingFlowspecSize >0 then
    FReceivingFlowspec:=TFlowSpec(GetDataFromTAPIStructP(FLineCallInfo,FLineCallInfo^.dwReceivingFlowspecOffset,FLineCallInfo^.dwReceivingFlowspecSize)^);
  {$ENDIF}
