...
//    OnNewRecord:=ProviderCatalogNewRecord;
  ...
  with FResultFrame do begin
    ...
    with Provider.FieldNames do begin
      ...
      FFieldNameDateCreate:=Add(SFieldDateCreate,'Дата создания',110);
      Add(SFieldBarcode,'Штрих-код',90);
      Add(SFieldCode,'Код',110);
      Add(SFieldName,'Наименование',150);
      Add(SFieldFileName,'Файл изображения',230);
      AddCheckBox(SFieldUpload,'Загружен',30);
    end;
...
begin
  if (Key=VK_RETURN) then begin
    ...
    if SearchBarcode then begin
      ...
      ShowWarning(Format('Объект по штрих-коду %s не найден.',[EditBarcode.Text]));
    end;
  end;
end;
...
begin
  if FChangesPresent then begin
    Ret:=ShowQuestionCancel('Выйти с сохранением результата?');
    case Ret of
      mrYes: begin
        Update;
...
begin
  ...
  if Provider.Active and not Provider.IsEmpty then begin
    ...
    if Flag then begin
      MemoName.Lines.Add(Format('Код: %s',[Provider.FieldByName(SFieldCode).AsString]));
      MemoName.Lines.Add(Format('Наименование: %s',[Provider.FieldByName(SFieldName).AsString]));
      MemoName.Lines.Add(Format('Производитель: %s',[Provider.FieldByName(SFieldProducer).AsString]));
      MemoName.Lines.Add(Format('Страна: %s',[Provider.FieldByName(SFieldCountry).AsString]));
    end;
  end;
end;
...
begin
  ...
  if not FCanon.Connected then begin
    ButtonFotoConnect.Caption:='Соединить';
    ButtonFotoConnect.Hint:='Соединить с фотоаппаратом';
    ButtonFotoConnect.OnClick:=ButtonFotoConnectClick;
  end else begin
    ButtonFotoConnect.Caption:='Разорвать';
    ButtonFotoConnect.Hint:='Разорвать соединение с фотоаппаратом';
...
begin
  ...
  if not FScanner.Connected then begin
    ButtonScannerConnect.Caption:='Соединить';
    ButtonScannerConnect.Hint:='Соединить со сканером';
    ButtonScannerConnect.OnClick:=ButtonScannerConnectClick;
  end else begin
    ButtonScannerConnect.Caption:='Разорвать';
    ButtonScannerConnect.Hint:='Разорвать соединение со сканером';
...
begin
  if Trim(Message)<>'' then begin
    LoggerWrite(Format('%s: %s',['Сканер',Message]),ltInformation);
    MemoDevice.Lines.Add(Format('%s %s: %s',[TimeToStr(Time),'Сканер',Message]));
...
begin
  if Trim(Message)<>'' then begin
    LoggerWrite(Format('%s: %s',['Фотоаппарат',Message]),ltInformation);
    MemoDevice.Lines.Add(Format('%s %s: %s',[TimeToStr(Time),'Фотоаппарат',Message]));
    MemoDevice.SelStart:=Length(MemoDevice.Text);
    MemoDevice.SelLength:=0;
    RefreshButtonFoto;
  end;
