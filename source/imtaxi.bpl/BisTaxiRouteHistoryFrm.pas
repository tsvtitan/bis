...
uses
  ...
  VirtualTrees,
  BisDbTree, BisDataGridFrm, BisFieldNames, BisDataSet, BisDataEditFm;
...
type
  ...
  private
    FRouteHistory: TBisDataSetCollectionItem;
    FPhone: Variant;
    FLocalityId: Variant;
    FHouse: Variant;
    FStreetId: Variant;
  ...
  TBisTaxiRouteHistoryUpdateIface=class(TBisDataEditFormIface)
  public
...
uses
     BisConsts, BisOrders, BisProvider, BisFilterGroups, BisUtils, BisMemoFm;
...
{ TBisTaxiRouteHistoryUpdateIface }

procedure TBisTaxiRouteHistoryUpdateIface.Execute;
var
  Form: TBisMemoForm;
  P: TBisProvider;
  S: String;
begin
  if Assigned(ParentDataSet) then begin
    ...
    try
      Form.Caption:='Изменить описание';
      Form.Memo.Text:=ParentDataSet.FieldByName('DESCRIPTION').AsString;
      Form.Memo.WordWrap:=true;
      Form.Memo.ScrollBars:=ssNone;
      Form.ButtonOk.Visible:=true;
      if Form.ShowModal=mrOk then begin
        S:=Trim(Form.Memo.Lines.Text);
        P:=TBisProvider.Create(nil);
        try
          P.ProviderName:='U_ROUTE_HISTORY';
          with P.Params do begin
            AddKey('PHONE').Older('OLD_PHONE');
            AddKey('STREET_ID').Older('OLD_STREET_ID');
            AddKey('HOUSE').Older('OLD_HOUSE');
            AddInvisible('FLAT');
            AddInvisible('PORCH');
            AddInvisible('DESCRIPTION');
            AddInvisible('CLIENT_ID');
            AddInvisible('AMOUNT');
            AddInvisible('DATE_FIRST');
            AddInvisible('DATE_LAST');
            RefreshByDataSet(ParentDataSet,true,false);
          end;
          P.ParamByName('DESCRIPTION').Value:=iff(S<>'',S,NULL);
          P.Execute;
          if P.Success then begin
            ParentDataSet.BeginUpdate(true);
            try
              ParentDataSet.Edit;
              ParentDataSet.FieldByName('DESCRIPTION').Value:=P.ParamByName('DESCRIPTION').Value;
              ParentDataSet.Post;
            finally
              ParentDataSet.EndUpdate;
            end;
          end;
        finally
          P.Free;
...
begin
  ...
  UpdateClass:=TBisTaxiRouteHistoryUpdateIface;

  Provider.Threaded:=false;
  ...
  with Provider.FieldDefs do begin
    ...
    Add('CLIENT_ID',ftString,32);
    Add('DATE_LAST',ftDateTime);
    Add('DATE_FIRST',ftDateTime);
  ...
  with Provider.FieldNames do begin
    ...
    Add('CLIENT_ID','Ид клиента',25).Visible:=false;
    Add('DATE_FIRST','Дата начала',80).Visible:=false;
    Add('DATE_LAST','Дата',80).DisplayFormat:=SDateTimeFormat;
    Add('PHONE','Телефон',90);
    Add('LOCALITY_NAME','Город',75);
    Add('STREET_NAME','Улица',135);
    Add('HOUSE','Дом',35);
    Add('FLAT','Квартира',35);
    Add('PORCH','Подъезд',35);
    Add('DESCRIPTION','Описание',125);
...
var
  ...
  ADirection: Integer;
begin
  Direction:=Grid.GetNodeValue(Node,'DIRECTION');
...
procedure TBisTaxiRouteHistoryFrame.RefreshProvider;
var
  P: TBisProvider;
  FlagFrom: Boolean;
  FlagTo: Boolean;
  ...
  if not VarIsNull(FPhone) or not VarIsNull(FClientId) then begin
    ...
    try
      ...
      if not VarIsNull(FPhone) then
        P.FilterGroups.Add.Filters.Add('PHONE',fcEqual,FPhone).CheckCase:=true;
      if not VarIsNull(FClientId) then
        P.FilterGroups.Add.Filters.Add('CLIENT_ID',fcEqual,FClientId).CheckCase:=true;
