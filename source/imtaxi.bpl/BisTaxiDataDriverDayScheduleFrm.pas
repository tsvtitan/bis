...
begin
  ...
  with Provider.FieldNames do begin
    Add('DAY_NAME','Äåíü',70).Alignment:=daCenter;
    for i:=0 to 24-1 do begin
      Add(GetFieldCount(i),IntToStr(i),25).Alignment:=daCenter;
      Add(GetFieldExists(i),IntToStr(i),30).Visible:=false;
    end;
    Add('WORK_DAY','Ğàáî÷èé äåíü',40).Visible:=false;
    Add('WEEK_DAY','Äåíü íåäåëè',40).Visible:=false;
    Add('REST_OR_WORK','Ôëàã ğàáîòû',40).Visible:=false;
    with Add('DAY_DATE','Äàòà äíÿ',40) do begin
      IsKey:=true;
      Visible:=false;
    end;
  end;
