...
type
  ...
  TBisTaxiDriverParksFrameThread=class(TBisWaitThread)
  private
...
procedure TBisTaxiDriverParksFrameThread.DoTimeout;
...
begin
  ...
  with Provider.FieldNames do begin
    AddKey('ID');
    Add('NAME','Код',40);
    FFieldNewName:=AddCalculate('NEW_NAME','Стоянка / Водитель',GetNewName,ftString,110,190);
    Add('PRIORITY','Порядок',20).Alignment:=daCenter;
    Add('DATE_IN','Время постановки',80).DisplayFormat:='hh:nn:ss dd.mm.yyyy';
    with AddParentKey('PARENT_ID') do begin
      Caption:='Ид родителя';
      Visible:=false;
    end;
    with AddInvisible('MAX_COUNT') do begin
      Caption:='Максимальное количество';
      Visible:=false;
    end;
    with AddInvisible('DRIVER_COUNT') do begin
      Caption:='Количество водителей';
      Visible:=false;
    end;
    Add('BRUSH_COLOR','Цвет фона',0).Visible:=false;
    Add('FONT_COLOR','Цвет шрифта',0).Visible:=false;
  end;
...
procedure TBisTaxiDriverParksFrame.StartAutoRefresh;
begin
  StopAutoRefresh;
  if not Assigned(FThread) then begin
    FThread:=TBisTaxiDriverParksFrameThread.Create(1000);
