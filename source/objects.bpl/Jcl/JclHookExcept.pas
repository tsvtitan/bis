...
type
  ...
  // by TSV
  TJclExceptNotifyMethodEx = procedure(ExceptObj: TObject; ExceptAddr: Pointer; OSException: Boolean; PException: Pointer) of object;
...
// by TSV
function JclAddExceptNotifier(const NotifyMethod: TJclExceptNotifyMethodEx; Priority: TJclExceptNotifyPriority = npNormal): Boolean; overload;
...
// by TSV
function JclRemoveExceptNotifier(const NotifyMethod: TJclExceptNotifyMethodEx): Boolean;  overload;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jcl.svn.sourceforge.net:443/svnroot/jcl/tags/JCL-1.100-Build2646/jcl/source/windows/JclHookExcept.pas $';
...
type
  ...
  private
    ...
    // by TSV
    FNotifyMethodEx: TJclExceptNotifyMethodEx;
  ...
  public
    ...
    // by TSV
    constructor Create(const NotifyMethod: TJclExceptNotifyMethodEx; Priority: TJclExceptNotifyPriority); overload;
//    procedure DoNotify(ExceptObj: TObject; ExceptAddr: Pointer; OSException: Boolean; ESP: Pointer);
    // TSV
    procedure DoNotify(ExceptObj: TObject; ExceptAddr: Pointer; OSException: Boolean; ESP: Pointer; P: PExceptionRecord=nil);
...
// by TSV
constructor TNotifierItem.Create(const NotifyMethod: TJclExceptNotifyMethodEx; Priority: TJclExceptNotifyPriority);
begin
  inherited Create;
  FNotifyMethodEx:= NotifyMethod;
  FPriority := Priority;
end;

{procedure TNotifierItem.DoNotify(ExceptObj: TObject; ExceptAddr: Pointer; OSException: Boolean; ESP: Pointer);
begin
...
// by TSV
procedure TNotifierItem.DoNotify(ExceptObj: TObject; ExceptAddr: Pointer; OSException: Boolean; ESP: Pointer; P: PExceptionRecord=nil);
begin
  if Assigned(FNotifyProc) then
    FNotifyProc(ExceptObj, ExceptAddr, OSException)
  else
  ...
  if Assigned(FNotifyMethodEx) then
    FNotifyMethodEx(ExceptObj, ExceptAddr, OSException, P);
end;
...
// by TSV
procedure DoExceptNotify(ExceptObj: Exception; ExceptAddr: Pointer; OSException: Boolean; ESP: Pointer; P: PExceptionRecord);
var
  Priorities: TJclExceptNotifyPriority;
  I: Integer;
begin
  ...
  begin
    ...
    try
      ...
      try
        ...
        begin
          with TNotifierItem(Items[0]) do
            DoNotify(ExceptObj, ExceptAddr, OSException, ESP, P);
        end
        else
        begin
          for Priorities := High(Priorities) downto Low(Priorities) do
...
begin
  ...
  begin
    DoExceptNotify(Arguments.ExceptObj, Arguments.ExceptAddr, False, GetEBP, nil);
  end;
...
begin
  ...
  DoExceptNotify(Result, P^.ExceptionAddress, True, GetEBP,P);
...
// by TSV
function JclAddExceptNotifier(const NotifyMethod: TJclExceptNotifyMethodEx; Priority: TJclExceptNotifyPriority): Boolean;
begin
  Result := Assigned(NotifyMethod);
  if Result then
    with Notifiers.LockList do
...
// by TSV
function JclRemoveExceptNotifier(const NotifyMethod: TJclExceptNotifyMethodEx): Boolean;
var
  O: TNotifierItem;
  I: Integer;
begin
  ...
  if Result then
    ...
    try
      ...
      begin
        O := TNotifierItem(Items[I]);
        if (TMethod(O.FNotifyMethodEx).Code = TMethod(NotifyMethod).Code) and
          (TMethod(O.FNotifyMethodEx).Data = TMethod(NotifyMethod).Data) then
        begin
          O.Free;
          Items[I] := nil;
        end;
