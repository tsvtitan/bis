...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        Interceptor := FInterceptorList[i];
        Break;
      end;
    end;
    ...
    if (PropInfo = nil) or (PPropInfo(PropInfo)^.GetProc = nil) then Exit;
    ...
    begin
      ...
      begin
        ...
        if Assigned(FDefnPropList.Objects[Index]) and
           ((PMethod(FDefnPropList.Objects[Index])^).Code <> nil) then
        begin
          WritePropName(PropName);
          TWriterProc(PMethod(FDefnPropList.Objects[Index])^)(Self);
        end;
      ...
      begin
        ...
        begin
          WritePropName(PropName);
          WriteBinary(TStreamProc(PMethod(FDefnBinPropList.Objects[Index])^));
        end;
      end else
...
begin
  New(Pm);
  Pm^ := TMethod(WriteData);
  FDefnBinPropList.AddObject(Name, TObject(Pm));
end;
...
begin
  New(Pm);
  Pm^ := TMethod(WriteData);
  FDefnPropList.AddObject(Name, TObject(Pm));
end;
...
begin
  ...
  OldParen := Parent;
  try
    Owner := TComponentCrack(Component).GetChildOwner;
    Parent := TComponentCrack(Component).GetChildParent;
...
//      if InterceptorClass = TStoragePropertyInterceptor(FInterceptorList[i]).ClassType then
    ...
    PropInfo := GetPropInfo(Interceptor.ClassInfo, PropName);
    if (PropInfo = nil) or (PPropInfo(PropInfo)^.SetProc = nil) then Exit;
...
procedure TPropReaderEh.ReadPropValue(Instance: TPersistent; PropInfo: Pointer);
const
...
{$ENDIF}
  ...
  procedure SetObjectIdent(Instance: TPersistent; PropInfo: Pointer; Ident: string);
  var
    ...
    begin
      Result := False;
      S := PChar(Pointer(Ident));
      P := S;
      while not (P^ in ['.', #0]) do Inc(P);
      if P^ = #0 then Exit;
      SetString(GlobalName, S, P - S);
      Delete(Ident, 1, P - S + 1);
      Result := True;
    end;
  ...
  begin
    ...
    if Component <> nil then
      SetOrdProp(Instance, PropInfo, Longint(Component))
...
function TPropReaderEh.ReadSet(SetType: Pointer): Integer;
var
  EnumType: PTypeInfo;
  EnumName: string;
begin
...
finalization
  InterceptorList.Free;
  TargetList.Free;
  ForChildListObj.Free;
end.