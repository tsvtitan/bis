...
begin
  ...
  FSSuspendTimer:='Остановить таймер';
  FSResumeTimer:='Запустить таймер';

  FSButtonCaptionYes:='Да';
  FSButtonCaptionNo:='Нет';
  FSButtonCaptionOK:='ОК';
  FSButtonCaptionCancel:='Отмена';
  FSButtonCaptionAbort:='Прервать';
  FSButtonCaptionRetry:='Повторить';
  FSButtonCaptionIgnore:='Игнорировать';
  FSButtonCaptionAll:='Все';
  FSButtonCaptionNoToAll:='Нет для всех';
  FSButtonCaptionYesToAll:='Да для всех';
  FSButtonCaptionHelp:='Помощь';
...
begin
  ...
  Canvas.Font.Assign(LabelMessage.Font);
  try
...
{  LabelMessage.SetBounds(ALeft, VertMargin+mcVertSpacing-mcVertMargin,
  ...
  LabelMessage.SetBounds(ALeft, VertMargin, TextRect.Right, PanelControls.Height-VertMargin*2+VertMargin div 2);
