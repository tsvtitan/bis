...
{**********************************************************************}
{}
{ $Log:  56953: IdSSLOpenSSLHeadersNET.pas 
{
...
type
 TERR_STATE=record
  ...
  err_file: Array[0..OPENSSL_ERR_NUM_ERRORS-1] of String;
  ...
  PLHASH	  = IntPtr;
{
  ...
  PEVP_MD	  = IntPtr;
  PEVP_CIPHER	  = IntPtr;
  PEVP_CIPHER_CTX = IntPtr;
  PEVP_PKEY	  = IntPtr;
//  PPEVP_PKEY      =^PEVP_PKEY;
  PEVP_ENCODE_CTX = IntPtr;
  PX509_LOOKUP    = IntPtr;
  PX509_STORE     = IntPtr;
  PX509_STORE_CTX = IntPtr;
  PX509_CRL	  = IntPtr;
  PPX509_CRL               =^PX509_CRL;
  PX509_LOOKUP_METHOD      = IntPtr;
  PX509_NAME               = IntPtr;
  PPX509_NAME              =^PX509_NAME;
  PX509_OBJECT             = IntPtr;
  PX509                    = IntPtr;
  PPX509		   =^PX509;
  PX509_EXTENSION_METHOD   = IntPtr;
  PX509_REQ	           = IntPtr;
  PPX509_REQ		   =^PX509_REQ;
  PX509_ATTRIBUTE	   = IntPtr;
  PPX509_ATTRIBUTE	   =^PX509_ATTRIBUTE;
  PX509_EXTENSION	   = IntPtr;
  PPX509_EXTENSION	   =^PX509_EXTENSION;
  PX509_NAME_ENTRY	   = IntPtr;
...
{
  PPKCS7_ISSUER_AND_SERIAL = IntPtr;
  PKCS7			   = IntPtr;
  PPKCS7                   =^PKCS7;
  PPPKCS7		   =^PPKCS7;
  PKCS7_SIGNER_INFO	   = IntPtr;
}
{
  PPKCS7_SIGNER_INFO       =^PKCS7_SIGNER_INFO;
  PPPKCS7_SIGNER_INFO      =^PPKCS7_SIGNER_INFO;
  PKCS7_RECIP_INFO	   = IntPtr;
  ...
  PPPKCS7_SIGNED	   = IntPtr;
  PPKCS7_ENC_CONTENT	   = IntPtr;
  PPPKCS7_ENC_CONTENT	   =^PPKCS7_ENC_CONTENT;
  PPKCS7_ENVELOPE	   = IntPtr;
  PPPKCS7_ENVELOPE	   =^PPKCS7_ENVELOPE;
  PPKCS7_SIGN_ENVELOPE	   = IntPtr;
  PPPKCS7_SIGN_ENVELOPE	   =^PPKCS7_SIGN_ENVELOPE;
  PPKCS7_DIGEST		   = IntPtr;
  PPPKCS7_DIGEST	   =^PPKCS7_DIGEST;
  PPKCS7_ENCRYPT	   = IntPtr;
  PPPKCS7_ENCRYPT	   =^PPKCS7_ENCRYPT;
  PNETSCAPE_SPKI	   = IntPtr;
  PPNETSCAPE_SPKI	   =^PNETSCAPE_SPKI;
  PNETSCAPE_SPKAC	   = IntPtr;
  PPNETSCAPE_SPKAC	   =^PNETSCAPE_SPKAC;
  PNETSCAPE_CERT_SEQUENCE  = IntPtr;
  PPNETSCAPE_CERT_SEQUENCE =^PNETSCAPE_CERT_SEQUENCE;
  Pbio_st		   = IntPtr;
  PMD2_CTX		   = IntPtr;
  PMD5_CTX		   = IntPtr;
  PSHA_CTX		   = IntPtr;
  PRIPEMD160_CTX	   = IntPtr;
  PRC4_KEY		   = IntPtr;
  PRC2_KEY		   = IntPtr;
  PRC5_32_KEY		   = IntPtr;
  PBF_KEY		   = IntPtr;
  PCAST_KEY		   = IntPtr;
  PIDEA_KEY_SCHEDULE       = IntPtr;
  PMDC2_CTX                = IntPtr;
  PPDSA_SIG                = IntPtr;
  P_des_cblock		   = IntPtr;
  Pdes_cblock		   = IntPtr;
  PDSA_SIG		   = IntPtr;
  PSTACK_ASN1_TYPE	   = IntPtr;
  PPSTACK_ASN1_TYPE	   = IntPtr;
  PSTACK_X509_NAME_ENTRY   = IntPtr;
  PPSTACK_X509_NAME_ENTRY  = IntPtr;
}
  PSTACK_X509_NAME         = IntPtr;
{
  PSTACK_X509_EXTENSION	   = IntPtr;
  PPSTACK_X509_EXTENSIO    = IntPtr;
  PSTACK_X509_ATTRIBUTE    = IntPtr;
  PPSTACK_X509_ATTRIBUTE   =^PSTACK_X509_ATTRIBUTE;
  PSTACK_X509		   = IntPtr;
  PPSTACK_X509		   =^PSTACK_X509;
  PSTACK_X509_INFO         = IntPtr;
  PPBEPARAM		   = IntPtr;
  ...
  PPSTACK_X509_EXTENSION   = IntPtr;
  PPKCS8_PRIV_KEY_INFO	   = IntPtr;
  PPPKCS8_PRIV_KEY_INFO    =^PPKCS8_PRIV_KEY_INFO;
  PEVP_PBE_KEYGEN          = IntPtr;
  PEVP_CIPHER_INFO	   = IntPtr;
}
  Ppem_password_cb	   = IntPtr;
  PPEM_ENCODE_SEAL_CTX     = IntPtr;
  PSTACK_SSL_CIPHER        = IntPtr;
  PSTACK_SSL_COMP          = IntPtr;
  PSSL_COMP		   = IntPtr;
  // PASN1_UTCTIME		   = IntPtr;
...
//procedures
  ...
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_cipher_list)]
    function IdSslCtxSetCipherList(arg0: PSSL_CTX;
      var str: String):Integer;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_new)]
    function IdSslCtxNew(meth: PSSL_METHOD):PSSL_CTX;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_free)]
    procedure IdSslCtxFree (arg0: PSSL_CTX);   external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_set_fd)]
    function IdSslSetFd(s: PSSL;
      fd: Integer):Integer;    external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_use_PrivateKey_file)]                                             //TODO:  See if this is really correct
    function IdSslCtxUsePrivateKeyFile(ctx: PSSL_CTX;
      _file: String;
      _type: Integer):Integer;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_use_certificate_file)]
    function IdSslCtxUseCertificateFile(ctx: PSSL_CTX;  
      _file: String;
      _type: Integer):Integer;   external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_load_error_strings)]
    procedure IdSslLoadErrorStrings;   external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_state_string_long)]
    function IdSslStateStringLong(s: PSSL): String;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_get_peer_certificate)]
    function IdSslGetPeerCertificate(s: PSSL):PX509;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_verify)]
    procedure IdSslCtxSetVerify(ctx: PSSL_CTX;
      mode: Integer;
      arg2: PFunction);  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_verify_depth)]
    procedure IdSslCtxSetVerifyDepth(ctx: PSSL_CTX;
      depth: Integer);  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_get_verify_depth)]
    function IdSslCtxGetVerifyDepth(ctx: PSSL_CTX):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_default_passwd_cb)]
    procedure IdSslCtxSetDefaultPasswdCb(ctx: PSSL_CTX; cb: Ppem_password_cb); external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_default_passwd_cb_userdata)]
    procedure IdSslCtxSetDefaultPasswdCbUserdata(ctx: PSSL_CTX; u: IntPtr); external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_check_private_key)]
    function IdSslCtxCheckPrivateKeyFile(ctx: PSSL_CTX):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint =fn_SSL_new)]
    function IdSslNew(ctx: PSSL_CTX):PSSL;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint =fn_SSL_free)]
    procedure IdSslFree(ssl: PSSL); external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint =fn_SSL_accept)]
    function IdSslAccept(ssl: PSSL):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint =fn_SSL_connect)]
    function IdSslConnect(ssl: PSSL):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_read)]
    function IdSslRead(ssl: PSSL; var buf: String; num: Integer):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_peek)]
    function IdSslPeek(ssl: PSSL; var buf: String; num: Integer):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_write)]
    function IdSslWrite(ssl: PSSL;  buf: String; num: Integer):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_get_error)]
    function IdSslGetError(s: PSSL; ret_code: Integer):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv2_method)]
    function IdSslMethodV2 : PSSL_METHOD; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv2_server_method)]
    function IdSslMethodServerV2 : PSSL_METHOD; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv2_client_method)]
    function IdSslMethodClientV2 : PSSL_METHOD; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv3_method)]
    function IdSslMethodV3 : PSSL_METHOD;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv3_server_method)]
    function IdSslMethodServerV3 : PSSL_METHOD; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv3_client_method)]
    function IdSslMethodClientV3 : PSSL_METHOD; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv23_method)]
    function IdSslMethodV23 : PSSL_METHOD; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv23_server_method)]
    function IdSslMethodServerV23 : PSSL_METHOD;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLv23_client_method)]
    function IdSslMethodClientV23 : PSSL_METHOD;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_TLSv1_method)]
    function IdSslMethodTLSV1:PSSL_METHOD;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_TLSv1_server_method)]
    function IdSslMethodServerTLSV1:PSSL_METHOD;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_TLSv1_client_method)]
    function IdSslMethodClientTLSV1 :PSSL_METHOD;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_shutdown)]
    function IdSslShutdown(s: PSSL):Integer;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_set_connect_state)]
    procedure IdSslSetConnectState(s: PSSL);  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_set_accept_state)]
    procedure IdSslSetAcceptState(s: PSSL);  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_set_shutdown)]
    procedure IdSslSetShutdown(ssl: PSSL; mode: Integer);  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_load_verify_locations)]
    function IdSslCtxLoadVerifyLocations(ctx: PSSL_CTX;
      CAfile: String;
      CApath: String):Integer; external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_get_session)]
    function IdSslGetSession(ssl: PSSL):PSSL_SESSION;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSLeay_add_ssl_algorithms)]
    function IdSslAddSslAlgorithms : Integer;   external;
  // IdSslSetAppData : function(s: PSSL; arg: Pointer):Integer cdecl = nil;
  // IdSslGetAppData : function(s: PSSL):Pointer cdecl = nil;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_info_callback)]
    procedure IdSslCtxSetInfoCallback(ctx: PSSL_CTX; cb: PFunction);  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_STORE_CTX_get_app_data)]
    function IdSslX509StoreCtxGetAppData(ctx: PX509_STORE_CTX): IntPtr;   external;
  ...
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_SESSION_get_id)]
    function IdSslSessionGetId(s: PSSL_SESSION; var id: String; var length: Integer):Integer;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_SESSION_get_id_ctx)]
    function IdSslSessionGetIdCtx (s: PSSL_SESSION; var id: String; var length: Integer):Integer;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_get_version)]
    function IdSslCtxGetVersion(ctx: PSSL_CTX):Integer;  external;
  [DllImport(SSL_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_options)]
    function IdSslCtxSetOptions(ctx: PSSL_CTX; op: Longint):Longint;   external;

  //Crypt Library

  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_NAME_oneline)]
    function IdSslX509NameOneline(a: PX509_NAME; var buf: String; size: Integer):String;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_NAME_hash)]
    function IdSslX509NameHash(x: PX509_NAME):Cardinal;   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_set_issuer_name)]
    function IdSslX509SetIssuerName(x: PX509; name: PX509_NAME):Integer;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_get_issuer_name)]
    function IdSslX509GetIssuerName(a: PX509):PX509_NAME;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_set_subject_name)]
    function IdSslX509SetSubjectName(x: PX509; name: PX509_NAME):Integer;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_get_subject_name)]
    function IdSslX509GetSubjectName(a: PX509):PX509_NAME;   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_digest)]
    function IdSslX509Digest (data: PX509; _type: PEVP_MD; md: String; var len: UInteger):Integer;   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint =  fn_EVP_md5)]
    function IdSslEvpMd5:PEVP_MD;   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_get_notBefore)]
    function  IdSslX509GetNotBefore(x509: PX509):  PASN1_UTCTIME;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_get_notAfter)]
    function IdSslX509GetNotAfter(x509: PX509):PASN1_UTCTIME;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_STORE_CTX_get_error)]
    function IdSslX509StoreCtxGetError(ctx: PX509_STORE_CTX):Integer;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_STORE_CTX_set_error)]
    procedure IdSslX509StoreCtxSetError(ctx: PX509_STORE_CTX; s: Integer);  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_STORE_CTX_get_error_depth)]
    function IdSslX509StoreCtxGetErrorDepth(ctx: PX509_STORE_CTX):Integer;   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_X509_STORE_CTX_get_current_cert)]
    function IdSslX509StoreCtxGetCurrentCert(ctx: PX509_STORE_CTX):PX509;  external;

  // 3DES functions
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_des_set_odd_parity)]
    procedure IdDES_set_odd_parity(Key: des_cblock);   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_des_set_key)]
    function IdDES_set_key(key: const_des_cblock; schedule: des_key_schedule): Integer;   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_des_ecb_encrypt)]
   procedure IdDES_ecb_encrypt(Input: des_cblock; output: des_cblock; ks: des_key_schedule; enc: Integer);   external;
  ...
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_set_ex_data)]
    function IdSSL_set_ex_data(ssl: PSSL; idx: Integer; data: IntPtr): Integer;  external;
  // void *SSL_get_ex_data(SSL *ssl,int idx);
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_get_ex_data)]
    function IdSSL_get_ex_data(ssl: PSSL; idx: integer): IntPtr;    external;

  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_load_client_CA_file)]
    function IdSSLLoadClientCAFile(const _file: String):PSTACK_X509_NAME;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_client_CA_list)]
    procedure IdSSLCtxSetClientCAList(ctx: PSSL_CTX; list: PSTACK_X509_NAME);   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_default_verify_paths)]
    function IdSSLCtxSetDefaultVerifyPaths(ctx: PSSL_CTX): Integer;   external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CTX_set_session_id_context)]
    function IdSSLCtxSetSessionIdContext(ctx: PSSL_CTX;
      const sid_ctx: String;
      sid_ctx_len: UInteger):Integer;  external;

  //SSL_CIPHER_description
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CIPHER_description )]
    function IdSSLCipherDescription(arg0: PSSL_CIPHER;
      buf: String;
  ...
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_get_current_cipher)]
    function IdSSLGetCurrentCipher(s: PSSL):PSSL_CIPHER;  external;

  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CIPHER_get_name)]
    function IdSSLCipherGetName(c: PSSL_CIPHER):String;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CIPHER_get_version)]
    function IdSSLCipherGetVersion(c: PSSL_CIPHER):String;  external;
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl,
    EntryPoint = fn_SSL_CIPHER_get_bits)]
    function IdSSLCipherGetBits(c: PSSL_CIPHER; var alg_bits: Integer):Integer;  external;

  //expirimental
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention= CallingConvention.cdecl ,
    EntryPoint = fn_ERR_error_string_n)]
    procedure IdSSLERR_error_string_n(AError : Cardinal; ErrorStr : String;
      Len : Cardinal);  external;
//unsigned long ERR_get_error(void );
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    //TODO:  See if this is right
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_get_error)]
    function IdSSLERR_get_err : Cardinal;   external;
   //unsigned long ERR_peek_error(void );
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    //TODO:  See if this is right
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_peek_error)]
    function IdSSLERR_peek_err : Cardinal;   external;
//  ERR_error_string(
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_clear_error)]
    procedure IdSSLERR_clear_error;  external;
//     char *ERR_error_string(unsigned long e,char *buf);

  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_error_string)]
    function IdSSLERR_error_string(e: cardinal; buff: String): String;  external;
  //const char *ERR_lib_error_string(unsigned long e);
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_lib_error_string)]
    function IdSSLERR_lib_error_string(e : Cardinal): String;  external;
  //const char *ERR_func_error_string(unsigned long e);
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_func_error_string)]
    function IdSSLERR_func_error_string(e : Cardinal): String;  external;
  //const char *ERR_reason_error_string(unsigned long e);
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_reason_error_string)]
    function IdSSLERR_reason_error_string(e : Cardinal): String;  external;
  // void ERR_load_ERR_strings(void);
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint =  fn_ERR_load_ERR_strings)]
    procedure IdSSLERR_load_ERR_strings;   external;
  //void ERR_load_crypto_strings(void);
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_load_crypto_strings)]
    procedure IdSSLERR_load_crypto_strings;   external;
  //void ERR_free_strings(void)
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_ERR_free_strings )]
    procedure IdSSLERR_free_strings;    external;

function IdSslUCTTimeDecode(var UCTtime : ASN1_UTCTIME; Var year, month, day, hour, min, sec: Word;
  Var tz_hour, tz_min: Integer): Integer;
//function IdSslSetAppData(s: PSSL; arg: IntPtr): Integer;
function IdSslGetAppData(s: PSSL): IntPtr;
...
uses
  ...
  [DllImport(SSLCLIB_DLL_name, CharSet = CharSet.Ansi,
    SetLastError = False,
    CallingConvention = CallingConvention.cdecl ,
    EntryPoint = fn_RAND_screen )]
    procedure IdSslRandScreen;  external;

var
   GFailedToLoad : String;
    GLoadedAttempt : Boolean;
    GLoaded : Boolean;
    // LIBEAY functions - open SSL 0.9.6a
    GSslRandScreen : Boolean;
...
Begin
  ...
  IdSSLERR_error_string_n(AErr,LString,300);
  Result := LString;
end;

procedure IndyInternalFailedToLoad(const AMod : HMODULE; var VRes : Boolean; const AProcName : String);
var LAddr : IntPtr;
begin
  LAddr := GetProcAddress(AMod, AProcName);
  if LAddr.ToInt32 = 0 then
  begin
    GFailedToLoad := GFailedToLoad + ' ' + AProcName;
    if VRes then
    begin
      VRes := False;
    end;
...
function LoadedAndSetup : Boolean;
var LIdCrypto, LIdSSL : HMODULE;
  LAddr : IntPtr;
begin
  Result := True;
  LIdSSL := 0;
  LIdCrypto := LoadLibrary(SSLCLIB_DLL_name);
  try
    if LIdCrypto=0 then
    begin
      GFailedToLoad := 'Failed to load '+SSLCLIB_DLL_name+'.';  {Do not localize}
      Result := False;
    end
    else
    begin
      LIdSSL := LoadLibrary(SSL_DLL_name);
    end;
    if (LIdCrypto<>0) and (LIdSSL<>0) then
    begin
      GFailedToLoad := '';  {Do not localize}
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_cipher_list);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_new);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_free);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_set_fd);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_use_PrivateKey_file);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_use_certificate_file);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_load_error_strings);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_state_string_long);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_get_peer_certificate);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_verify);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_verify_depth);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_get_verify_depth);

      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_default_passwd_cb);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_default_passwd_cb_userdata);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_check_private_key);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_new);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_free);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_accept);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_connect);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_read);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_peek);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_write);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_get_error);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv2_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv2_server_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv2_client_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv3_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv3_server_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv3_client_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv23_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv23_server_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLv23_client_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_TLSv1_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_TLSv1_server_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_TLSv1_client_method);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_shutdown);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_set_connect_state);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_set_accept_state);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_set_shutdown);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_load_verify_locations);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_get_session);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSLeay_add_ssl_algorithms);

    // Indy custom library

    // IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_set_app_data);
    // IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_get_app_data);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_info_callback);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_X509_STORE_CTX_get_app_data);
    // IndyInternalFailedToLoad(LIdSSL,Result,fn_UCTTimeDecode);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_SESSION_get_id);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_SESSION_get_id_ctx);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_get_version);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_options);
    //IndyInternalFailedToLoad
    //Crypto Lib
      IndyInternalFailedToLoad(LIdCrypto, Result,fn_X509_NAME_oneline);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_NAME_hash);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_set_issuer_name);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_get_issuer_name);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_set_subject_name);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_get_subject_name);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_digest);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_EVP_md5);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_STORE_CTX_get_error);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_STORE_CTX_set_error);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_STORE_CTX_get_error_depth);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_X509_STORE_CTX_get_current_cert);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_RAND_screen);

    // 3DES
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_des_set_odd_parity);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_des_set_key);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_des_ecb_encrypt);
    // More SSL functions

      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_set_ex_data);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_get_ex_data);

      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_load_client_CA_file);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_client_CA_list);

      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_default_verify_paths);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CTX_set_session_id_context);

      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CIPHER_description);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_get_current_cipher);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CIPHER_get_name);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CIPHER_get_version);
      IndyInternalFailedToLoad(LIdSSL,Result,fn_SSL_CIPHER_get_bits);

      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_get_error);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_peek_error);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_clear_error);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_error_string);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_error_string_n);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_lib_error_string );
      IndyInternalFailedToLoad(LIdCrypto,Result, fn_ERR_func_error_string );
      IndyInternalFailedToLoad(LIdCrypto,Result, fn_ERR_reason_error_string );
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_load_ERR_strings);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_load_crypto_strings);
      IndyInternalFailedToLoad(LIdCrypto,Result,fn_ERR_free_strings);

      LAddr := GetProcAddress(LIdCrypto,fn_RAND_screen);

      if Result then
      begin
        GSslRandScreen := (LAddr.ToInt32 = 0);
      end;

    end
    else
    begin
      GFailedToLoad := 'Failed to load '+SSL_DLL_name+'.';  {Do not localize}
      Result := False;
    end;
  finally
    if LIdCrypto <> 0 then
    begin
      FreeLibrary(LIdCrypto);
    end;
    if LIdSSL <> 0 then
    begin
      FreeLibrary(LIdCrypto);
...
procedure Unload;
begin
  //This is left empty because DLL's are managed in DotNET
  //Differently.  Load just loads the DLL's and sees if the functions
  //we need exist, than unloads.  THis is for compatibility
  //with Win32 behavior.
  //In DotNET, the DLL functions are dynamically when called and unloaded
  //at program termination.
end;

Function WhichFailedToLoad:String;
Begin
   Result := GFailedToLoad;
End;
...
begin
  ...
  time_Str := UCTTime.data;
   result := 1;
  // Check if first 12 chars are numbers
  for i := 1 to 12 do begin
    if (time_str[i] > '9') or (time_str[i] < '0') then
    begin
...
initialization
  GLoadedAttempt := False;
  GLoaded := False;
  GSslRandScreen := False;
end.
