...
uses
  ...
  BisFm, BisEvents, BisCrypter, BisValues, BisWave, BisThread,
...
//    procedure SendData(const Data: Pointer; const DataSize: Cardinal);
    ...
    function GetCallInfo(AccountId,Phone: Variant): Boolean;
...
uses Math,
     ...
     BisParamEditDataSelect, BisDataSet, BisParam, BisFilterGroups, BisLogger,
     BisTaxiConsts, BisTaxiDataDriversFm, BisTaxiDataClientsFm, BisTaxiDataDispatchersFm,
     BisTaxiPhoneFormatFm;
...
begin
  ...
  if Assigned(Result) then begin
    if FNumber<>'' then begin
      with TBisTaxiPhoneForm(Result) do begin
        SetAcceptorType(atPhone);
        SetAcceptor(FNumber);
        FOrderId:=Self.FOrderId;
      end;
      FNumber:='';
      FOrderId:=Null;
    end;
...
procedure TBisTaxiPhoneFormIface.Dial(Number: String; OrderId: Variant);
var
  Last: TBisTaxiPhoneForm;
begin
  Last:=TBisTaxiPhoneForm(LastForm);
...
begin
  ...
  FServer.ThreadName:='TaxiPhoneChannelSend';
...
procedure TBisTaxiPhoneChannel.StopSendThread;
begin
...
begin
  if (FState in [csRinning,csProcessing,csHolding]) then
    Hangup
  else
...
function TBisTaxiPhoneChannel.CanClose: Boolean;
begin
  Result:=true;
  if Result then begin
    if (FState in [csProcessing,csFinished]) and
       (Assigned(FFrame) and FFrame.Incoming and not FFrame.Flashing) then
      Result:=Assigned(FFrame) and FFrame.QueryCallResultId;
  end;
end;
...
function TBisTaxiPhoneChannel.GetCallInfo(AccountId,Phone: Variant): Boolean;
var
  P: TBisProvider;
begin
  Result:=false;
  if not VarIsNull(AccountId) or not VarIsNull(Phone) then begin
    P:=TBisProvider.Create(nil);
    try
      P.WithWaitCursor:=false;
      ...
      with P.Params do begin
        ...
        AddInvisible('ACCOUNT_PHONE').Value:=Phone;
...
begin
  if not VarIsNull(FCallId) and
     (Assigned(FFrame) and FFrame.CallResultQueried) then begin
    P:=TBisProvider.Create(nil);
    try
      P.WithWaitCursor:=false;
      ...
      with P.Params do begin
        ...
        AddInvisible('ORDER_ID').Value:=FOrderId;
      end;
      P.Execute;
      if P.Success then
        FCallId:=Null;
      ...
      try
        ...
        if GetRemoteEventParams(FRemoteSessionId) and Assigned(FRemoteFormat) then begin
          ...
          if FChannels.CanAdd then begin
            Result:=GetCallInfo(Request.CallerId,Request.CallerPhone);
            if Result then begin
              FState:=csRinning;
    ...
    try
      try
        ...
        if Assigned(FRemoteFormat) then begin
          ...
          FFrame.Active:=true;
          FChannels.StopMusic;
          FChannels.StartVoicePlayer;
          FChannels.StartRecorder;
          StartSendThread;
          ResponseType:=rtOK;
        end;
        Result:=true;
...
var
  Request: TBisTaxiPhoneRequest;
begin
  if FState in [csRinning,csProcessing,csHolding] then begin
...
begin
  ...
  if Result then begin
    Result:=FForm.UpDownMaxChannels.Position>=Count;
    if Result then begin
      for i:=0 to Count-1 do begin
        Item:=Items[i];
...
begin
  Result:=true;
  for i:=Count-1 downto 0 do begin
    if not Items[i].CanClose then begin
      Result:=false;
      exit;
    end;
  end;
...
function TBisTaxiPhoneChannels.AddOutgoing(OrderId: Variant): TBisTaxiPhoneChannel;
begin
  Result:=Add(GetUniqueID,false);
  if Assigned(Result) then
    Result.FOrderId:=OrderId;
end;
  ...
  if Assigned(FForm) then begin
    ...
    with FForm do begin
      if FIncomingStream.Size>0 then begin
        ...
        FMusicPlayer.PlayStream(FIncomingStream,iff(Full,MaxInt,1));
      end;
    end;
  end;
end;
...
procedure TBisTaxiPhoneChannels.StartVoicePlayer;
begin
  if Assigned(FForm) then begin
    FForm.FVoicePlayer.Active:=true;
    FForm.FVoicePlayer.WaitForStart;
  end;
end;

procedure TBisTaxiPhoneChannels.StartRecorder;
begin
  if Assigned(FForm) then begin
    FForm.FRecorder.Active:=true;
    FForm.FRecorder.WaitForStart;
  end;
end;
...
begin
  ...
  if Core.LocalBase.ReadParam(SParamRequestRetryCount,Buffer) then
     FChannels.FWaitRetryCount:=StrToIntDef(Buffer,0);
  if Core.LocalBase.ReadParam(SParamRequestTimeout,Buffer) then
     FChannels.FWaitTimeout:=StrToIntDef(Buffer,3000);
...
{function TBisTaxiPhoneForm.VoicePlayerDataPtr(Sender: TObject; var Buffer: Pointer; var NumLoops: Cardinal; var FreeIt: Boolean): Cardinal;
var
  ASize: Cardinal;
begin
  FVoicePlayerLock.Enter;
  try
    ...
    if ASize>0 then begin
      GetMem(Buffer,ASize);
      FVoicePlayerStream.Read(Buffer^,ASize);
      FVoicePlayerStream.Clear;
      FreeIt:=true;
      Result:=ASize;
    end else begin
...
begin
  ...
  try
    FreeIt:=true;
    if Assigned(FChannel) and (BufferSize=FChannel.FDataSize) and not ButtonMicOff.Down then begin
      FChannel.SendData(Buffer,BufferSize);
    end;
  finally
    FRecorderLock.Leave;
  end;
...
begin
  Result:=false;
  if Assigned(FEditBuffer) then begin
    FRecorderLock.Enter;
    try
      FRecorderStream.Position:=0;
      ASize:=FRecorderStream.Size;
      if ASize>=DataSize then begin
        ...
        if ASize>0 then begin
          Temp:=TMemoryStream.Create;
          try
            MaxSize:=Round((Channel.FDataSize*(1000+Cardinal(FEditBuffer.Value)))/Channel.FPacketTime);
            if ASize>MaxSize then begin
              ASize:=MaxSize;
              FRecorderStream.Position:=FRecorderStream.Size-ASize;
            end;
...
begin
  if NewTab=TabSheetPhone.PageIndex then
    PageControl.ActivePage:=TabSheetPhone
  else
...
begin
  AClass:=Core.FindIfaceClass(SClassDataRolesAndAccountsFormIface);
...
var
  ...
  AIface: TBisDataFormIface;
begin
  AClass:=Core.FindIfaceClass(SClassDataSessionsFormIface);
  Result:=Assigned(AClass) and IsClassParent(AClass,TBisDataFormIface);
  if Result then begin
    AIface:=TBisDataFormIfaceClass(AClass).Create(nil);
    try
...
var
  Form: TBisTaxiPhoneFormatForm;
begin
  Form:=TBisTaxiPhoneFormatForm.Create(nil);
  try
...
begin
  Result:=FChannels.Count<UpDownMaxChannels.Position;
  if Result then begin
...
begin
  ...
  if CanDial then begin
    ...
    SetAcceptor('');
    Channel:=FChannels.AddOutgoing(FOrderId);
    FOrderId:=Null;
    if Assigned(Channel) then begin
      Update;
      if AcceptorType=atPhone then
        Acceptor:=Channel.TransformPhone(Acceptor);
      if Channel.GetCallInfo(Null,Acceptor) then begin
