...
uses
  ...
  BisFm, BisDataFrm, BisDataEditFm, BisParam, BisGradient, BisThreads, BisProvider,
  BisDataSet, BisParamComboBoxDataSelect, 
  BisCallDataCallEditFm,                      
  BisTaxiHistoryFrm, BisTaxiAddressFrm, BisTaxiRouteFrm,                                                        
  BisTaxiDataInMessagesFm, BisTaxiDataOutMessagesFm, BisTaxiDataReceiptsFrm,
...
type
  ...
  TBisTaxiOrderEditForm = class(TBisDataEditForm)
    ...
    procedure DateTimePickerArrivalTimeChange(Sender: TObject);
    procedure DateTimePickerArrivalChange(Sender: TObject);
  private
    ...
    FRefreshProvider: TBisProvider;
    FAlreadyRefreshed: Boolean;
    FParamRates: TBisDataSetCollectionItem;
    FParamCarTypes: TBisDataSetCollectionItem;
    FParamParks: TBisDataSetCollectionItem;
    FParamSources: TBisDataSetCollectionItem;
    FParamActions: TBisDataSetCollectionItem;
    ...
    procedure UpdateRoute(Route: TBisTaxiRoute);
    ...
    function CreateCollectionItem(Param: TBisParamComboBoxDataSelect): TBisDataSetCollectionItem;
    procedure RefreshProviderThreadEnd(Sender: TObject);
...
function GetDefaultLocalityName: String;

implementation

uses DateUtils, Math,
     BisValues, BisLogger, BisDialogs, BisFieldNames, BisIfaces, 
     BisUtils, BisParamEditDataSelect, BisParamComboBox,
     BisParamEditCalc, BisFilterGroups, BisDataFm, BisOrders,
     BisTaxiDataRatesFm, BisTaxiDataCarTypesFm, BisCore, BisTaxiConsts,
     BisTaxiDataBlackEditFm, BisTaxiDataParksFm,
     ...
     BisTaxiPhoneFm, BisTaxiPhoneFrm, BisTaxiClientFm;
...
begin
  ...
  try
    ...
    P.UseShowWait:=false;
    P.UseCache:=true;
    P.ProviderName:='GET_DEFAULT_LOCALITY';
    with P.Params do
      AddInvisible('NAME',ptOutput);
    P.Execute;
    if P.Success then
      Result:=P.ParamByName('NAME').AsString;
  finally
    P.Free;
  end;
end;
...
begin
  ...
  with Params do begin
    ...
    with AddComboBoxDataSelect('SOURCE_ID','ComboBoxSource','LabelSource','',
                               TBisTaxiDataSourcesFormIface,'SOURCE_NAME',false,false,'','NAME') do begin
      AutoRefresh:=false;
      FilterGroups.Add.Filters.Add('VISIBLE',fcEqual,1);
    end;
...
begin
  ...
  SMessageSuccess:='Заказ № %ORDER_NUM успешно создан.';
  FCallId:=Null;
end;
...
begin
  ...
  FAddressFrame.DefaultLocalityName:='';
  ...
  with FRouteHistoryFrame do begin
    Parent:=PanelRouteHistory;
    Align:=alClient;
    Caption:='История маршрутов';
    AsModal:=true;
    OnAddressChange:=RouteHistoryAddressChange;
    OnRouteChange:=RouteHistoryAddressChange;
  end;

  FRefreshProvider:=TBisProvider.Create(nil);
  FRefreshProvider.Threaded:=true;
  FRefreshProvider.UseWaitCursor:=false;
  FRefreshProvider.UseShowWait:=false;
  FRefreshProvider.OnThreadEnd:=RefreshProviderThreadEnd;
        ...
        if P.Success then begin
          ...
          if Assigned(Provider.ParentDataSet) and Provider.ParentDataSet.Active then begin
            if Provider.ParentDataSet.Locate('ORDER_ID',Param.Value,[loCaseInsensitive]) then begin
              ...
              Provider.ParentDataSet.Synchronize;
              Provider.ParentDataSet.Post;
            end;
          end;
        end;
...
begin
  Result:=Check;
  if Result and Assigned(Provider.ParentDataSet) and Provider.ParentDataSet.Active then begin
    S:=FormatEx('Заказ № %s заблокирован. Разблокировать?',
...
function TBisTaxiOrderEditForm.CreateCollectionItem(Param: TBisParamComboBoxDataSelect): TBisDataSetCollectionItem;

  function GetParamProvider(FilterGroups: TBisFilterGroups; Orders: TBisOrders): String;
  var
    AClass: TComponentClass;
    Iface: TBisDataFormIface;
  begin
     AClass:=Param.DataClass;
    if not Assigned(AClass) and Assigned(Core) then
      AClass:=Core.FindIfaceClass(Param.DataClassName);
    if Assigned(AClass) and IsClassParent(AClass,TBisDataFormIface) then begin
      Iface:=TBisDataFormIfaceClass(AClass).Create(nil);
      try
        Iface.Init;
        FilterGroups.CopyFrom(Iface.FilterGroups);
        FilterGroups.CopyFrom(Param.FilterGroups,false);
        Orders.CopyFrom(Iface.Orders);
        Orders.CopyFrom(Param.Orders,false);
        Result:=Iface.ProviderName;
      finally
        Iface.Free;
      end;
    end else begin
      FilterGroups.CopyFrom(Param.FilterGroups);
      Orders.CopyFrom(Param.Orders);
      Result:=Param.ProviderName;
    end;
  end;
...
begin
  ...
  if Assigned(Param) then begin
    ...
    try
      DS.ProviderName:=GetParamProvider(DS.FilterGroups,DS.Orders);
      if DS.ProviderName<>'' then
        Result:=FRefreshProvider.CollectionAfter.AddDataSet(DS);
    finally
      DS.Free;
    end;
  end;
...
procedure TBisTaxiOrderEditForm.RefreshProviderThreadEnd(Sender: TObject);

  procedure SetParamValues(Param: TBisParamComboBoxDataSelect; Item: TBisDataSetCollectionItem);
  var
    DS: TBisDataSet;
    P2: TBisParam;
    Old1,Old2: TBisParamChangeEvent;
  begin
    if Assigned(Param) and Assigned(Item) then begin
      DS:=TBisDataSet.Create(nil);
      Old1:=Param.OnChange;
      Param.OnChange:=nil;
      P2:=Provider.ParamByName(Param.DataName);
      Old2:=P2.OnChange;
      P2.OnChange:=nil;
      try
        if Item.GetDataSet(DS) then
          Param.RefreshValues(DS);
      finally
        P2.OnChange:=Old2;
        Param.OnChange:=Old1;
        DS.Free;
...
var
  i: Integer;
  Old: TBisParamChangeEvent;
begin
  SetParamValues(TBisParamComboBoxDataSelect(Provider.ParamByName('RATE_ID')),FParamRates);
  SetParamValues(TBisParamComboBoxDataSelect(Provider.ParamByName('CAR_TYPE_ID')),FParamCarTypes);
  SetParamValues(TBisParamComboBoxDataSelect(Provider.ParamByName('PARK_ID')),FParamParks);
  SetParamValues(TBisParamComboBoxDataSelect(Provider.ParamByName('SOURCE_ID')),FParamSources);
  SetParamValues(TBisParamComboBoxDataSelect(Provider.ParamByName('ACTION_ID')),FParamActions);

  for i:=0 to FRoutes.Count-1 do
    UpdateRoute(FRoutes.Items[i]);
...
{  if Mode in [emUpdate,emView,emDelete] then begin
    ChangeParam(Provider.Params.ParamByName('TYPE_PROCESS'));
  end; }

  if Mode in [emUpdate,emView,emDelete] then begin
    if Assigned(Provider.ParentDataSet) then begin
      with Provider do begin

        Old:=Params.OnChange;
        Params.OnChange:=nil;
        try
          ParamByName('RATE_ID').SetNewValue(ParentDataSet.FieldByName('RATE_ID').Value);
          ParamByName('CAR_TYPE_ID').SetNewValue(ParentDataSet.FieldByName('CAR_TYPE_ID').Value);
          ParamByName('PARK_ID').SetNewValue(ParentDataSet.FieldByName('PARK_ID').Value);
          ParamByName('SOURCE_ID').SetNewValue(ParentDataSet.FieldByName('SOURCE_ID').Value);
          ParamByName('TYPE_PROCESS').SetNewValue(ParentDataSet.FieldByName('TYPE_PROCESS').Value);
        finally
          Params.OnChange:=Old;
        end;

        ParamByName('ACTION_ID').SetNewValue(ParentDataSet.FieldByName('ACTION_ID').Value);
        ParamByName('RESULT_ID').SetNewValue(ParentDataSet.FieldByName('RESULT_ID').Value);
...
begin
  ...
  FRefreshProvider.CollectionAfter.Clear;

  FParamRates:=CreateCollectionItem(TBisParamComboBoxDataSelect(Provider.ParamByName('RATE_ID')));
  FParamCarTypes:=CreateCollectionItem(TBisParamComboBoxDataSelect(Provider.ParamByName('CAR_TYPE_ID')));
  FParamParks:=CreateCollectionItem(TBisParamComboBoxDataSelect(Provider.ParamByName('PARK_ID')));
  FParamSources:=CreateCollectionItem(TBisParamComboBoxDataSelect(Provider.ParamByName('SOURCE_ID')));
  FParamActions:=CreateCollectionItem(TBisParamComboBoxDataSelect(Provider.ParamByName('ACTION_ID')));

  FRefreshProvider.Open;
  ...
  try
    FAddressFrame.DefaultLocalityName:=GetDefaultLocalityName;
...
begin
  ...
  try
    ...
    LoggerWrite(Format('Время открытия = %d msec',[GetTickDifference(Tick,Freq,dtMilliSec)]));
...
begin
  ...
  if Mode in [emDelete] then begin
    ...
    FHistoryFrame.ShowType:=ShowType;
    ...
    FRouteHistoryFrame.ShowType:=ShowType;
    ActiveControl:=FAddressFrame.ComboBoxStreet;
  end;
...
begin
  ...
  if Assigned(Param) then begin
    ...
    if (Mode in [emInsert,emUpdate]) and FAlreadyRefreshed and
    ...
    if FAlreadyShowed and
      ...
      if FNeedClientChange then begin
        ...
        if not Param.Empty then
          FlagAccept:=ShowQuestion('Взять адрес клиента в качестве адреса подачи?')=mrYes;
    ...
    if (Mode in [emInsert,emUpdate]) and FAlreadyShowed and FAlreadyRefreshed and
...
//        Calculate;
    ...
    if(Mode in [emInsert,emUpdate,emView]) and FAlreadyShowed and FAlreadyRefreshed and
    ...
    if (Mode in [emInsert,emUpdate,emView]) and FAlreadyRefreshed and 
    ...
    if (Mode in [emInsert,emUpdate,emView]) and     
...
begin
  Result:=(inherited ChangesExists or FChangeServices or FChangeTypeProcess or
           FChangeRoutes or FChangeAddress or FChangeZone or FChangeCost) and FAlreadyShowed;
end;
...
procedure TBisTaxiOrderEditForm.UpdateRoute(Route: TBisTaxiRoute);
var
  RateParam: TBisParamComboBoxDataSelect;
  TypeRate: TBisTaxiTypeRate;
begin
...
procedure TBisTaxiOrderEditForm.DateTimePickerArrivalChange(Sender: TObject);
begin
  if FAlreadyShowed then begin
    TimerCostGross.Enabled:=false;
    TimerCostGross.Enabled:=true;
  end;
end;

procedure TBisTaxiOrderEditForm.DateTimePickerArrivalTimeChange(Sender: TObject);
begin
  if FAlreadyShowed then begin
    TimerCostGross.Enabled:=false;
    TimerCostGross.Enabled:=true;
...
begin
  ...
  if FAlreadyShowed and NewRoute.Panel.Visible and NewRoute.Frame.ComboBoxStreet.CanFocus then
...
begin
  if CanBlackInsert then begin
    ...
    if not ExistsPhoneBlack(ParamPhone.AsString,Description) then begin
      ...
      ShowError(FormatEx('Телефон %s уже находится в черном списке.'+#13#10+
                         'Описание: %s',
  ...
  if CanCheckPhoneBlack then begin
    ...
    if not Result then
      if QuestionMessage then begin
        if ShowWarningQuestion(FormatEx('Телефон %s находится в черном списке.'+#13#10+
                                        'Описание: %s'+#13#10+#13#10+
                                        'Пропустить ввод?',
        ...
        ShowWarning(FormatEx('Телефон %s находится в черном списке.'+#13#10+
                             'Описание: %s',
                             [ParamPhone.Value,Description]))
    else
      if SuccessMessage then
        ShowInfo(FormatEx('Телефон %s не находится в черном списке.',[ParamPhone.Value]));
  end;
end;
  ...
  begin
    ...
    if Trim(ProcName)<>'' then begin
      ...
      try
        ...
        with P.Params do begin
          CopyFrom(Provider.Params,false,false,[ptInput,ptOutput],true);
...
begin
  if CanCalculate then begin
    ...
    RateParam:=TBisParamComboBoxDataSelect(Provider.Params.ParamByName('RATE_ID'));
    RoundUp:=Null;
    if not RateParam.Empty then
      RoundUp:=RateParam.Values.GetValue('ROUND_UP');

    CostGross:=CostGrossParam.AsExtended;

    if Gross or (CostGross<=0.0) then begin
      CostGross:=GetCost(BaseCost,BaseDistance,BasePeriod);
      if FAlreadyShowed then
    ...
    if Rate then begin
      ...
      if not VarIsNull(RoundUp) then begin
        RoundUpV:=VarToExtendedDef(RoundUp,0.0);
        if RoundUpV<>0.0 then begin
          CostRate:=Round(CostRate/RoundUpV)*RoundUpV;
        end;
      end;
...
procedure TBisTaxiOrderEditForm.SetOrderServices;
var
  Obj: TBisTaxiService;
...
procedure TBisTaxiOrderEditForm.SetRoutes;
var
  OrderParam: TBisParam;
  Params: TBisParams;
  Route: TBisTaxiRoute;
...
function TBisTaxiOrderEditForm.CreateOrderHistory(var NewOrderId: Variant): Boolean;
var
  OrderParam: TBisParam;
  P: TBisProvider;
begin
...
procedure TBisTaxiOrderEditForm.Execute;
  ...
  var
    Params: TBisParams;
  begin
    if not VarIsNull(FCallId) then begin
      Params:=Provider.PackageAfter.Add;
      Params.ProviderName:='CALL_SET_ORDER_ID';
      with Params do begin
        AddInvisible('CALL_ID').Value:=FCallId;
        AddInvisible('ORDER_ID').Value:=Provider.Params.Find('ORDER_ID').Value;
      end;
    end;
  end;
  ...
  try
    ...
    case Mode of
      ...
      emUpdate: begin
        ...
        if CreateOrderHistory(NewOrderId) then begin
          SetOrderServices;
          SetRoutes;
//          SetCallId; need check current call
          Provider.Execute;
          GetParams(NewOrderId);
...
begin
  if Assigned(Provider.ParentDataSet) and not VarIsNull(OrderId) then begin
    Old:=Provider.Params.OnChange;
    Provider.Params.OnChange:=nil;
    P:=TBisProvider.Create(nil);
    try
      P.ProviderName:=Provider.ParentDataSet.ProviderName;
      ...
      P.Free;
      Provider.Params.OnChange:=Old;
    end;
  end;
end;

procedure TBisTaxiOrderEditForm.ProcessResult(ResultId: Variant);
var
  P: TBisProvider;
begin
...
function TBisTaxiOrderEditForm.SetDefaultResultId: Boolean;
var
...
type
  TBisTaxiMapObject=class(TObject)
  private
    FStreetId: Variant;
    FHouse: Variant;
    FDistance: Variant;
  ...
  TBisTaxiMapObjects=class(TObjectList)
  protected
    function GetItem(Index: Integer): TBisTaxiMapObject;
  public
    procedure Add(Item: TBisTaxiMapObject); overload;
    function Add(StreetId, House: Variant): TBisTaxiMapObject; overload;
    procedure GetItems(StreetId, House: Variant; List: TBisTaxiMapObjects);

    property Items[Index: Integer]: TBisTaxiMapObject read GetItem; default;
  end;

{ TBisTaxiMapObject }

constructor TBisTaxiMapObject.Create;
begin
  inherited Create;
  FStreetId:=Null;
  FHouse:=Null;
...
function TBisTaxiMapObject.GetDistance: Variant;
begin
  Result:=FDistance;
  if Assigned(FParams) then
    Result:=FParams.ParamByName('DISTANCE').Value;
end;

{ TBisTaxiMapObjects }

function TBisTaxiMapObjects.GetItem(Index: Integer): TBisTaxiMapObject;
begin
  Result:=TBisTaxiMapObject(inherited Items[Index]);
end;

procedure TBisTaxiMapObjects.Add(Item: TBisTaxiMapObject);
begin
  inherited Add(Item);
end;

function TBisTaxiMapObjects.Add(StreetId, House: Variant): TBisTaxiMapObject;
begin
  Result:=TBisTaxiMapObject.Create;
  Result.FStreetId:=StreetId;
  Result.FHouse:=House;
  Add(Result);
end;

procedure TBisTaxiMapObjects.GetItems(StreetId, House: Variant; List: TBisTaxiMapObjects);
var
  Item: TBisTaxiMapObject;
  i: Integer;
begin
  for i:=0 to Count-1 do begin
    Item:=Items[i];
    if VarSameValueEx(Item.FStreetId,StreetId) and
       VarSameValueEx(Item.FHouse,House) then begin
      List.Add(Item);
    end;
  end;
end;
...
procedure TBisTaxiOrderEditForm.SetDistanceByMap(WithCalc: Boolean);
  ...
  procedure GetMapObjects(P: TBisProvider; Objects: TBisTaxiMapObjects);
  var
    i: Integer;
    CanOpen: Boolean;
    Item: TBisTaxiMapObject;
    Params: TBisParams;
    FromStreetId, FromHouse: Variant;
    ToStreetId, ToHouse: Variant;
    List: TBisTaxiMapObjects; 
  begin
    ...
    for i:=0 to Objects.Count-1 do begin
      ...
      FromStreetId:=Item.FStreetId;
      FromHouse:=Item.FHouse;

      if not VarIsNull(FromStreetId) and not VarIsNull(FromStreetId) then begin
        if not (VarSameValueEx(FromStreetId,ToStreetId) and
                VarSameValueEx(FromHouse,ToHouse)) then begin
          with P.FilterGroups.Add(foOr) do begin
            Filters.Add('STREET_ID',fcEqual,Item.FStreetId).CheckCase:=true;
            Filters.Add('HOUSE',fcEqual,Item.FHouse);
          end;
        end;
        CanOpen:=true;
      end;
      ...
      if not VarIsNull(FromStreetId) and not VarIsNull(FromHouse) and
         not VarIsNull(ToStreetId) and not VarIsNull(ToHouse) then begin

        if VarSameValueEx(FromStreetId,ToStreetId) and
           VarSameValueEx(FromHouse,ToHouse) then begin
          Item.FDistance:=0.0;
        end else begin
          Params:=P.PackageAfter.Add;
          with Params do begin
            ProviderName:='GET_ROUTE_DISTANCE';
            AddInvisible('FROM_STREET_ID').Value:=FromStreetId;
            AddInvisible('FROM_HOUSE').Value:=FromHouse;
            AddInvisible('TO_STREET_ID').Value:=ToStreetId;
            AddInvisible('TO_HOUSE').Value:=ToHouse;
    ...
    if P.Active then begin
      List:=TBisTaxiMapObjects.Create;
      try
        List.OwnsObjects:=false;
        P.First;
        while not P.Eof do begin
          List.Clear;
          Objects.GetItems(P.FieldByName('STREET_ID').Value,P.FieldByName('HOUSE').Value,List);
...
begin
  ...
  if not RateParam.Empty then begin
    ...
    if (Mode in [emInsert,emUpdate]) and (TypeRate in [trMap1km]) then begin
      P:=TBisProvider.Create(nil);
      Objects:=TBisTaxiMapObjects.Create;
      try

        Objects.Add(FAddressFrame.StreetId,FAddressFrame.House);
        for i:=0 to FRoutes.Count-1 do begin
          Route:=FRoutes.Items[i];
          Objects.Add(Route.Frame.StreetId,Route.Frame.House);
        end;

        GetMapObjects(P,Objects);

        FAddressFrame.AddressFound:=Objects.Items[0].FExists;
        FAddressFrame.ImageAddress.Visible:=true;
        for i:=0 to FRoutes.Count-1 do begin
          Route:=FRoutes.Items[i];
          Route.Frame.DisableChanges;
          try
            Item:=Objects.Items[i+1];
            Route.Frame.AddressFound:=Item.FExists;
            Route.Frame.ImageAddress.Visible:=true;
            Route.Frame.Distance:=Item.GetDistance;
          finally
            Route.Frame.EnableChanges;
          end;
        end;
...
begin
  ...
  if (Mode in [emInsert]) and not ParamUserName.Empty then begin
    ...
    try
      ...
      P.ProviderName:='GET_CLIENT_BY_USER_NAME';
      with P.Params do begin
        AddInvisible('USER_NAME',ptInput).Value:=ParamUserName.Value;
        AddInvisible('CLIENT_ID',ptOutput);
        AddInvisible('PHONE',ptOutput);
        AddInvisible('BALANCE',ptOutput);
        AddInvisible('SURNAME',ptOutput);
        AddInvisible('NAME',ptOutput);
        AddInvisible('PATRONYMIC',ptOutput);
        AddInvisible('FIRM_SMALL_NAME',ptOutput);
        AddInvisible('JOB_TITLE',ptOutput);
        AddInvisible('LOCALITY_ID',ptOutput);
        AddInvisible('LOCALITY_PREFIX',ptOutput);
        AddInvisible('LOCALITY_NAME',ptOutput);
        AddInvisible('STREET_ID',ptOutput);
        AddInvisible('STREET_PREFIX',ptOutput);
        AddInvisible('STREET_NAME',ptOutput);
        AddInvisible('HOUSE',ptOutput);
        AddInvisible('FLAT',ptOutput);
        AddInvisible('PORCH',ptOutput);
        AddInvisible('ADDRESS_DESC',ptOutput);
      end;
      P.Execute;
      if P.Success then begin
        if not P.ParamByName('PHONE').Empty then begin
          Form:=TBisTaxiClientForm.Create(nil);
          try
            Form.Init;
            Form.SetParams(P);
            if Form.ShowModal=mrOk then begin
              with Provider do begin
                ParamByName('CLIENT_ID').Value:=P.ParamByName('CLIENT_ID').Value;
                ParamByName('PHONE').Value:=P.ParamByName('PHONE').Value;
                ParamByName('CLIENT_SURNAME').Value:=P.ParamByName('SURNAME').Value;
                ParamByName('CLIENT_NAME').Value:=P.ParamByName('NAME').Value;
                ParamByName('CLIENT_PATRONYMIC').Value:=P.ParamByName('PATRONYMIC').Value;
                ParamByName('CLIENT_FIRM_SMALL_NAME').Value:=P.ParamByName('FIRM_SMALL_NAME').Value;
                P1:=ParamByName('CLIENT_SURNAME;CLIENT_NAME;CLIENT_PATRONYMIC;CLIENT_FIRM_SMALL_NAME');
                P1.Value:=(FormatEx(P1.ParamFormat,[P.ParamByName('SURNAME').Value,
                                                    P.ParamByName('NAME').Value,
                                                    P.ParamByName('PATRONYMIC').Value,
                                                    P.ParamByName('FIRM_SMALL_NAME').Value]));
                ParamByName('DESCRIPTION').Value:=P.ParamByName('ADDRESS_DESC').Value;
              end;
              FAddressFrame.LocalityId:=P.ParamByName('LOCALITY_ID').Value;
              FAddressFrame.StreetId:=P.ParamByName('STREET_ID').Value;
              FAddressFrame.House:=P.ParamByName('HOUSE').Value;
              FAddressFrame.Flat:=P.ParamByName('FLAT').Value;
              FAddressFrame.Porch:=P.ParamByName('PORCH').Value;
            end;
          finally
            Form.Free;
          end;
...
begin
  ...
  if (Mode in [emInsert]) and not ParamPhone.Empty then begin
    ...
    try
      P.UseWaitCursor:=false;
      P.ProviderName:='GET_CLIENT_BY_PHONE';
      with P.Params do begin
        AddInvisible('PHONE',ptInput).Value:=ParamPhone.Value;
        AddInvisible('CLIENT_ID',ptOutput);
        AddInvisible('USER_NAME',ptOutput);
        AddInvisible('BALANCE',ptOutput);
        AddInvisible('SURNAME',ptOutput);
        AddInvisible('NAME',ptOutput);
        AddInvisible('PATRONYMIC',ptOutput);
      ...
      P.Execute;
      if P.Success then begin
        if not P.ParamByName('USER_NAME').Empty then begin
          Form:=TBisTaxiClientForm.Create(nil);
          try
            Form.Init;
            Form.SetParams(P);
            if Form.ShowModal=mrOk then begin
              with Provider do begin
                ParamByName('CLIENT_ID').Value:=P.ParamByName('CLIENT_ID').Value;
                ParamByName('CLIENT_USER_NAME').Value:=P.ParamByName('USER_NAME').Value;
                ParamByName('CLIENT_SURNAME').Value:=P.ParamByName('SURNAME').Value;
                ParamByName('CLIENT_NAME').Value:=P.ParamByName('NAME').Value;
                ParamByName('CLIENT_PATRONYMIC').Value:=P.ParamByName('PATRONYMIC').Value;
                ParamByName('CLIENT_FIRM_SMALL_NAME').Value:=P.ParamByName('FIRM_SMALL_NAME').Value;
...
begin
  ...
  if (Mode in [emInsert]) and
    ...
    try
      ...
      PanelRouteHistory.BringToFront;
    finally
