...
{$IFDEF LEVEL9}
        procedure ClearCalcFields(Buffer:{$IFDEF DOTNET}TRecordBuffer{$ELSE}PChar{$ENDIF}); override;
{$ENDIF}
...
begin
  {$IFDEF LEVEL10}
     sz:=(length(AWideString)+1)*sizeof(WideChar); // Includes ending 00.
     p:=PChar(ABuffer);
  {$ELSE}
...
{$IFDEF LEVEL9}
procedure TkbmCustomMemTable.ClearCalcFields(Buffer:{$IFDEF DOTNET}TRecordBuffer{$ELSE}PChar{$ENDIF});
 {$IFDEF DOTNET}
var
   i:integer;
   pRec:PkbmRecord;
                    ...
                    case iOperator of
                         coFUNC2:
                            begin
                                 ...
                                 else if S1='GETDATE' then
                                    Result:=Now

                                 else if S1='YEAR' then
                                 begin
                                      Arg2:=ParseNode(pfdStart,pArg2);
                                      if VIsNull(Arg2) then Result:=Null
                                      else
                                 ...
                                 else if S1='DATE' then  // Format DATE('datestring','formatstring')
                                 ...
                                 else if S1='TIME' then  // Format TIME('timestring','formatstring')
...
// ba[i]:=Marshal.ReadByte(pfdStart,i);
     ...
     // Show header.
//     Dialogs.ShowMessage(Format('Version=%d, TotalSize=%d, Nodes=%d, NodeStart=%d, LiteralStart=%d',
                    ...
                    case iOperator of
                         coCONST2:
                           begin
                                ...
                                // Check type.
                                case DataType of
                                     ftSmallInt: Result:=PSmallInt(pArg1)^;
                                     ftWord: Result:=PWord(pArg1)^;
...
//     else Result:=PkbmRecord(ActiveBuffer)^.RecordNo+1;
  // by TSV
     if (State=dsInactive) or IsEmpty or (ActiveBuffer=nil) or
        ((ActiveBuffer<>nil) and (PkbMRecord(ActiveBuffer)^.RecordNo<0)) then Result:=-1
     else Result:=PkbmRecord(ActiveBuffer)^.RecordNo+1;
end;
{$ENDIF}
...
begin
     ...
     for i:=0 to List1.Count-1 do
         if (List1.Fields[i]<>List2.Fields[i]) or
            (ASameCase and ((mtifoCaseInsensitive in List1.Options[i])<>(mtifoCaseInsensitive in List2.Options[i]))) then exit;
     Result:=true;
end;
