...
type
  ...
  public
    procedure LockAddStart(Request: TBisSipRequest; Method: TBisSipRequestMethod);
  ...
  private
    ...
    procedure WaitsError(Thread: TBisThread; const E: Exception);
...
const
  ...
  SMaxThreadCount='Max thread count';
...
begin
  ...
  FClient:=nil;
  FreeAndNilEx(FReceivers);
  FLock.Free;
  inherited Destroy;
end;

procedure TBisSipTransport.ThreadsItemRemove(Sender: TBisThreads; Item: TBisThread);
begin
  Item.OnDestroy:=nil;
...
procedure TBisSipTransport.ThreadDestroy(Thread: TBisThread);
begin
...
begin
  ...
  if not Active then begin
    ...
    try
      ...
      MaxPort:=POWER_2;
      Counter:=0;
      while (First<MaxPort) and (Counter<FMaxRetryCount) do begin
...
begin
  ...
  if Assigned(AThread) and Assigned(FClient) then begin
    try
      ...
      try
        ...
        if ParseStrings(Strings,Headers,H1,H2,H3,LastIndex) then begin
          ...
          if Assigned(CallID) then begin
            ...
            if Assigned(FReceivers) then begin
...
begin
  try
    if Active then begin
      ...
      Flag:=Assigned(FThreads);
      if FCheckRemoteIP then
        Flag:=Flag and (ABinding.PeerIP=FClient.RemoteIP) and
        ...
        if FThreads.TryLock then begin
          try
            if FThreads.Count<FThreads.MaxCount then begin
              ...
              FClient.DoError(SMaxThreadCount);
          finally
            FThreads.UnLock;
          end;
        end;
...
function TBisSipSession.ReceiveRequest(Request: TBisSipRequest): Boolean;
  ...
  begin
    if FDirection=sdIncoming then begin
...
//    RequestAck(Request);
  ...
  procedure ProcessUnknown(Request: TBisSipRequest);
  begin
    FState:=ssDestroying;
    FClient.DoSessionTerminate(Self);
    FReadyForDestroy:=true;
            ...
            try
              ...
              if Flag then begin
                ...
                case Response.ResponseKind of
                  ...
                  rkPaymentRequired,rkForbidden,rkNotFound,
                  rkMethodNotAllowed,rkNotAcceptableClient: begin
                    ProcessUnknown(Request);
                    Result:=true;
                  end;
                  ...
                  rkRequestTimeout,rkGone,rkRequestEntityTooLarge,
                  rkRequestURITooLarge,rkUnsupportedMediaType,rkUnsupportedURIScheme,
                  rkBadExtension,rkExtensionRequired,rkIntervalTooBrief,
                  rkTemporarilyUnavailable,rkCallLegOrTransactionDoesNotExist,
                  rkLoopDetected,rkTooManyHops,rkAddressIncomplete,
                  rkAmbiguous,rkNotAcceptableHere,rkBadEvent,
                  rkRequestPending,rkUndecipherable,rkInternalServerError,
                  rkNotImplemented,rkBadGateway,rkServiceUnavailable,
                  rkServerTimeOut,rkSIPVersionNotSupported,rkMessageTooLarge,
                  rkBusyEverywhere,rkDecline,rkDoesNotExistAnywhere,
                  rkNotAcceptableGlobal: begin
                    ProcessUnknown(Request);
                    Result:=true;
                  end;
                else
...
procedure TBisSipRequestWaitThreads.DoItemRemove(Item: TBisThread);
var
...
begin
  if Assigned(Item) and (Item is TBisSipRequestWaitThread) then begin
    ...
    Thread.FRequest:=nil;
    Thread.FMethod:=nil;
    Thread.OnDestroy:=nil;
    if Thread.Working and Destroying then
      Thread.Stop
    else
      Thread.Terminate;
  end;
...
procedure TBisSipRequestWaitThreads.ThreadDestroy(Thread: TBisThread);
begin
  if Assigned(Self) and not Self.Destroying then
    LockRemove(Thread);
end;

procedure TBisSipRequestWaitThreads.ThreadError(Thread: TBisThread; const E: Exception);
begin
  if Assigned(FOnError) then
    FOnError(Thread,E);
end;

procedure TBisSipRequestWaitThreads.LockAddStart(Request: TBisSipRequest; Method: TBisSipRequestMethod);
var
  Item: TBisSipRequestWaitThread;
begin
  Lock;
  try
    Item:=TBisSipRequestWaitThread.Create(FTimeOut,Request,Method);
    Item.OnTimeout:=FOnTimeOut;
    Item.OnDestroy:=ThreadDestroy;
    Item.OnError:=ThreadError;
    inherited Add(Item);
    Item.Start;
  finally
    UnLock;
...
begin
  ...
  FWaits.OwnsObjects:=false;
  FWaits.FOnTimeOut:=WaitsTimeOut;
  FWaits.FOnError:=WaitsError;
  ...
  FRegisterThread.StopOnDestroy:=true;
  ...
  FKeepAliveThread.StopOnDestroy:=true;
...
begin
  try
    if FRegistered and (FState=csDefault) and Assigned(FTransport) then
...
{procedure TBisSipClient.WaitsEnd(Thread: TBisThread);
begin
  if Assigned(AException) then
    DoError(AException.Message);
end;}

procedure TBisSipClient.WaitsError(Thread: TBisThread; const E: Exception);
begin
  if Assigned(E) then
    DoError(E.Message);
end;
...
begin
  if Assigned(Request) then begin
    if WithAdd then
      Requests.LockAdd(Request);
    if Assigned(FTransport) and FTransport.Send(FRemoteIP,FRemotePort,Request.AsString) and WithWait then
      FWaits.LockAddStart(Request,Method);
  end;
end;

procedure TBisSipClient.SendResponse(Response: TBisSipResponse);
begin
  if Assigned(Response) and Assigned(FTransport) then
              ...
              if Result then begin
                ...
                case Response.ResponseKind of
                  ...
                  rkServiceUnavailable: begin
                    if not FRegistered and  (FState in [csRegistering]) then begin
                      Register(true);
                    end;
                  end;
                  rkOK: begin
...
begin
  ...
  if Assigned(FTransport) and FTransport.Active then begin
    Method:=nil;
