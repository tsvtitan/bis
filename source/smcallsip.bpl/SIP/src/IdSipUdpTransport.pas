...
uses
  ...
  IdSocketHandle, IdTimerQueue, IdUDPServer, IdGlobal, SysUtils;
...
//    B.SendTo(Dest.PeerIP, Dest.PeerPort, S[1], Length(S));
    ...
    B.SendTo(Dest.PeerIP, Dest.PeerPort, ToBytes(S));
  end;
end;
...
procedure TIdSipUdpServer.DoUDPRead(AData: TIdBytes; ABinding: TIdSocketHandle);
const
  MaxUdpPacketSize = 64*1024;
var
  Msg:             TIdSipMessage;
...
begin
  ...
  try
    ...
    try
      ...
      Msg := TIdSipMessage.ReadMessageFrom(Stream);
      try
        // Msg might not have a Content-Length header. Cf. RFC 3261, section 18.3:
        //
        //   If the message [using a message-oriented transport] has no
