...
type
  ...
  TBisSmsServerModemThread=class(TBisThread)
  private
    FModem: TBisGsmModem;
  ...
  public
    ...
    property Server: TBisSmsServer read FServer write FServer;
  ...
  TBisSmsServer=class(TBisServer)
  private
    FSStart: String;
    FSStop: String;
    FModems: TBisSmsServerModems;
...
begin
  ...
  AModule.ServerClass:=TBisSmsServer;
end;
...
procedure TBisSmsServerModemThread.DoStatus(Message: String; LogType: TBisLoggerType=ltInformation);
begin
  FMessage:=Message;
  FLoggerType:=LogType;
...
constructor TBisSmsServer.Create(AOwner: TComponent);
var
  Buffer: String;
begin
  inherited Create(AOwner);
  Params.OnChange:=ChangeParams;
  FModems:=TBisSmsServerModems.Create(Self);

  FOnlyOneModem:=false;
  if Core.LocalBase.ReadParam(SParamOnlyOneModem,Buffer) then
    FOnlyOneModem:=Boolean(StrToIntDef(Buffer,0));

  FSStart:='Отрытие портов ...';
  FSStartSuccess:='Отрытие портов выполнено успешно.';
  FSStartFail:='Отрытие портов не выполнено. Ошибка: %s';
  FSStop:='Закрытие портов ...';
  FSStopSuccess:='Закрытие портов выполнено успешно.';
  FSStopFail:='Закрытие портов не выполнено. Ошибка: %s';
  ...
  FSExecuteAnswerFail:='Ответ не выполнен. Ошибка: %s';
  ...
  FSOutMessagesStart:='Обработка исходящих сообщений ...';
  ...
  FSOutMessageSendFail:='Исходящее сообщение не отправлено. Ошибка: %s';
  ...
  FSUnlockOutMessageFail:='Разблокировка сообщения не выполнена. Ошибка: %s';
  FSCheckImeiFail:='Не верный IMEI.';
  FSCheckImsiFail:='Не верный IMSI.';
  FSChannelFormat:='%s:%s';


end;

destructor TBisSmsServer.Destroy;
begin
  FModems.Free;
  inherited Destroy;
end;

function TBisSmsServer.GetStarted: Boolean;
begin
  Result:=FModems.Connected;
end;

procedure TBisSmsServer.Init;
begin
  inherited Init;
  FModems.Init;
end;

procedure TBisSmsServer.ChangeParams(Sender: TObject);
var
  i: Integer;
                ...
                if AEnabled then begin
                  ...
                  if Trim(Port)<>'' then begin
                    ...
                    if Assigned(Modem) then begin
                      Modem.Mode:=TBisSmsServerModemMode(Table.FieldByName(SFieldMode).AsInteger);
...
{procedure TBisSmsServer.ResetMessages;
var
  P: TBisProvider;
begin
  P:=TBisProvider.Create(nil);
