unit BisTaxiDataOutMessagesImportFm;
...
type
  TBisTaxiDataOutMessagesImportForm = class(TBisExecuteForm)
  ...
  TBisTaxiDataOutMessagesImportFormIface=class(TBisExecuteFormIface)
  public
    constructor Create(AOwner: TComponent); override;
  end;

var
  BisTaxiDataOutMessagesImportForm: TBisTaxiDataOutMessagesImportForm;
...
{ TBisTaxiDataOutMessagesImportFormIface }

constructor TBisTaxiDataOutMessagesImportFormIface.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FormClass:=TBisTaxiDataOutMessagesImportForm;
end;

{ TBisTaxiDataOutMessagesImportForm }

constructor TBisTaxiDataOutMessagesImportForm.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  SizesStored:=true;
...
destructor TBisTaxiDataOutMessagesImportForm.Destroy;
begin
  inherited Destroy;
end;

procedure TBisTaxiDataOutMessagesImportForm.RecountDateBegin;
var
  OldCursor: TCursor;
  Position: Integer;
  DBegin: TDateTime;
...
procedure TBisTaxiDataOutMessagesImportForm.FormResize(Sender: TObject);
begin
...
procedure TBisTaxiDataOutMessagesImportForm.ActionLoadExecute(Sender: TObject);
begin
  Load;
end;

procedure TBisTaxiDataOutMessagesImportForm.ActionLoadUpdate(Sender: TObject);
begin
  ActionLoad.Enabled:=CanLoad;
end;

function TBisTaxiDataOutMessagesImportForm.CanLoad: Boolean;
begin
  Result:=not Working;
end;

procedure TBisTaxiDataOutMessagesImportForm.BeforeProcess;
begin
  ProgressBar.Position:=0;
  ProgressBar.Visible:=true;
...
procedure TBisTaxiDataOutMessagesImportForm.AfterProcess;
begin
...
procedure TBisTaxiDataOutMessagesImportForm.CheckBoxDateEndClick(Sender: TObject);
begin
...
procedure TBisTaxiDataOutMessagesImportForm.CheckBoxOffsetClick(Sender: TObject);
begin
...
procedure TBisTaxiDataOutMessagesImportForm.UpdateLabelCount(Count: Integer);
begin
...
procedure TBisTaxiDataOutMessagesImportForm.AlignColumnsWidth(Grid: TDBGrid);
var
  i: Integer;
  w1,w2: Integer;
  Col: TColumn;
...
procedure TBisTaxiDataOutMessagesImportForm.ButtonAccountClick(Sender: TObject);
var
  AClass: TBisDataFormIfaceClass;
  AIface: TBisDataFormIface;
  P: TBisProvider;
begin
  AClass:=TBisDataFormIfaceClass(Core.FindIfaceClass(SClassDataAccountsFormIface));
...
procedure TBisTaxiDataOutMessagesImportForm.ButtonRecountClick(Sender: TObject);
begin
  RecountDateBegin;
end;

function TBisTaxiDataOutMessagesImportForm.GetMessageCount(L: Integer): Integer;
begin
  if L<=70 then
    Result:=1
  else
...
function TBisTaxiDataOutMessagesImportForm.PrepareContact(S: String): String;
var
  L: Integer;
  F: String;
begin
...
procedure TBisTaxiDataOutMessagesImportForm.LoadFromTxtDelim(FileName: String; Delim: String);
var
  Strings: TStringList;
  Fields: TStringList;
  i,j: Integer;
...
procedure TBisTaxiDataOutMessagesImportForm.LoadFromXls(FileName: String);
var
  Excel: OleVariant;
  WorkBook: OleVariant;
  Sheet: OleVariant;
...
procedure TBisTaxiDataOutMessagesImportForm.LoadFromXml(FileName: String);
var
  Xml: TALXMLDocument;
  i,j,x: Integer;
...
procedure TBisTaxiDataOutMessagesImportForm.Load;
begin
...
function TBisTaxiDataOutMessagesImportForm.CanStart: Boolean;
begin
...
function TBisTaxiDataOutMessagesImportForm.CheckFields: Boolean;
var
...
procedure TBisTaxiDataOutMessagesImportForm.Start;
begin
...
function TBisTaxiDataOutMessagesImportForm.GetAllCount: Integer;
begin
  FDataSet.DisableControls;
  try
    Result:=0;
...
procedure TBisTaxiDataOutMessagesImportForm.DoTerminate;
begin
...
procedure TBisTaxiDataOutMessagesImportForm.SyncError;
begin
  if Trim(FError)<>'' then
    ShowError(FError);
end;

procedure TBisTaxiDataOutMessagesImportForm.SyncProgress;
begin
  ProgressBar.Position:=FPostion;
  ProgressBar.Update;
end;

procedure TBisTaxiDataOutMessagesImportForm.ThreadExecute(AThread: TBisExecuteFormThread);
