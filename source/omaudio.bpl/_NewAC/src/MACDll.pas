...
type
  c_APEDecompress_CreateW_t = function(
  pFilename: PWideChar;
  var pErrorCode: Integer
...
var
  ...
  c_APEDecompress_CreateW : c_APEDecompress_CreateW_t;
...
constructor TAPEDecompress.Create(Filename: WideString);
begin
  FFilename := Filename;
  FHandle := c_APEDecompress_CreateW(PWideChar(FFilename), ErrorCode);
  if ErrorCode<>0 then
    raise EAPEException.Create(ErrorCode);
end;
...
initialization
  ...
  begin
    ...
    c_APEDecompress_CreateW := GetProcAddress(Libhandle, '_c_APEDecompress_CreateW@8');
