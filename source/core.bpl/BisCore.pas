...
begin
  ...
  FSStartModuleInfo:='Модуль=%s Версия=%s (%s) Размер=%d Хеш=%s';
  FSStartConfigFileName:='Конфигурационный файл=%s';
  FSStartLoggerFileName:='Файл логирования=%s';
  ...
  FSApplicationExit:='Выход из приложения по причине: %s';

  FSApplicationID:='Идентификатор приложения';
  FSApplicationName:='Имя приложения';
  FSApplicationTitle:='Заголовок приложения';
  FSApplicationVersion:='Версия приложения';

  FSCmdParamConfigDesc:='подключение файла инициализации [/config test.ini]';
  FSCmdParamBaseDesc:='подключение файла параметров [/base test.bis]';
  FSCmdParamLogDesc:='подключение файла журнала [/log test.log]';
  FSCmdParamHelpDesc:='помощь по командной строке';
  FSCmdParamInstallDesc:='установка службы [/install /service name="Name"]';
  FSCmdParamUninstallDesc:='удаление службы [/uninstall /service name="Name"]';
  FSCmdParamServiceDesc:='запуск службы';
  ...
  FSCmdParamSetDesc:='настройка параметров [/set param_values.txt или /set param=value или /set param=value.txt]';
  FSCmdParamNoLoginDesc:='без соединения';
