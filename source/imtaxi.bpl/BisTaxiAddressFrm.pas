...
begin
  ...
  try
    ...
    if CZ.GetDataSet(PZ) and CC.GetDataSet(PC) and CC2.GetDataSet(PC2) then begin
      ComboBoxZone.Items.BeginUpdate;
      try
        if PZ.Active and not PZ.Empty and PC.Active and PC2.Active then begin
          ...
          PZ.First;
          while not PZ.Eof do begin
            ObjZ:=TBisTaxiZone.Create;
            ObjZ.Id:=PZ.FieldByName('ZONE_ID').Value;
            ObjZ.Name:=PZ.FieldByName('NAME').AsString;
            ObjZ.CostIn:=PZ.FieldByName('COST_IN').Value;
            ObjZ.CostOut:=PZ.FieldByName('COST_OUT').Value;
            ComboBoxZone.Items.AddObject(ObjZ.Name,ObjZ);

            PC.Filtered:=false;
            PC.Filter:=Format('ZONE_ID=%s',[QuotedStr(VarToStrDef(ObjZ.Id,''))]);
            PC.Filtered:=true;
            PC.First;
            while not PC.Eof do begin
              ObjZ.Compositions.Add(PC.FieldByName('STREET_ID').Value,
                                   TBisTaxiTypeComposition(PC.FieldByName('TYPE_COMPOSITION').AsInteger),
                                   PC.FieldByName('HOUSES').AsString,
                                   PC.FieldByName('EXCEPTIONS').AsString);
              PC.Next;
            ...
            PC2.Filtered:=false;
            PC2.Filter:=Format('ZONE_FROM_ID=%s',[QuotedStr(VarToStrDef(ObjZ.Id,''))]);
            PC2.Filtered:=true;
            PC2.First;
            while not PC2.Eof do begin
              ObjZ.Costs.Add(PC2.FieldByName('ZONE_TO_ID').Value,
                            PC2.FieldByName('DISTANCE').Value,
                            PC2.FieldByName('PERIOD').Value,
                            PC2.FieldByName('COST').Value);
              PC2.Next;
