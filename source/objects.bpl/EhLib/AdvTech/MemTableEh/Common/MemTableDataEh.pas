...
{                     EhLib v4.0                        }
{                                                       }
{          TMemTableDataEh component (Build 4.0.20)     }
{                                                       }
...
{$ENDIF}
  ...
  TStringDataFieldTypesEh = (fdtStringEh, fdtFixedCharEh, fdtWideStringEh, fdtGuidEh);
...
//  TRecordsViewEh = class(TPersistent)
  ...
  { TODO -oKostik : А вот этот список надо отсортировать }
...
begin
  ...
  begin
    SetLength(Result, 1);
    Result[0] := FieldIndex(FieldNames);
  end;
end;
...
begin
  ...
  begin
    ...
    try
      DataStruct.GetFieldList(Fields, FieldNames);
      for I := 0 to Fields.Count - 1 do
        Value[TMTDataFieldEh(Fields[I]).Index, dvvValueEh] := VarValue[I];
    finally
      Fields.Free;
    end;
  end else
    Data[DataStruct.FieldIndex(FieldNames)] := VarValue;
end;
...
begin
  Notify(Rec[Index], Index, rlnRecDeletingEh);
  DelRec := Delete(Index);
...
{function BinSearch(SortedList: TObjectList; Value: Integer): Integer;
var
...
function TRecordsViewEh.SearchNewPos(SortedList: TObjectList; MemRec: TMemoryRecordEh): Integer;
var
  CurIndex, AMin, AMax : Integer;
begin
...
var
  ...
  begin
    if NewIndex>=FFilteredRecsList.Count then
      FFilteredRecsList.Add(MemRec)
    else
    begin
      FFilteredRecsList.Insert(NewIndex, MemRec);
    end;
  ...
  case Action of
    rlnRecAddedEh:
      begin
        ...
        begin
          NewIndex := SearchNewPos(FFilteredRecsList, MemRec);
          AddToFilterList;
        end;
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Notify(MemRec, NewIndex, rlnRecDeletedEh);
          end;
...
begin
  ...
  begin
    ...
    begin
      Index := Integer(FFilteredRecsList[i]);
      if Index = OldIndex then
      begin
        FFilteredRecsList[i] := TObject(NewIndex);
        iFrom := i;
      end else if (Index >= NewIndex) and (Index < OldIndex) then
      begin
        FFilteredRecsList[i] := TObject(Integer(FFilteredRecsList[i]) + 1);
  ...
  begin
    ...
    begin
      ...
      begin
        FFilteredRecsList[i] := TObject(NewIndex);
        iFrom := i;
      end else if (Index > OldIndex) and (Index <= NewIndex) then
        FFilteredRecsList[i] := TObject(Integer(FFilteredRecsList[i]) - 1)
      else if (Index > NewIndex) and (iTo = -1) then
        iTo := i-1;
    end;
    if iTo = -1 then
      iTo := Count-1;
...
begin
  //{ TODO : Возможно ли сделать за один проход? }
