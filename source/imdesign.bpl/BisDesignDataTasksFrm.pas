...
//      Task.OnEnd:=TaskEnd;
      Task.Reset;
      Task.Start(true);
      if Provider.Locate('TASK_ID',Task.ID,[loCaseInsensitive]) then begin
        Provider.Edit;
        Provider.FieldByName('DATE_EXECUTE').Value:=Task.DateExecute;
        Provider.FieldByName('RESULT_STRING').Value:=Task.ResultString;
        Provider.Post;
      end;
    finally
      if Assigned(B) and Provider.BookmarkValid(B) then
