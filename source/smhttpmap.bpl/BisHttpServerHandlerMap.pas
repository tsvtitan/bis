...
begin
  if FWebModule is TBisHttpServerHandlerMapWebModule then begin
    TBisHttpServerHandlerMapWebModule(FWebModule).Servers.CopyFrom(FServers);
    TBisHttpServerHandlerMapWebModule(FWebModule).ServersTimeout:=FServersTimeout;
  end;
...
begin
  ...
  if Assigned(Source) and (Source is TBisHttpServerHandlerMap) then begin
    FServers.CopyFrom(TBisHttpServerHandlerMap(Source).FServers);
    FServersTimeout:=TBisHttpServerHandlerMap(Source).FServersTimeout;
  end;
end;
