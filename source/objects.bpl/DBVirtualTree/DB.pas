...
{ TNamedItem }
  ...
  protected
    ...
    procedure SetDisplayName(const Value: WideString); overload; virtual;
  published
    property Name: WideString read FName write SetDisplayName;
  end;
...
{ TDefCollection }
  ...
  public
    ...
    function IndexOf(const AName: WideString): Integer;
...
{ TFieldDef }
  ...
  public
    constructor Create(Owner: TFieldDefs; const Name: WideString;
...
{ TFlatList }
  ...
  TFlatList = class(TWideStringList)
  private
    FDataSet: TDataSet;
    FLocked: Boolean;
    FUpdated: Boolean;
  protected
    ...
    function FindItem(const Name: WideString; MustExist: Boolean): TObject;
...
{ TFields }
  ...
  public
    ...
    procedure CheckFieldName(const FieldName: WideString);
...
{$ENDIF}
    ...
    FKeyFields: WideString;
    FLookupKeyFields: WideString;
    FLookupResultField: WideString;
    ...
    function GetDisplayLabel: WideString;
    ...
    procedure SetDisplayLabel(Value: WideString);
...
{$ENDIF}
    ...
    procedure SetLookupKeyFields(const Value: WideString);
    procedure SetLookupResultField(const Value: WideString);
    procedure SetKeyFields(const Value: WideString);
  ...
  protected
    ...
    procedure GetWideText(var Text: WideString; DisplayText: Boolean); virtual;
    ...
    procedure SetWideText(const Value: WideString); virtual;
...
{$ENDIF}
  ...
  published
    ...
    property LookupKeyFields: WideString read FLookupKeyFields write SetLookupKeyFields;
    property LookupResultField: WideString read FLookupResultField write SetLookupResultField;
    property KeyFields: WideString read FKeyFields write SetKeyFields;
...
{ TDataSet }
  ...
  protected
    ...
    function PSGetCommandText: string; virtual; deprecated;
    function PSGetCommandTextW: WideString; virtual;
    function PSGetKeyFields: string; virtual; deprecated;
    function PSGetKeyFieldsW: WideString; virtual;
    function PSGetQuoteChar: string; virtual; deprecated;
    function PSGetQuoteCharW: WideString; virtual;
    function PSGetTableName: string; virtual; deprecated;
    function PSGetTableNameW: WideString; virtual;
    procedure PSSetCommandText(const CommandText: string); overload; virtual; deprecated;
    procedure PSSetCommandText(const CommandText: WideString); overload; virtual;
    { IProviderSupport }
  ...
  public
    ...
    procedure GetFieldNames(List: TStrings); overload; virtual; deprecated;
...
const
  ...
  FieldTypeVarMap: array[TFieldType] of Word = (
    ...
    varOleStr, varOleStr, varEmpty, varString);
...
{$IFDEF MSWINDOWS}
uses DBConsts, RTLConsts, ComObj, ActiveX, WideStrUtils;
{$ENDIF}
{$IFDEF LINUX}
...
procedure DatabaseErrorFmt(const Message: WideString; const Args: array of const;
  Component: TComponent = nil);
begin
  DatabaseError(WideFormat(Message, Args), Component);
end;
...
begin
  case VarType of
    varSmallint, varShortInt, varByte: Result := ftSmallInt;
    varWord, varInteger: Result := ftInteger;
    varCurrency: Result := ftBCD;
    varLongWord, varSingle, varDouble: Result := ftFloat;
...
procedure TNamedItem.SetDisplayName(const Value: WideString);
begin
  ...
  FName := Value;
  inherited SetDisplayName( AnsiString(Value) );
end;
...
function TDefCollection.IndexOf(const AName: WideString): Integer;
begin
  for Result := 0 to Count - 1 do
    if WideCompareText(TNamedItem(Items[Result]).Name, AName) = 0 then Exit;
  Result := -1;
end;

function TDefCollection.Find(const AName: WideString): TNamedItem;
var
  I: Integer;
begin
  I := IndexOf(AName);
...
constructor TFieldDef.Create(Owner: TFieldDefs; const Name: WideString;
...
function TFlatList.FindItem(const Name: WideString; MustExist: Boolean): TObject;
var
  I: Integer;
begin
...
function TFieldDefList.Find(const Name: WideString): TFieldDef;
begin
  Result := TFieldDef(FindItem(Name, False));
end;

function TFieldDefList.FieldByName(const Name: WideString): TFieldDef;
begin
...
function TFieldList.Find(const Name: WideString): TField;
begin
...
procedure TFields.CheckFieldName(const FieldName: WideString);
begin
...
procedure TFields.CheckFieldNames(const FieldNames: WideString);
var
  Pos: Integer;
begin
  Pos := 1;
...
function TField.GetDisplayLabel: WideString;
begin
  Result := GetDisplayName;
end;

function TField.GetDisplayName: WideString;
begin
...
function TField.GetFullName: WideString;
begin
...
procedure TField.GetWideText(var Text: WideString; DisplayText: Boolean);
var
  AText : AnsiString;
begin
...
procedure TField.SetAsWideString(const Value: WideString);
begin
...
procedure TField.SetDisplayLabel(Value: WideString);
begin
...
procedure TField.SetFieldName(const Value: WideString);
begin
  CheckInactive;
...
procedure TField.SetLookupKeyFields(const Value: WideString);
begin
  CheckInactive;
  FLookupKeyFields := Value;
end;

procedure TField.SetLookupResultField(const Value: WideString);
begin
  CheckInactive;
  FLookupResultField := Value;
end;

procedure TField.SetKeyFields(const Value: WideString);
begin
  CheckInactive;
  FKeyFields := Value;
end;
...
procedure TField.SetWideText(const Value: WideString);
begin
  SetText(Value);
end;
...
function TWideStringField.GetValue(var Value: WideString): Boolean;
var
  Temp: array of WideChar;
  pBuff: PWideChar;
  Buffer: array[0..dsMaxStringSize div 2] of WideChar;
begin
  if DataSize > dsMaxStringSize then
  begin
    SetLength(Temp, (DataSize div 2)+1);
    pBuff := pWideChar(Temp);
    Result := GetData(pBuff, False);
    if Result then
      Value := pBuff;
  end else
  begin
...
begin
  if DataSize > dsMaxStringSize then
  begin
    SetLength(Temp, wstrlen(PWideChar(Value))+1);
    pBuff := PWideChar(Temp);
    WStrLCopy(pBuff, PWideChar(Value), wstrlen(PWideChar(Value)));
    SetData(pBuff, False);
  end else
  begin
    WStrLCopy(Buffer, PWideChar(Value), Size);
...
procedure TBlobField.SetAsWideString(const Value: WideString);
begin
  with DataSet.CreateBlobStream(Self, bmWrite) do
    try
      WriteBuffer(Pointer(Value)^, Length(Value) * 2);
    finally
      Free;
    end;
end;
...
begin
  // January 2007
  // Previously this method raised an error, but since TBlobField implements
  // SetAsString there is no reason this should be disallowed.
  // This method override has been left in to avoid changing the interface.
  inherited;
end;
...
procedure TMemoField.SetAsWideString(const Value: WideString);
begin
  SetAsString(Value);
end;
...
begin
  ...
  if VarIsNull(V) then
    Exit;
  if FResultType in [ftFloat, ftCurrency, ftBCD, ftFMTBCD] then
  begin
    if DisplayText then
...
function TIndexDef.GetFields: WideString;
begin
  if ixExpression in Options then Result := '' else Result := FFieldExpression;
end;

procedure TIndexDef.SetFields(const Value: WideString);
begin
...
function TParams.FindParam(const Value: WideString): TParam;
var
  I: Integer;
begin
  for I := 0 to Count - 1 do
  begin
    ...
    if WideCompareText(Result.Name, Value) = 0 then Exit;
  end;
  Result := nil;
end;
...
procedure TParams.GetParamList(List: TList; const ParamNames: WideString);
var
  Pos: Integer;
begin
  Pos := 1;
...
begin
  ...
  case DataType of
    ...
    ftWideString, ftWideMemo:
      Result := (Length(VarToWideStr(FData)) + 1) * 2;                        
...
begin
  case DataType of
    ...
    ftWideString, ftWideMemo:
      WStrMove(Buffer, PWideChar(GetAsWideString), Length(GetAsWideString) + 1);
...
procedure TParam.SetAsWideString(const Value: WideString);
begin
...
var
  ...
  var
    Pos, j: Integer;
    KeyFields, FieldName: WideString;
    PS2 : IProviderSupport2;
  begin
    if Supports(self, IProviderSupport2, PS2) then
      KeyFields := PS2.PSGetKeyFields
    else
    ...
    begin
      ...
      for j := 0 to FieldCount - 1 do
        if WideCompareText(FieldName, Fields[j].FieldName) = 0 then
        begin
...
begin
  ...
  else if Field.DataSize > dsMaxStringSize then
  begin
    SetLength(Temp,Field.DataSize);
    pBuff := PChar(Temp);
    if Buffer <> nil then
      DataConvert(Field, Buffer, pBuff, True);
    SetFieldData(Field, pBuff);
  end else
  begin
    if Buffer <> nil then
...
function TDataSet.FieldByName(const FieldName: WideString): TField;
begin
  Result := FindField(FieldName);
...
function TDataSet.FindField(const FieldName: WideString): TField;
begin
  ...
  begin
    ...
    try
      ...
      if DataSize < Field.Size then
        FillChar((PChar(Buffer)+DataSize)^, Field.Size - DataSize, 0);
    finally
      VarArrayUnlock(Data);
    end;
  end;
...
procedure TDataSet.GetFieldList(List: TList; const FieldNames: WideString);
var
  Pos: Integer;
  Field: TField;
begin
...
function TDataSet.PSExecuteStatement(const ASQL: WideString; AParams: TParams;
  ResultSet: Pointer = nil): Integer;
begin
  Result := PSExecuteStatement(AnsiString(ASQL), AParams, ResultSet);
end;
