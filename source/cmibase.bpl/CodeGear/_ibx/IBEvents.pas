...
uses
  ...
  Libc,
  {$ENDIF}
  DB, IBHeader, IBExternals, IB, IBDatabase, IBIntf;
...
var
  i: Integer;
begin
  FGDSLibrary.isc_event_counts(StatusVector, EventBufferLen, EventBuffer, ResultBuffer);
...
begin
  Parent.Database.Call(FGDSLibrary.isc_cancel_events(StatusVector, @Parent.Database.Handle,
     @EventID), True);
  FGDSLibrary.isc_free(EventBuffer);
  EventBuffer := nil;
  FGDSLibrary.isc_free(ResultBuffer);
  ResultBuffer := nil;
end;
...
begin
  ...
  for i := 1 to 15 do
    EBPArray[i] := EBP(i);
  EventBufferLen := FGDSLibrary.isc_event_block(@EventBuffer,
...
begin
  try
    Parent.Database.Call(FGDSLibrary.isc_que_events(StatusVector, @Parent.Database.Handle,
