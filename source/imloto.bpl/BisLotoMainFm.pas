...
type
  TBisLotoMainForm = class(TBisMdiMainForm)
  ...
  TBisLotoMainFormIface=class(TBisMDIMainFormIface)
  private
...
var
  BisLotoMainForm: TBisLotoMainForm;
...
uses BisIfaceModules, BisConsts, BisUtils, BisFilterGroups, BisFieldNames, BisOrders,
     BisLotoConsts, BisDialogs, BisPicture;

{ TBisLotoMainFormIface }

constructor TBisLotoMainFormIface.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FormClass:=TBisLotoMainForm;
  FAfterLoginEvent:=Core.AfterLoginEvents.Add(AfterLogin);
  FBeforeLogoutEvent:=Core.BeforeLogoutEvents.Add(BeforeLogout);

end;

destructor TBisLotoMainFormIface.Destroy;
begin
...
procedure TBisLotoMainFormIface.AfterLogin(Sender: TObject);
begin
end;

procedure TBisLotoMainFormIface.BeforeLogout(Sender: TObject);
begin
end;

{ TBisTaxiMainForm }

constructor TBisLotoMainForm.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  AutoMenu:=true;
  SizesStored:=true;
...
destructor TBisLotoMainForm.Destroy;
begin
  inherited Destroy;
end;

procedure TBisLotoMainForm.RefreshMenus;
var
...
procedure TBisLotoMainForm.SetLogoPosition(Visible: Boolean);
begin
  PanelLogo.Anchors:=[];
...
procedure TBisLotoMainForm.ActionExportExecute(Sender: TObject);
begin
  if Assigned(ExportIface) then ExportIface.Show;
end;

procedure TBisLotoMainForm.ActionExportUpdate(Sender: TObject);
begin
  ActionExport.Enabled:=Assigned(ExportIface) and ExportIface.CanShow;
end;

procedure TBisLotoMainForm.ActionImportExecute(Sender: TObject);
begin
  if Assigned(ImportIface) then ImportIface.Show;
end;

procedure TBisLotoMainForm.ActionImportUpdate(Sender: TObject);
begin
  ActionImport.Enabled:=Assigned(ImportIface) and ImportIface.CanShow;
end;

procedure TBisLotoMainForm.ActionManageExecute(Sender: TObject);
begin
  if Assigned(ManageIface) then ManageIface.Show;
end;

procedure TBisLotoMainForm.ActionManageUpdate(Sender: TObject);
begin
  ActionManage.Enabled:=Assigned(ManageIface) and ManageIface.CanShow;
end;

procedure TBisLotoMainForm.ActionTiragesExecute(Sender: TObject);
begin
  if Assigned(TiragesIface) then TiragesIface.Show;
end;

procedure TBisLotoMainForm.ActionTiragesUpdate(Sender: TObject);
begin
  ActionTirages.Enabled:=Assigned(TiragesIface) and TiragesIface.CanShow;
end;

procedure TBisLotoMainForm.ActionLotteryExecute(Sender: TObject);
begin
  if Assigned(LotteryIface) then LotteryIface.Show;
end;

procedure TBisLotoMainForm.ActionLotteryUpdate(Sender: TObject);
begin
