...
var
  LStrm: TIdStringStream;
  LStream : TIdStreamVCL;
begin
  LStrm := TIdStringStream.Create('');    {do not localize}
  if SendCmd('HELP ' + ACommand, [211, 214, 500]) <> 500 then       {do not localize}
  begin
    LStream := TIdStreamVCL.Create(LStrm);
    try
...
begin
  SendCmd('ACCT ' + AInfo, [202, 230, 500]);   {do not localize}
end;
  ...
  fpcmNone:
    begin
      if SendCmd('USER ' + FUserName, [230, 232, 331]) = 331 then begin {do not localize}
        SendCmd('PASS ' +GetLoginPassword, 230);  {do not localize}
      end;
    end;//fpcmNone
  fpcmUserSite:
    begin
    ...
    begin
      if SendCmd(Format('USER %s@%s@%s', [FUserName, ProxySettings.UserName, FtpHost]), [230, 232, 331])=331 then begin    {do not localize}
        if Length(ProxySettings.Password)>0 then begin
          SendCmd('PASS ' + GetLoginPassword + '@' + ProxySettings.Password, 230); {do not localize}
        end
...
var LSH : TIdIOHandlerSocket;
begin
  inherited;
  if (AValue=nil) or (AValue is TIdIOHandlerSocket) then begin

    if AValue is TIdIOHandlerSocket then
    begin
      LSH := TIdIOHandlerSocket(AValue);
      // UseExtensionDataPort must be true for IPv6 connections.
      // PORT and PASV can not communicate IPv6 Addresses
      if LSH.IPVersion = Id_IPv6 then begin
        FUseExtensionDataPort := True;
      end;
    end;
  end else begin
    raise EIdFTPWrongIOHandler.Create(RSFTPIOHandlerWrong); // RS + EXCEPTION {do not localize}
  end;
end;
...
begin
  if (AValue = False)
   and (IPVersion = Id_IPv6) then begin
...
begin
  if (AValue=False) and (FTryNATFastTrack) then
  begin
...
begin
  if (FCapabilities.IndexOf('EPSV')>-1) then  {do not localize}
  begin
...
var  LDest: TIdStringStream;
  LStream : TIdStreamVCL;
begin
  LDest := TIdStringStream.Create('');
  LStream := TIdStreamVCL.Create(LDest);
  try
    InternalGet(trim('MLSD ' + ADirectory), LStream);  {do not localize}
  finally
    FreeAndNil(LStream);
  end;
  FreeAndNil(FDirectoryListing);
...
begin
  if Assigned(FOnCustomFTPProxy) then
  begin
...
begin
  if ATargetUsesPasv then
  begin
    if AToSite.UsingNATFastTrack then
    begin
  ...
  begin
    if AFromSite.UsingNATFastTrack then
    begin
      raise EIdFTPSToSNATFastTrack.Create(RSFTPNoSToSWithNATFastTrack);
    end;
  end;
  if AFromSite.IPVersion <> AToSite.IPVersion then
  begin
    raise EIdFTPStoSIPProtoMustBeSame.Create(RSFTPSToSProtosMustBeSame);
  end;
  if AFromSite.CurrentTransferMode <> AToSite.CurrentTransferMode then
  begin
  ...
  if AFromSite.FUsingSFTP <> AToSite.FUsingSFTP then
  begin
    raise EIdFTPSToSNoDataProtection.Create(RSFTPSToSNoDataProtection);
  end;
  if Result then
  begin
    if AFromSite.IsExtSupported('SSCN') or AToSite.IsExtSupported('SSCN') then {do not localize}
    begin
    end
    else
    begin
      ...
      begin
        if ATargetUsesPasv then
        begin
          if not AToSite.IsExtSupported('CPSV') then {do not localize}
          begin
        ...
        begin
          if not AFromSite.IsExtSupported('CPSV') then {do not localize}
          begin
            raise EIdFTPSToSNATFastTrack.Create(RSFTPSToSSSCNNotSupported);
          end;
        end;
  ...
  begin
{
  ...
  ===
    ...
    begin
      ...
      SendCmd('MDTM '+FTPLocalDateTimeToMLS(ALocalTime)+ ' '+AFileName,[253]); {do not localize}
    end
    else
    begin
      //use old method  Serv-U
      //BPFTP Server
      if (IndexOfFeatLine('MDTM YYYYMMDDHHMMSS[+-TZ] filename')>0) or {do not localize}
       IsServerMDTZAndListTForm then
      begin
  ...
  begin
{
  ...
  ===
    ...
    begin
      //we use the new method
      SendCmd('MDTM '+FTPGMTDateTimeToMLS(AGMTTime)+ ' '+AFileName,[253]); {do not localize}
    end
    else
    begin
...
begin
  Result := (Copy(FGreeting.Text[0],1,7) = 'Serv-U ') or         {do not localize}
            (Copy(FGreeting.Text[0],1,13) = 'BPFTP Server ') or  {do not localize}
            (Copy(FGreeting.Text[0],1,16) = 'TitanFTP server ') or {do not localize}
...
begin
  ...
  begin
    if (AComponent = FCompressor ) then
    begin
      FCompressor := nil;
      if Connected then
      begin
...
begin
  ...
  if FDataPortProtection <> AValue  then
  begin
...
begin
  if Assigned(OnRetrievedDir) then
  begin
    OnRetrievedDir(Self);
  end;
end;
...
begin
  if Assigned(FOnDirParseEnd) then
  begin
    FOnDirParseEnd(Self);
  end;
end;
