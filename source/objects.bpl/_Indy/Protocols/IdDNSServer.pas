...
begin
  ...
  //if not (RR.RecType = qtSOA) then begin
  if (not (RR.RecType = qtSOA)) and (Self.Handed_DomainList.IndexOf(UpperCase(RR.Name)) = -1) then begin
  ...
  if not IsSearchCache then begin
    ...
    if TargetNode <> nil then begin //Assemble the Answer.
      ...
      if RRIndex = -1 then begin
         ...
         RRIndex := TargetNode.RRs.ItemNames.IndexOf(AnsiLowercase(QName));
      ...
      if Length(Answer) > 0 then begin
        InternalSearch(Header, '*.'+QName, QType, Answer, IfMAinQuestion,
    ...
    {if Length(Answer) = 0 then begin
          ...
          if RRIndex <> -1 then begin
            ...
            if (((TM1.Date = TM2.Date) and (TM1.Time > TM2.Time)) or
              ...
              case QType of
                TypeCode_Error:
                begin
                  AppendBytes(Answer, ToBytes('Error')); {do not localize}
                end;
                TypeCode_A:
                begin
                  if TargetNode.RRs.Items[RRIndex] is TIdRR_A then begin
      ...
      if MoreAddrSearch.Count > 0 then begin
         for Count := 0 to MoreAddrSearch.Count -1 do begin
                ...
                if Self.Handed_DomainList.Count > 0 then begin
                   repeat
                      if (AnsiPos(AnsiLowercase(Self.Handed_DomainList.Strings[Server_Index]),
                              AnsiLowercase(MoreAddrSearch.Strings[Count])) > 0) then begin
                          IsMyDomains := True;
                      end else begin
                          IsMyDomains := False;
                      end;
  ...
  if Self.FDataSize >= 12 then begin
     ...
     try
        if DNSHeader_Processing.ParseQuery(ExternalQuery) <> 0 then begin
           ...
           AppendBytes(FinalResult, ToBytes(Temp)); // Do not localize;
        end else begin
            if DNSHeader_Processing.QDCount > 0 then begin
               QPos := 13;
               QLength := Length(ExternalQuery);
               if (QLength > 12) then begin
                  ...
                  QType := TwoByteToWord(ExternalQuery[QPos], ExternalQuery[QPos + 1]);
                  Inc(QPos, 2);
                  QClass := TwoByteToWord(ExternalQuery[QPos], ExternalQuery[QPos + 1]);
                  ...
                  AppendBytes(FinalResult, ToBytes(Temp));
               end;
            end;
        end;
      finally
...
begin
   ...
   AppendBytes(Result, Copy(EQuery, 13, Length(EQuery) -12));
       ...
       Class_CHAOS : begin
           if TempDomain = 'version.bind.' then begin {do not localize}
              if Self.FServer.offerDNSVersion then begin
                 lAnswer := DomainNameToDNSStr('version.bind.'); {do not localize}

                 AppendBytes(lAnswer, ToBytes(SmallInt((TypeCode_TXT))));
                 AppendBytes(lAnswer, ToBytes(SmallInt(Class_CHAOS)));
                 AppendBytes(lAnswer, ToBytes(('86400'))); {do not localize}
                 AppendBytes(lAnswer, ToBytes((Length(NormalStrToDNSStr(Self.FServer.DNSVersion)))));
                 AppendBytes(lAnswer, NormalStrToDNSStr(Self.FServer.DNSVersion));
