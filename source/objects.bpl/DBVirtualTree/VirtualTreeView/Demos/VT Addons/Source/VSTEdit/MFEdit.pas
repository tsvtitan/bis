...
type
  ...
  public
    ...
    procedure Clear; {$ifdef COMPILER_6_UP} override; {$endif COMPILER_6_UP}
    procedure RequiredColorize;
...
procedure TMFCustomNumEdit.SpeedButtonMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
    APoint,
    Pos    : Tpoint;
begin
...
procedure TMFCustomNumEdit.SpeedButtonMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
    if Button <> mbLeft then
        Exit;
    Form.Visible            := False;
...
procedure TMFNumEditGauge.WMLBtnDown(var Message: TMessage);
begin
    MouseCapture := True;
    WMMouseMove( Message );
    FEdit.SetFocus;
...
procedure TMFNumEditGauge.WMMouseMove(var Message: TMessage);
var
    Pos           : SmallInt;
    Diff,
    V             : Double;
...
begin
    if ( not MouseCapture ) then
        Exit;
    Pos := SmallInt(Message.LParamLo) + 1;
    LocalMinValue := FEdit.GaugeMinValue;
    LocalMaxValue := FEdit.GaugeMaxValue;
    if ( LocalMinValue = 0 ) and ( LocalMaxValue = 0 ) then
    begin
...
procedure Register;
begin
    RegisterComponents('Virtual Controls', [TMFEdit,
                                            TMFButtonEdit,
                                            TMFComboEdit,
                                            TMFNumEdit,
                                            TMFDateTimeEdit]);
