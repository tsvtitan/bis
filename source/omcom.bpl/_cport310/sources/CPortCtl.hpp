...
typedef Graphics::TBitmap* TComLedGlyphs[2];
...
private:
	...
	Graphics::TBitmap* FGlyphs[2];
	Cport::TComLink* FComLink;
	Extctrls::TTimer* FRingTimer;
	Graphics::TBitmap* __fastcall GetGlyph(const int Index);
	int __fastcall GetRingDuration(void);
	...
	void __fastcall SetGlyph(const int Index, const Graphics::TBitmap* Value);
	...
	Graphics::TBitmap* __fastcall BitmapToDraw(void);
...
__published:
	...
	__property Graphics::TBitmap* GlyphOn = {read=GetGlyph, write=SetGlyph, stored=StoredGlyph, index=0};
	__property Graphics::TBitmap* GlyphOff = {read=GetGlyph, write=SetGlyph, stored=StoredGlyph, index=1};
...
#pragma option push -b-
enum TAdvanceCaret { acChar, acReturn, acLineFeed, acReverseLineFeed, acTab, acBackspace, acPage };
#pragma option pop
...
public:
	...
	void __fastcall Write(const void *Buffer, int Size);
