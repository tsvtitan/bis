...
type
  ...
  public
   ...
   // procedure LocalOrMappedAddressFor(DestinationIP: String; LocalAddress: TIdSipLocation; DefaultPort: Cardinal = 0); overload;
    // by TSV
    procedure LocalOrMappedAddressFor(DestinationIP: String; LocalAddress: TIdSipLocation;
                                      DefaultIPAddress: String=''; DefaultPort: Cardinal = 0); overload;
...
procedure TIdRoutingTable.LocalOrMappedAddressFor(DestinationIP: String; LocalAddress: TIdSipLocation;
                                                  DefaultIPAddress: String=''; DefaultPort: Cardinal = 0);
var
  LocalIP: String;
begin
  // Return not only the gateway needed for a (mapped) route, but also the port
  ...
  if Self.BestRouteIsDefaultRoute(DestinationIP, LocalIP) then begin
    ...
    if (LocalAddress.IPAddress = '') then
      // There's no mapped route, so using the OS's default route is the right
      // thing to do.
      if Trim(DefaultIPAddress)<>'' then
        LocalAddress.IPAddress := DefaultIPAddress
      else
        LocalAddress.IPAddress := LocalIP
    else
      LocalAddress.IPAddress:=DefaultIPAddress;
