...
begin
  ...
  if (s is TParamSymbol) and Assigned(TParamSymbol(s).DefaultValue) then
      Result:=Result+'='+VarToStrDef(TParamSymbol(s).DefaultValue[0],'');
...
begin
  ...
  begin
    ...
    if s.Params[i] is TVarParamSymbol then begin
      if TVarParamSymbol(s.Params[i]).IsWritable then
        Result := Result + 'var '
      else
        Result := Result + 'const ';
    end else
