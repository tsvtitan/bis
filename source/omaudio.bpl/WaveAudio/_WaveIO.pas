...
type
  ...
  private
    ...
    // by TSV
    FWaveThread: TThread;
//    procedure StopWaveThread;
    function StartWaveThread: DWORD;
    procedure WaveThreadTerminate(Sender: TObject);
...
procedure TWaveAudioIO.WaveThreadTerminate(Sender: TObject);
begin
  if Assigned(FWaveThread) then begin
    FWaveThread:=nil;
  end;
end;

function TWaveAudioIO.StartWaveThread: DWORD;
begin
  Result:=0;
 // StopWaveThread;
  if not Assigned(FWaveThread) then begin
    FWaveThread:=TWaveThread.Create(Self);
    FWaveThread.OnTerminate:=WaveThreadTerminate;
    FWaveThread.FreeOnTerminate:=true;
    FWaveThread.Resume;
    Result:=FWaveThread.ThreadID;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // by TSV
      while Opening and Assigned(FWaveThread) do
        WaitForSyncObject(ThreadEvent, INFINITE);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // by TSV
      while Closing and Assigned(FWaveThread) do
