...
type
  ...
  TBisSipPhoneLineState=(lsDestroying,lsPlaying,lsHolding,lsRinging);
  ...
  private
    ...
    FLocalPayloadLength: Integer;
    FRemotePayloadLength: Integer;
    ...
    procedure DtmfThreadEnd(Thread: TBisThread; const AException: Exception);
    procedure DtmfThreadTimeout(Thread: TBisWaitThread);

    procedure OutThreadBegin(Thread: TBisThread);
    procedure OutThreadEnd(Thread: TBisThread; const AException: Exception);
  ...
  public
    ...
    property RemotePayloadLength: Integer read FRemotePayloadLength;
    ...
    property LocalPayloadLength: Integer read FLocalPayloadLength;
  ...
  TBisSipPhoneLineDtmfEvent=procedure (Phen: TBisSipPhone; Line: TBisSipPhoneLine; const Code: Char) of object;
  ...
  private
    ...
    procedure ÑlientSessionCreate(Client: TBisSipClient; Session: TBisSipSession);
...
//    property MaxOutLines: Integer read FMaxOutLines write FMaxOutLines;
    ...
    property Channels: Word read FChannels write FChannels;
    property SamplesPerSec: LongWord read FSamplesPerSec write FSamplesPerSec;
    property BitsPerSample: Word read FBitsPerSample write FBitsPerSample;
...
const
  AllThreadOptions=[toConnect,toDisconnect,toError,toSend,toReceive,toTimeout,
                    toLineCheck,toLineConnect,toLineCreate,toLineDestroy,toLineHold,
                    toLineDisconnect,toLineInData,toLineInDataEx,toLineOutData,
                    toLinePlayBegin,toLinePlayEnd,toLineRing,toLineTimeout,toLineDtmf];
...
type
  TBisSipPhoneSyncMode=(smDoConnect,smDoDisconnect,smDoError,smDoSend,smDoReceive,smDoTimeout,
                        ...
                        smDoLinePlayBegin,smDoLinePlayEnd,smDoLineTimeout,smDoLineDtmf);
...
procedure TBisSipPhoneLineUDPServer.Lock;
begin
  FLock.Enter;
end;

procedure TBisSipPhoneLineUDPServer.UnLock;
begin
  FLock.Leave
end;
...
begin
  ...
  FDtmfThread:=TBisWaitThread.Create(FPhone.FDtmfTimeout);
  FDtmfThread.StopOnDestroy:=true;
  FDtmfThread.OnTimeout:=DtmfThreadTimeout;
  FDtmfThread.OnEnd:=DtmfThreadEnd;
  ...
  FLocalChannels:=FPhone.Channels;
  FLocalSamplesPerSec:=FPhone.SamplesPerSec;
  FLocalBitsPerSample:=FPhone.BitsPerSample;
  FLocalDriverFormat:=FPhone.FDrivers.FindFormat(FPhone.DriverName,FPhone.FormatName,
...
begin
  if FDtmfThread.TryLock then begin
    try
      if (FDtmfCode<>Code) and Assigned(FDtmf.OnCode) and
          not FDtmfThread.Exists then begin
...
procedure TBisSipPhoneLine.DtmfThreadEnd(Thread: TBisThread; const AException: Exception);
begin
  FDtmf.OnCode:=DtmfCode;
end;
...
procedure TBisSipPhoneLine.OutThreadEnd(Thread: TBisThread; const AException: Exception);
begin
  try
    ...
    if Assigned(AException) then
      TBisSipPhoneSync.DoError(FPhone,AException.Message);
  end;
end;
        ...
        if (L>0) then begin
          ...
          try
            ...
            if Flag then begin
              ...
              Flag:=FPhone.FCollectInPackets;
              if Flag then
                FInPackets.LockAdd(Packet);
                ...
                try
                  ...
                  if Converter.ConvertTo(@FDtmfFormat) then begin
                    ...
                    if ASize>0 then begin
                      ...
                      NeedSize:=Round(ASize/FRemotePacketTime*FDtmfThread.Timeout);
                      if (FDtmfStream.Size>=NeedSize) and Assigned(FDtmf.OnCode) then begin
...
begin
  if Assigned(FSession) and
     ...
     (FSession.State in [ssBreaking,ssWaiting,ssRinging,ssConfirming,
                         ssProgressing,ssProcessing]) then begin
    case FSession.State of
      ssBreaking: begin
...
begin
  ...
  FCollectInPackets:=false;
  FCollectOutPackets:=false;
  FTransportMode:=tmUDP;

  FHoldMode:=lhmEmulate;
  FRtpLocalPort:=20000;
  FDriverName:='';
  FFormatName:='CCITT A-Law';
  FChannels:=1;
  FSamplesPerSec:=8000;
  FBitsPerSample:=8;
  FConfirmCount:=3;
  ...
  FClient.OnSessionCreate:=ÑlientSessionCreate;
...
procedure TBisSipPhone.ÑlientSessionCreate(Client: TBisSipClient; Session: TBisSipSession);
var
  Line: TBisSipPhoneLine;
begin
...
begin
  Result:=false;
  Line:=FLines.LockFind(Session);
  if Assigned(Line) and not Busy then begin
    Line.Lock;
    try
