...
begin
  ...
  FSNeedControlValue:='Необходимо заполнить значание в поле [%s]';
  ...
  if not VarIsNull(FPresentationId) then begin
    ...
    try
      ...
      if P.Active and not P.Empty then begin
        ...
        Column.Title.Caption:='Приоритет';
        Column.Width:=25;

        Column:=TColumn.Create(GridPreview.Columns);
        Column.FieldName:='VIEW_NUM';
        Column.Title.Caption:='Вид';
        ...
        Column.Title.Caption:='Оформление';
        Column.ButtonStyle:=cbsEllipsis;
        Column.ReadOnly:=true;
        Column.Width:=30;
...
begin
  ...
  LabelCount.Caption:='Всего записей: '+InttoStr(ACount);
  LabelCount.Update;
end;
          ...
          try
            ...
            if AIfaceParamDepends.SelectInto(P) then begin
              ...
              if Assigned(ValueInfo) and (ValueInfo.Depends.Count>0) then
                if ShowQuestion(Format('Заменить связи для параметра %s?',[AColumn.Title.Caption]),mbNo)=mrYes then
        ...
        if Assigned(Field)then begin
          ...
          if (Column is TBisPreviewColumn) then begin
            ...
            if (S='') and AColumn.NotEmpty then begin
              if ShowErrorQuestion(Format('Поле [%s] должно содержать значение. Прервать импорт?',[AColumn.Title.Caption]))=mrYes then
                Breaked:=true;
              Result:=false;
            ...
            if (S<>'') and AColumn.Handbook and not AColumn.Values.NameExists(S) then begin
              if ShowErrorQuestion(Format('Поле [%s] содержит неверное значение. Прервать импорт?',[AColumn.Title.Caption]))=mrYes then
                Breaked:=true;
              Result:=false;
              GridPreview.SelectedIndex:=i;
              GridPreview.SetFocus;
            ...
            if (S='') and
              ...
              ShowError(Format('Поле [%s] должно содержать значение.',[Column.Title.Caption]));
              Breaked:=true;
              Result:=false;
              GridPreview.SelectedIndex:=i;
              GridPreview.SetFocus;
