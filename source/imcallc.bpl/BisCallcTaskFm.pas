...
begin
  ...
  if TaskExists and (ComboBoxResult.ItemIndex<>-1) then begin
    ...
    if Assigned(Obj) then begin
      ...
      if (Obj.ResultType=rtAction) and (Purpose=0) then begin
        ...
        try
          ...
          with P.Params do begin
            AddInvisible('ACCOUNT_ID').Value:=Core.AccountId;
            AddInvisible('TASK_ID').Value:=Frame.TaskId;
            AddInvisible('ACTION_ID').Value:=Obj.ActionId;
            AddInvisible('PURPOSE').Value:=Purpose;
            AddInvisible('DATE_CREATE').Value:=DateOf(DateTimePickerNext.Date);
            AddInvisible('COUNT',ptOutput);
          end;
          P.Execute;
          with ATaskExecuteForm do begin
            EditResult.Text:=ComboBoxResult.Items.Strings[ComboBoxResult.ItemIndex];
            EditAction.Text:=Obj.ActionName;
            EditDate.Text:=DateToStr(DateOf(DateTimePickerNext.Date));
            EditTaskCount.Text:=VarToStrDef(P.Params.ParamByName('COUNT').Value,'');
          end;
      ...
      if Flag then begin
        ...
        if Obj.ChoicePerformer then begin
          ...
          if Assigned(AClassAccounts) and IsClassParent(AClassAccounts,TBisDataFormIface) then begin
            ...
            P:=TBisProvider.Create(nil);
            try
              AIfaceAccounts.Caption:='Выбор исполнителя';
  ...
  if TaskExists then begin
    ...
    try
      ...
      if Provider.Active and not Provider.IsEmpty then begin
        ...
        GroupBoxTask.Caption:=Format(' %s от %s ',[Trim(FOldGroupBoxTaskCaption),VarToStrDef(FDateCreate,'')]);
...
begin
  ...
  if TryStrToDateTime(EditDateBegin.Text,DStart) then begin
    ...
    if FTimeDays=0 then
      ...
      EditTime.Text:=FormatEx('%d д. %s',[FTimeDays,FormatDateTime(LongTimeFormat,DEnd)]);
  end;
end;
