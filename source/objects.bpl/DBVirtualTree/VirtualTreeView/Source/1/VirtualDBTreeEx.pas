...
Type
  ...
  TBaseVirtualDBTreeExListFields=class(TObjectList)
  private
    function GetItem(Index: Integer): TField;
    function GetValue: Variant;
    function GetAsString: String;
  public
    procedure AddFields(DataSet: TDataSet; Fields: String);
    function Empty: Boolean;

    property Items[Index: Integer]: TField read GetItem;
...
procedure TBaseVirtualDBTreeExListFields.AddFields(DataSet: TDataSet; Fields: String);
var
  Pos: Integer;
  FieldName: String;
  Field: TField;
...
begin
  ...
  if Count>0 then
    if Count=1 then
      Result:=VarToStrDef(Items[0].Value,'')
    else begin
      for i:=0 to Count-1 do
        Result:=Result+'.'+VarToStrDef(Items[i].Value,'');
    end;
end;

function TBaseVirtualDBTreeExListFields.GetItem(Index: Integer): TField;
begin
...
Begin
  If FKeyFieldNames <> Value Then Begin
    ...
    Else Begin
      ...
      FKeyFields.Clear;
      If FDataLink.Active And (FKeyFieldNames <> '') Then FKeyFields.Add(FDataLink.DataSet.FieldByName(FPathFieldName));
    End;
  End;
End;
...
Begin
  If Not (csDesigning In ComponentState) Then Begin
    If Not (dbtsDataChanging In FDBStatus) And (dboTrackCursor In FDBOptions) Then Begin
      ...
      KeyID := FKeyFields.Value;
      If not VarSameValueEx(KeyID,Null) And not VarSameValueEx(KeyID,FCurID) Then SetFocusToNode(FindNode(Nil, KeyID));
      FDBStatus := FDBStatus - [dbtsDataChanging];
...
Begin
  If Not (csDesigning In ComponentState) Then Begin
    If Assigned(Field) And (dboTrackChanges In FDBOptions) Then Begin
      ...
      Else Node := FindNode(Nil, FKeyFields.Value);
...
Begin
  If Assigned(Node) Then Begin
    Data := GetNodeData(Node);
    If not VarSameValueEx(Data.ID,FCurID) Then Begin
      FCurID := Data.ID;
      If not VarSameValueEx(FCurID,Null) And Not (dbtsDataChanging In FDBStatus) And (dboTrackCursor In FDBOptions) Then Begin
        FDBStatus := FDBStatus + [dbtsDataChanging];
        FDataLink.DataSet.Locate(FKeyFieldNames, Data.ID, []);
        FDBStatus := FDBStatus - [dbtsDataChanging];
      End;
    End;
  End;
...
Begin
  ...
  If CanWriteToDataSet(DropTargetNode, 0, dbcmStructure) Then Begin
    ...
    If CanProcess Then Begin
      Focus := FocusedNode;
      BeginUpdate;
      OldDisabled:=FDataLink.DataSet.ControlsDisabled;
...
Begin
  If (dbtsDragDrop In FDBStatus) Then Begin
    ...
    If (dboParentStructure In FDBOptions) Or (dboWriteSecondary In FDBOptions) Then Begin
      FParentField.AsString := ParentID;
    End;
...
Begin
  ...
  If (Data.Status = dbnsDelete) Then Begin
    If FDataLink.DataSet.Locate(FKeyFieldNames, Data.ID, []) Then FDataLink.DataSet.Delete;
  End;
  Inherited;
End;
...
Begin
  ...
  While Assigned(Result) Do Begin
    ...
    If VarSameValueEx(Data.ID,ID) Then break;
...
Begin
  ...
  While Assigned(Result) Do Begin
    Data := GetNodeData(Result);
    If not VarSameValueEx(Data.ID,ID) Then break;
  ...
  If CanWriteToDataSet(Parent, 0, dbcmInsert) Then Begin
    ...
    If Not (dboListView In FDBOptions) Then Begin
      If (dboPathStructure In FDBOptions) Or (dboWriteSecondary In FDBOptions) Then Begin
        ...
        FPathField.AsString := Path;
      End;
      If (dboParentStructure In FDBOptions) Or (dboWriteSecondary In FDBOptions) Then FParentField.AsString := ParentID;
...
Begin
  If Not (dbtsDataChanging In FDBStatus) And CanOpenDataSet Then Begin
    ...
    if not OldDisabled then
      FDataLink.DataSet.DisableControls;
    try
      If Not FDataLink.DataSet.EOF Or Not FDataLink.DataSet.Bof Then FCurID := FKeyFields.Value;
      I := 0;
      ...
      While Assigned(Temp) Do Begin
        ...
        End Else Begin
          ...
          End;
          If (dboTrackCursor In FDBOptions) And Not Assigned(FocusedNode) And VarSameValueEx(Data.ID,FCurID) Then Begin
            Selected[Temp] := True;
            FocusedNode := Temp;
...
Begin
  ...
  Parent := Nil;
  Temp := GetFirst;
  If VarSameValueEx(ParentID,Null) Or VarSameValueEx(ParentID,ID) Then Begin
    Parent := RootNode;
    ParentID := Null;
  End;
  While Assigned(Temp) And (Not Assigned(This) Or Not Assigned(Parent)) Do Begin
    Data := GetNodeData(Temp);
    If VarSameValueEx(Data.ID,ID) Then Begin
      If (Data.Status = dbnsRefreshed) Then Begin
        Exit;
      End;
      This := Temp;
    End;
    If VarSameValueEx(Data.ID,ParentID) And not VarSameValueEx(ParentID,Null) Then Parent := Temp;
    Temp := GetNext(Temp);
  End;
...
{var
  ...
  //Field: TField;
Begin
  If (FKeyFieldNames <> '') Then FKeyFields.AddFields(FDataLink.DataSet,FKeyFieldNames);
...
Begin
  Result := Not (dboReadOnly In FDBOptions) And not FKeyFields.Empty And FDataLink.DataSet.CanModify;
  If Result Then Begin
