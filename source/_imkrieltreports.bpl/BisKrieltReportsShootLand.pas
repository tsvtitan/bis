unit BisKrieltReportsShootLand;
...
procedure ExportShootLandKrs(BisKrieltReportsForm: TBisKrieltReportsForm; DateBegin, DateEnd: TDate);
procedure ExportShootLandOthers(BisKrieltReportsForm: TBisKrieltReportsForm; DateBegin, DateEnd: TDate);
procedure ExportShootLandOutPoint(BisKrieltReportsForm: TBisKrieltReportsForm; DateBegin, DateEnd: TDate);
...
begin
  ...
  Provider.ProviderName:='_SHOOT_LAND_KRS_EXPORT';      
  ...
  XLSNew:=CopyExcelDocToNew('shoot_land_krs.xls','Сниму_землю_в_г._Красноярск');
  Excel:=OpenExcelDoc(XLSNew);
  Sheet:=Excel.ActiveSheet;

  Sheet.Range['A1']:='сниму в аренду землю';
  ...
  Repeat
    ...
    Sheet.Range['B'+IntToStr(LastRow-1)]:=Provider.FieldByName('Участок').AsString;
...
begin
  ...
  Provider:=TBisProvider.Create(nil);
  Provider.ProviderName:='_SHOOT_LAND_OTHERS_EXPORT';      
  ...
  XLSNew:=CopyExcelDocToNew('shoot_land_others.xls','Сниму_землю_в_других_населенных_пунктах');
  Excel:=OpenExcelDoc(XLSNew);
  Sheet:=Excel.ActiveSheet;

  Sheet.Range['A1']:='сниму в аренду землю';
  Sheet.Range['C1']:='в других населенных пунктах';
...
begin
  ...
  Provider:=TBisProvider.Create(nil);
  Provider.ProviderName:='_SHOOT_LAND_OUT_POINT_EXPORT';      
  ...
  XLSNew:=CopyExcelDocToNew('shoot_land_out_point.xls','Сниму_землю_за_населенными_пунктами');
  Excel:=OpenExcelDoc(XLSNew);
  Sheet:=Excel.ActiveSheet;
