...
procedure TSQLParams.AddFieldType(const Name: string; FieldType: TUIBFieldType;
  ...
  begin
    case FieldType of
      ...
      uftChar            : SetFieldType(AddField(name), 0                    , SQL_TEXT      + 1, 0);
      uftVarchar         : SetFieldType(AddField(name), 0                    , SQL_VARYING   + 1, 0);
...
{$ENDIF}
  ...
  begin
    if GetSQLType(Index)=SQL_TEXT then
      SetFieldType(Index, Length(Value), SQL_TEXT + 1, 0)
    else
      SetFieldType(Index, Length(Value), SQL_VARYING + 1, 0);
    inherited;
  end;
  ...
  begin
    if GetSQLType(Index)=SQL_TEXT then
      ...
      SetFieldType(Index, Length(Value) * 2, SQL_VARYING + 1, 0);
    inherited;
  end;
