CREATE OR ALTER PROCEDURE GET_COUNT_ORDERS_REPORT2 (DATE_BEGIN Date,
       DATE_END   Date,
       ZONE_ID    Varchar(32))
...
begin
FOR
 SELECT MIN(GCO.DATE_ACCEPT) AS DATE_ACCEPT,
        GCO.DATE_YEAR_DAY,
        GCO.DATE_WEEKDAY,
        MIN(GCO.DATE_MONTH) AS DATE_MONTH,
        MIN(GCO.DATE_YEAR) AS DATE_YEAR,          
        MIN(GCO.ZONE_ID_R) AS ZONE_ID_R,
        MIN(GCO.ZONE_NAME) AS ZONE_NAME,
        MIN(EXTRACT(WEEKDAY FROM CAST(('01.01.'||EXTRACT(YEAR FROM GCO.DATE_ACCEPT)) AS DATE)) - 1) AS FIRST_YEAR_DAY,                
       (CASE 
...
FROM GET_COUNT_ORDERS2(:DATE_BEGIN, :DATE_END, :ZONE_ID) GCO
GROUP BY GCO.ZONE_NAME, GCO.DATE_YEAR_DAY, GCO.DATE_WEEKDAY
ORDER BY GCO.ZONE_NAME, MIN(GCO.DATE_ACCEPT)
            INTO :DATE_ACCEPT, 
             :DATE_YEAR_DAY,
             :DATE_WEEKDAY,
             :DATE_MONTH,
