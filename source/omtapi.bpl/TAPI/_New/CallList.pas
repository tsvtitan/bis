...
type
  ...
  TCallList=class(TCollection)
   ...
   public
     ...
     procedure Delete(ACall: TTAPICall);
...
constructor TCallList.Create(ItemClass: TCollectionItemClass);
begin
...
procedure TTAPICallList.Delete(ACall: TTAPICall);
var i: Integer;
begin
  for i:=0 to Count-1 do
  begin
    if TCallItem(FCallList.Items[i]).Call.Handle=ACall.Handle then
      FCallList.Delete(i);
  end;
end;
...
function TTAPICallList.GetCount: Integer;
begin
...
procedure TTAPICallList.PerformMsg(Msg: TCMTAPI);
var i: Integer;
begin
  inherited;
  with Msg.TAPIRec^ do
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Item:=FCallList.Add;
        Offset:=lpCallList^.dwCallsOffset+DWord(i*4);
        ahCall:=hCall(GetDataFromTAPIStructP(lpCallList,Offset,lpCallList^.dwCallsSize)^);
...
constructor TCallItem.Create(Collection: TCollection);
begin
  inherited;
