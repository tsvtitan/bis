...
Author(s):    Stéphane Vander Clock (svanderclock@delphinaute.com)
...
type
  ...
  public
    ...
    procedure LoadFromFile(const AFileName: TALXMLString; Const LoadOnlyChildNode: Boolean=False); // [added from TXMLNode]
    procedure LoadFromStream(const Stream: TStream; Const LoadOnlyChildNode: Boolean=False); // [added from TXMLNode]
  end;
...
Var RawXmlString: TALXMLString;
    ...
    If P1 > 0 then begin
      ...
      If (aContent <> '') then begin
        ...
        For i := 0 to LstParams.Count - 1 do begin
          ...
          else LstParams.ValueFromIndex[i] := ALXMLTextElementDecode(alCopyStr(aContent,2,aContentLN-2));
        end;
  ...
  Begin
    ...
    If (PreserveWhiteSpace) or (Trim(Str1) <> '') then Begin
      If FirstTagElement then ALXmlDocError(CALXmlParseError);
      str1 := ALXMLTextElementDecode(Str1);
      if (notSaxMode) and
...
begin
  ...
  if not VarIsNull(Value) then begin
    ...
    if not assigned(aNode) then ANode := ALCreateXmlNode(
                                                         AttrName,
                                                         ntattribute,
                                                         ''
                                                        );
...
Var NodeStack: Tstack;
  ...
  Begin
    with aAttributeNode do
      WriteStr2Stream(' ' + NodeName + '="' + ALXMLTextElementEncode(text) + '"');
  end;
  ...
  Begin
    with aTextNode do
      WriteStr2Stream(ALXMLTextElementEncode(text));
  end;
...
{      CurrentParentNode := CurrentNode.ParentNode;
      If Assigned(CurrentParentNode) and
        (((CurrentParentNode.nodeType = ntDocument) and (CurrentNode.nodeType <> ntelement)) or
          ((CurrentNode.nodeType = ntelement) and not CurrentNode.IsTextElement))
        then WriteStr2Stream(#13#10);}   
...
{*******************************************************************************************************}
procedure TALXMLNode.LoadFromFile(const AFileName: TALXMLString; Const LoadOnlyChildNode: Boolean=False);
Var afileStream: TfileStream;
Begin
  aFileStream := TfileStream.Create(AFileName,fmOpenRead);
  Try
    LoadFromStream(aFileStream, LoadOnlyChildNode);
  finally
    aFileStream.Free;
  end;
end;
...
{*************************************************************************************************}
procedure TALXMLNode.LoadFromStream(const Stream: TStream; Const LoadOnlyChildNode: Boolean=False);
Begin
  If not (NodeType in [ntElement, ntDocument]) then exit;
  ChildNodes.Clear;
  AttributeNodes.Clear;
  Try
    FDocument.InternalParseXml(Stream, self, LoadOnlyChildNode);
  except
    ChildNodes.Clear;
    AttributeNodes.Clear;
    raise;
...
begin
  ...
  // Check if the name abides the rules.
  // just accept any name that at least does not contain ]]>
  If AlPos(']]>',NameOrData) > 0 then AlXmlDocError(CALXMLParameterIsIncorrect);
