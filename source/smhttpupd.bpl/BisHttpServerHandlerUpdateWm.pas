...
type
  ...
  private
    ...
    function Execute(Request: TWebRequest; Response: TWebResponse;
                     StreamFormat: TBisUpdateStreamFormat): Boolean;
  public
  ...
  if Assigned(FHandler) then begin
    ...
    try
      ...
      case StreamFormat of
        sfRaw: Flag:=ReadRawStream(RequestStream,Method);
        sfXml: Flag:=ReadXmlStream(RequestStream,Method);
      end;
      if Flag then begin
        ...
        if Assigned(Response.ContentStream) then begin
          ...
          case StreamFormat of
            sfRaw: WriteRawStream(Response.ContentStream,Success,Error,WithList);
            sfXml: WriteXmlStream(Response.ContentStream,Success,Error,WithList);
          end;
          Result:=true;
        end;
