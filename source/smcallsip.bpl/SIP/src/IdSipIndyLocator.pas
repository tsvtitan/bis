...
//  Self.Resolver.QueryRecords := [qtNAPTR];
  // by TSV
  Self.Resolver.QueryType := [qtNAPTR];
...
//  Self.Resolver.QueryRecords := [qtSRV];
  // by TSV
  Self.Resolver.QueryType := [qtService];
...
//  Naptr := Result.RecordFor(Service + '.' + SrvRecord.Domain);
  // by TSV
  Naptr := Result.RecordFor(Service + '.' + SrvRecord.Name);
...
//  Result := Self.Resolver.ReceiveTimeout;
  // by TSV
  Result := Self.Resolver.WaitingTime;
end;
...
//  Self.Resolver.ReceiveTimeout := Value;
  // by TSV
  Self.Resolver.WaitingTime:= Value;
end;
