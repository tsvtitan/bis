...
begin
  if FDecimalPlaceRound <> Value then begin
...
var
  DP: Integer;
begin
  if FDecimalPlaceRound then begin//Polaris
    DP := FDecimalPlaces;
...
// by TSV
   CheckValue(StrToFloatDef(TextToValText(EditText),0.0), True);
end;
...
// by TSV
  FValue := CheckValue(StrToFloatDef(TextToValText(EditText),0.0), False);
end;
...
// by TSV
    if Trim(AValue)<>'' then
      FTemp:=StrToFloatDef(AValue,0.0)
    else
      FTemp:=Null;
    
    FValue := CheckValue(StrToFloatDef(TextToValText(AValue),0.0), False);
    DataChanged;
    Invalidate;
  end;
end;
