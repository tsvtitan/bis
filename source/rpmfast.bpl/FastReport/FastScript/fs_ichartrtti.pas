...
begin
  ...
  begin
    ...
    // by TSV
    AddClass(TTeeFont, 'TPersistent');
    AddClass(TTeeShadow, 'TPersistent');
    AddClass(TChartSeriesList, 'TPersistent');
    ...
    with AddClass(TChartSeries, 'TComponent') do   begin
      ...
      // By TSV
      AddMethod('procedure AddXY(const AXValue: Double; const AYValue: Double; const ALabel: String)', CallMethod);
    end;
...
begin
  ...
  begin
    if MethodName = 'CLEAR' then
      ...
      TChartSeries(Instance).AddXY(Caller.Params[0], Caller.Params[1], Caller.Params[2]);
  end
end;
