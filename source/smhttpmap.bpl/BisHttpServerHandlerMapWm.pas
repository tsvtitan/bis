...
begin
  ...
  if Flag then
    ...
    if FLastResult<>'' then
      LoggerWrite(FormatEx('результат => %s',[FLastResult]),ltError);
...
function TBisMapServerThread.GetResult: String;
  ...
  begin
    ...
    if not Terminated and UrlServerExists(Url,Host,IP,Port) then begin
      ...
      LoggerWrite(FormatEx('%s:%d не найден',[Host,Port]),ltError);
  end;
...
begin
  inherited DoBegin;
  FDistance:=FParams.Add('Расстояние',FServer.DistanceIdent);
  FDuration:=FParams.Add('Время',FServer.DurationIdent);
end;
...
begin
  inherited DoBegin;
  FLatitude:=FParams.Add('Широта',FServer.LatitudeIdent);
  FLongitude:=FParams.Add('Долгота',FServer.LongitudeIdent);
end;
...
begin
  ...
  FSCoordinates:='Пробки=%d широта1=%g долгота1=%g широта2=%g долгота2=%g';
  FSDistance:='Среднее расстояние=%d время=%d';
  FSLocation:='Расположение %s, %s, %s, %s, %s';
  FSPosition:='Широта=%g долгота=%g';
