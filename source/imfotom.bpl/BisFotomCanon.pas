...
begin
  ...
  FSDisconnect:='Соединение разорвано.';
  FSDeviceNotFound:='Устройство не найдено.';
  FSResponseOk:='Операция успешно выполена.';
...
begin
  ...
  if not Connected and ApiExists and (Trim(FDeviceName)<>'') then begin
    ...
    // Если объектив выдвинут то не делать нижнюю хрень

    DoStatus('Выдвигаем объектив ...');    
...
begin
  DoStatus('Разрыв соединения ...');
  if Connected and ApiExists then begin

    CloseViewFinder;

    DoStatus('Задвигаем объектив ...');
...
var
  ...
  Context: prContext;
begin
  DoStatus('Открытие видоискателя ...');
...
begin
  Result:=false;
  DoStatus('Получение изображения ...');
...
begin
  ...
  if Connected and FViewFinderVisible then begin
    ...
    DoStatus('По умолчанию выполено.');
