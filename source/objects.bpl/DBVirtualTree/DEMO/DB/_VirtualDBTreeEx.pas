...
Type
  ...
  Private
    ...
    FNormalIndexField: TField;
    FNormalIndexFieldName: String;
    ...
    procedure SetNormalIndexFieldName(const Value: String);
  ...
  Protected
    ...
    Function FindChild(Node: PVirtualNode; ID: String): PVirtualNode;
    Function FindNode(Start: PVirtualNode; ID: String): PVirtualNode;
  ...
  Public
    ...
    Property NormalIndexField: TField Read FNormalIndexField;
    property Canvas;
  ...
  Published {Since all these properties are published for all descendants, we might as well publish them here and save whitespace}
    ...
    Property NormalIndexFieldName: String Read FNormalIndexFieldName Write SetNormalIndexFieldName;
    Property Visible;
    Property WantTabs;
  End;
...
procedure TBaseVirtualDBTreeEx.SetNormalIndexFieldName(const Value: String);
begin
  If FNormalIndexFieldName <> Value Then Begin
    FNormalIndexField := Nil;
    FNormalIndexFieldName := Value;
...
Begin
  If Not (csDesigning In ComponentState) Then Begin
    If Assigned(FKeyField) And Not (dbtsDataChanging In FDBStatus) And (dboTrackCursor In FDBOptions) Then Begin
      ...
      KeyID := FKeyField.AsString;
      If (KeyID <> '') And (KeyID <> FCurID) Then SetFocusToNode(FindNode(Nil, KeyID));
      FDBStatus := FDBStatus - [dbtsDataChanging];
...
Begin
  If Not (csDesigning In ComponentState) Then Begin
    If Assigned(Field) And (dboTrackChanges In FDBOptions) Then Begin
      ...
      Else Node := FindNode(Nil, FKeyField.AsString);
...
Begin
  If Assigned(Node) Then Begin
    ...
    If Data.ID <> FCurID Then Begin
      FCurID := Data.ID;
      If (FCurID <> '') And Not (dbtsDataChanging In FDBStatus) And (dboTrackCursor In FDBOptions) Then Begin
        FDBStatus := FDBStatus + [dbtsDataChanging];
...
Function TBaseVirtualDBTreeEx.FindChild(Node: PVirtualNode; ID: String): PVirtualNode;
Var
  Data: PDBVTData;
Begin
...
Function TBaseVirtualDBTreeEx.FindNode(Start: PVirtualNode; ID: String): PVirtualNode;
Var
  Data: PDBVTData;
Begin
...
Begin
  If Not (dbtsDataChanging In FDBStatus) And CanOpenDataSet Then Begin
    ...
    End;
    FDataLink.DataSet.DisableControls;
    If Not FDataLink.DataSet.EOF Or Not FDataLink.DataSet.Bof Then FCurID := FKeyField.AsString;
    I := 0;
...
Begin
  ID := FKeyField.AsString;
  ParentID := FParentField.AsString;
  If (ID = '') Then Begin
    Exit;
  End;
  This := Nil;
  Parent := Nil;
  Temp := GetFirst;
  If (ParentID = '') Or (ParentID = ID) Then Begin
    Parent := RootNode;
    ParentID := '';
  End;
  While Assigned(Temp) And (Not Assigned(This) Or Not Assigned(Parent)) Do Begin
    ...
    If (Data.ID = ID) Then Begin
      ...
      This := Temp;
    End;
    If (Data.ID = ParentID) And (ParentID <> '') Then Parent := Temp;
    Temp := GetNext(Temp);
  End;
...
Begin
  ...
  If FNormalIndexFieldName <> '' Then FNormalIndexField := DataSource.DataSet.FieldByName(FNormalIndexFieldName);
End;
...
Begin
  ...
  Data.NormalIndex:=-1;
  if Assigned(NormalIndexField) then
    Data.NormalIndex := NormalIndexField.AsInteger;
End;
...
Begin
  If Field = ViewField Then Begin
    ...
    UpdateNode := True;
  End
  else if (Field = NormalIndexField) then begin
    Data := PDBNodeData(GetDBNodeData(Node));
    Data.NormalIndex := Field.AsInteger;
    UpdateNode := true;
  end;
...
Begin
  If Assigned(Node) And (Node <> RootNode) Then Begin
    If (Column = Header.MainColumn) And (TextType = ttNormal) Then begin
      Text := NodeText[Node];
    end Else Inherited;
  End;
End;
