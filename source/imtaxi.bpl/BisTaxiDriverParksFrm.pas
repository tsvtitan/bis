...
procedure RefreshProvider(DataSet,Parks,DriverParks: TBisDataSet; CarFontColor,CarBrushColor: Variant);
var
  ParkId: Variant;
  ParentId: Variant;
  DriverCount: Integer;
...
begin
  ...
  try
    ...
    FDriverParks:=Provider.CollectionAfter.AddDataSet(P2);
  ...
  if Assigned(FParks) and Assigned(FDriverParks) then begin
    ...
    try
      if FParks.GetDataSet(D1) and FDriverParks.GetDataSet(D2) then  begin
        Provider.Close;
        Provider.CreateTable;
        RefreshProvider(Provider,D1,D2,GetCarFontColor,GetCarBrushColor);
      end;
    finally
      D2.Free;
      D1.Free;
...
procedure TBisTaxiDriverParksFrame.TreeGetImageIndex(Sender: TBaseVirtualTree; Node: PVirtualNode;
