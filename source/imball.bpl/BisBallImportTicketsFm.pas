...
var
  ...
  procedure AppendTicket(Num,Series: String;
                         G1Prize1,G1Prize2,G1Money1,G1Money2,G1Code: String;
  ...
  begin
    ...
    try
      ...
      with P.Params do begin
        ...
        AddInvisible('G1_PRIZE_1').Value:=iff(Trim(G1Prize1)<>'',Trim(G1Prize1),Null);
        AddInvisible('G1_PRIZE_2').Value:=iff(Trim(G1Prize2)<>'',Trim(G1Prize2),Null);
        AddInvisible('G1_MONEY_1').Value:=iff(Trim(G1Money1)<>'',Trim(G1Money1),Null);
        AddInvisible('G1_MONEY_2').Value:=iff(Trim(G1Money2)<>'',Trim(G1Money2),Null);
  ...
  var
    ...
    Row: TALXMLNode;
    Attr: TALXMLNode;
    G1Prize1, G1Prize2: String;
    G1Money1, G1Money2, G1Code: String;
    Num,Series: String;
          ...
          for j:=0 to Node.ChildNodes.Count-1 do begin
            ...
            if AnsiSameText(Row.NodeName,'row') then begin
              ...
              for x:=0 to Row.AttributeNodes.Count-1 do begin
                ...
                if AnsiSameText(Attr.NodeName,'g1_prize_1') then G1Prize1:=VarToStrDef(Attr.NodeValue,'');
                if AnsiSameText(Attr.NodeName,'g1_prize_2') then G1Prize2:=VarToStrDef(Attr.NodeValue,'');
                if AnsiSameText(Attr.NodeName,'g1_money_1') then G1Money1:=VarToStrDef(Attr.NodeValue,'');
                if AnsiSameText(Attr.NodeName,'g1_money_2') then G1Money2:=VarToStrDef(Attr.NodeValue,'');
              ...
              AppendTicket(Num,Series,G1Prize1,G1Prize2,G1Money1,G1Money2,G1Code,G2_1,G2_2,G2_3,G2_4,G2_5,G2_6);
              Inc(AllCount);
              LabelCounter.Caption:=Format('Обработано %d из %d',[AllCount,ProgressBar.Max]);
              LabelCounter.Update;
...
procedure TBisBallImportTicketsForm.SetDataBaseStatistics;
var
  P: TBisProvider;
begin
  P:=TBisProvider.Create(nil);
