...
type
  TBisUtilsTableEditFrame = class(TFrame)
    PanelTop: TPanel;
    ButtonLoad: TButton;
    ButtonSave: TButton;
...
uses BisUtils, BisDialogs, BisDBTableEditFm, BisUtilsTableEditFm;

constructor TBisUtilsTableEditFrame.Create(AOwner: TComponent);
begin
...
procedure TBisUtilsTableEditFrame.DataSetAfterInsert(DataSet: TDataSet);
begin
  RefreshCount;
end;

procedure TBisUtilsTableEditFrame.DBGridCellClick(Column: TColumn);
begin
...
procedure TBisUtilsTableEditFrame.DBGridColEnter(Sender: TObject);
begin
  if Assigned(DBGrid.SelectedField) then begin
...
procedure TBisUtilsTableEditFrame.DBGridDrawColumnCell(Sender: TObject;
...
procedure TBisUtilsTableEditFrame.ButtonCreateClick(Sender: TObject);
var
...
procedure TBisUtilsTableEditFrame.ButtonLoadClick(Sender: TObject);
begin
...
procedure TBisUtilsTableEditFrame.ButtonSaveClick(Sender: TObject);
begin
  if SaveDialog.Execute then begin
    if FDataSet.Active then begin
...
procedure TBisUtilsTableEditFrame.ButtonGenClick(Sender: TObject);
var
  S: String;
begin
  S:=GetUniqueID;
...
procedure TBisUtilsTableEditFrame.ButtonClearClick(Sender: TObject);
begin
  EditTableName.Text:='';
  FDataSet.EmptyTable;
  FDataSet.Close;
...
procedure TBisUtilsTableEditFrame.ButtonClearValueClick(Sender: TObject);
begin
  if FDataSet.Active and (Trim(DBMemoValue.DataField)<>'')then begin
    FDataSet.Edit;
...
procedure TBisUtilsTableEditFrame.ButtonLoadValueClick(Sender: TObject);
var
  Stream: TFileStream;
  AValue: String;
begin
...
procedure TBisUtilsTableEditFrame.ButtonSaveValueClick(Sender: TObject);
var
  Stream: TFileStream;
  AValue: String;
begin
...
procedure TBisUtilsTableEditFrame.btUpColumnsClick(Sender: TObject);
begin
  FDataSet.MoveData(true);
end;

procedure TBisUtilsTableEditFrame.btDownColumnsClick(Sender: TObject);
begin
  FDataSet.MoveData(false);
end;

procedure TBisUtilsTableEditFrame.ButtonApplyClick(Sender: TObject);
var
  FieldName: String;
begin
...
procedure TBisUtilsTableEditFrame.RefreshCount;
var
  Count: Integer;
begin
  Count:=0;
...
procedure TBisUtilsTableEditFrame.ButtonEditorValueClick(Sender: TObject);
var
  Pt: TPoint;
begin
...
procedure TBisUtilsTableEditFrame.MenuItemEditorTableClick(Sender: TObject);
var
  AForm: TBisUtilsTableEditForm;
  Field: TField;
...
begin
  if FDataSet.Active and (Trim(DBMemoValue.DataField)<>'') then begin
    ...
    if Field.IsBlob then begin
      AForm:=TBisUtilsTableEditForm.Create(nil);
