...
(********************************)
Tisc_get_client_version = procedure(buffer : PChar); {$IFDEF MSWINDOWS} stdcall; {$ENDIF} {$IFDEF LINUX} cdecl; {$ENDIF}
...
begin
  if GetGDSLibrary.GetIBClientVersion >= 7 then
...
begin
  ...
  if (p^.bstr_cnt >= 0) then begin
    ...
    result := Char(GetGDSLibrary.BLOB_get(p));
end;
...
begin
  ...
  if (x = Chr(Int('n') - Int('a'))) or (p^.bstr_cnt = 0) then
    result := GetGDSLibrary.BLOB_put(x, p)
  else begin
    p^.bstr_ptr^ := Char(x);
    result := UInt(x);
    Inc(p^.bstr_ptr^);
