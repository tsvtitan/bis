...
function  ALXMLTextElementEncode(Src: string): string;
function  ALXMLTextElementDecode(Src: string): string;
function  ALANSIHTMLEncode(Src: string; Const EncodeASCIIHtmlEntities: Boolean = True): string;
function  ALUTF8HTMLEncode(Src: UTF8String; Const EncodeASCIIHtmlEntities: Boolean = True): UTF8String;
  ...
  While i <= TagParams.Count - 3 do begin
    ...
    IF (P1 <= 0) and
      ...
      tagParams.Delete(i+1);
            ...
            (P1 = L1) then begin {<A HREF= "obie2.html">}
      TagParams[i] := S1 + S2;
      tagParams.Delete(i+1);
    end
    else if (L1 > 0) and
  ...
  while (CurrentSrcPos <= SrcLength) do begin
    ...
    If src[CurrentSrcPos]='&' then begin
      ...
      If (J<=SrcLength) and (j-CurrentSrcPos<=12) then Begin
        ...
        If Entity ='quot' then CopyCharToResult('"', J+1)
        else if Entity = 'apos' then CopyCharToResult('''', J+1)
        else if Entity = 'amp' then CopyCharToResult('&', J+1)
        else if Entity = 'lt' then CopyCharToResult('<', J+1)
        else if Entity = 'gt' then CopyCharToResult('>', J+1)
        ...
        Case aEntityInt of
          ...
          62: begin // gt >
            ...
            if (aEntityInt > 127) then begin
              aEntityInt := LstUnicodeEntitiesNumber.IndexOf(aEntityInt);
              If aEntityInt >= 0 Then begin
                aEntityStr := vALhtml_LstEntities[integer(LstUnicodeEntitiesNumber.Objects[aEntityInt])];
                If aEntityStr <> '' then  aEntityStr := '&' + aEntityStr + ';'
                else aEntityStr := UTF8Encode(aTmpWideString[i]);
              end
              else aEntityStr := UTF8Encode(aTmpWideString[i]);
            end
...
//  vALhtml_LstEntities.Free;
  FreeAndNil(vALhtml_LstEntities);
