...
uses
  ...
  Libc, Types,
  {$ENDIF}
  IBExternals, IB, IBHeader, IBDatabase, IBSQL, Db, IBUtils, IBBlob, IBIntf;
...
type
  ...
  TIBCustomDataSet = class(TWideDataSet)
  private
    ...
    begin
      ...
      case rdFields[j].fdDataType of
        ...
        begin
          ...
          p^.rdFields[j].fdDataLength := FGDSLibrary.isc_vax_integer(Qry.Current[i].Data.sqldata, 2);
          if RecordNumber >= 0 then
          begin
...
function TIBCustomDataSet.PSExecuteStatement(const ASQL: WideString; AParams: TParams;
...
function TIBCustomDataSet.PSGetQuoteCharW: WideString;
begin
  Result := '';
...
function TIBCustomDataSet.PSGetTableNameW: WideString;
var
  i : Integer;
begin
//  if not FInternalPrepared then
...
procedure TIBDataSet.PSSetCommandText(const CommandText: WideString);
begin
  if SelectSQL.Text <> CommandText then
    SelectSQL.Text := CommandText;
end;
...
function TIBDataSet.PSGetCommandTextW: WideString;
begin
