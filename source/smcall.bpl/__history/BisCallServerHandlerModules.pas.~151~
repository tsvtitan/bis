...
procedure TBisCallServerHandlerModules.GetHandlers(Location: TBisCallServerHandlerLocation; OperatorId: Variant; Handlers: TObjectList);
var
  i: Integer;
  Item: TBisCallServerHandlerModule;
begin
  for i:=0 to Count-1 do begin
    Item:=Items[i];
    if Item.Enabled then
      Item.Handlers.GetHandlers(Location,OperatorId,Handlers);
  end;
end;
