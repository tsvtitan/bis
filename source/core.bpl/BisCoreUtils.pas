...
procedure ObjectLoggerWrite(const ObjectName,Message: String; LogType: TBisLoggerType=ltInformation);
procedure ClassLoggerWrite(const ClassName,Message: String; LogType: TBisLoggerType=ltInformation);
...
begin
  if Assigned(Core) and Assigned(Core.Logger) then
    Core.Logger.Write(Message,LogType,ObjectName);
end;

procedure ClassLoggerWrite(const ClassName,Message: String; LogType: TBisLoggerType=ltInformation);
begin
  ObjectLoggerWrite(GetNameByClass(ClassName),Message,LogType);
end;
