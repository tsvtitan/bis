...
//    procedure ClientConnected(Thread: TIdPeerThread);
    // by TSV
    procedure ClientConnected(AContext: TIdContext);
//    procedure ClientDisconnected(Thread: TIdPeerThread);
    // by TSV
    procedure ClientDisconnected(AContext: TIdContext);
...
//    procedure ReadData(Thread: TIdPeerThread);
    // by TSV
    procedure ReadData(AContext: TIdContext);
...
begin
  ...
  try
    Self.ThreadLock.Acquire;
    try
      if Assigned(Self.ClientThread) and Self.Client.Connected then begin
//        Self.Client.WriteStream(Data);
        // by TSV
        Self.Client.Socket.Write(Data);
      end;
    finally
      Self.ThreadLock.Release;
    end;
...
{  Connections := Self.Connection.Threads.LockList;
  ...
  Connections := Self.Connection.Contexts.LockList;
  try
    for I := 0 to Connections.Count - 1 do
      TIdContext(Connections[I]).Connection.Disconnect;
  finally
    Self.Connection.Contexts.UnlockList;
  end;
...
{  Connections := Self.Connection.Threads.LockList;
  ...
  try
    for I := 0 to Connections.Count - 1 do begin
      try
        TIdContext(Connections[I]).Connection.Socket.Write(Data);
      except
        on EIdReadTimeout do;
        on EIdConnClosedGracefully do;
        on EIdClosedSocket do;
...
{  Connections := Self.Connection.Threads.LockList;
  ...
  try
    if (Connections.Count > 0) then begin
      C := TIdContext(Connections[0]).Connection;
...
procedure TIdSdpTcpServerConnection.ClientConnected(AContext: TIdContext);
begin
  KeepAliveSocket(AContext.Connection, true);
  Self.NotifyOfConnectionInTimerContext;
end;
...
procedure TIdSdpTcpServerConnection.ClientDisconnected(AContext: TIdContext);
begin
  Self.NotifyOfDisconnectionInTimerContext;
end;
...
procedure TIdSdpTcpServerConnection.ReadData(AContext: TIdContext);
var
  Binding:    TIdSocketHandle;
  NewText:    TStringStream;
  ReceivedOn: TIdConnectionBindings;
begin
//ReceiveData(Data: TStream; ReceivedOn: TIdConnectionBindings)
  Binding := AContext.Connection.Socket.Binding;
  ...
  ReceivedOn := TIdConnectionBindings.Create(Binding.IP,
                                             ...
                                             Id_SDP_TCP);
  try
    NewText := TStringStream.Create(AContext.Connection.Socket.AllData);
    try
      Self.ReceiveData(NewText, ReceivedOn);
    finally
      NewText.Free;
