...
type
  ...
  protected
    function AddSortedChildObject(Parent: TTreeNode; const S: string; Data: PNodeInfoEh): TTreeNode;
    function CompareNode(Node1, Node2: TTreeNode): Integer;
    function CompareNodeData(Data1, Data2: PNodeInfoEh): Integer;
    function CreateNodeInfo(Component: TComponent; Name, Path: String;
      NodeType: TNodeTypeEh; IsVoidProperty: Boolean): PNodeInfoEh;
    ...
    function GetObjectPropList(AObject: TObject; var ObjPropCount: Integer): PPropList;
...
{ TPredifinedPropsEh }
  ...
  protected
    ...
    function Caprion: String; virtual;
...
{ TPredifinedActiveControlEh }
  ...
  protected
    ...
    function Caprion: String; override;
...
procedure RegisterPredifinedPropsClass(PropsClass: TPredifinedPropsEhClass);
begin
  PredifinedPropsClassList.Add(PropsClass);
end;

function CompareNodeInfo(Pni1, Pni2: PNodeInfoEh): Boolean;
begin
...
{$IFNDEF EH_LIB_CLX}
 ...
 begin
  ...
  begin
    OnIconDownNode.ImageIndex := PNodeInfoEh(OnIconDownNode.Data)^.Checked + 4;
    OnIconDownNode.SelectedIndex := PNodeInfoEh(OnIconDownNode.Data)^.Checked + 4;
  end else
    OnIconDownNode := nil;
...
{$IFNDEF EH_LIB_CLX}
  if (OnIconDownNode <> nil) then
    ...
    begin
      OnIconDownNode.ImageIndex := PNodeInfoEh(OnIconDownNode.Data)^.Checked;
      OnIconDownNode.SelectedIndex := PNodeInfoEh(OnIconDownNode.Data)^.Checked;
    end;
{$ENDIF}
end;
...
begin
  if not (PNodeInfoEh(N.Data)^.NodeType = nthProperty) then Exit;
  if PNodeInfoEh(N.Data)^.Checked <> 1
...
begin
  if not (PNodeInfoEh(N.Data)^.NodeType = nthProperty) or
    (PNodeInfoEh(N.Data)^.Checked = 1) then Exit;
//  TreeView1.Items.BeginUpdate;
//  TreeView2.Items.BeginUpdate;
  try
    N.ImageIndex := 1;
    N.SelectedIndex := 1;
    PNodeInfoEh(N.Data)^.Checked := 1;
    AddPropertyNode(N);
    AddParentChecked(N);
//    ResetParentNodes(N);
    ResetChildNodes(N);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (AnsiCompareText(PNodeInfoEh(Node.Data).Name, Token) = 0) and
         (PNodeInfoEh(Node.Data)^.NodeType = NodeType) then
      begin
        if Path <> '' then
        begin
          if (Node.Count > 0) and PNodeInfoEh(Node.Item[0].Data)^.IsVoidProperty then
          begin
            Node.Item[0].Free;
            PTerm := '';

            AddProperties(Node, PNodeInfoEh(Node.Data)^.Instance,
              PNodeInfoEh(Node.Data)^.Path + PTerm,
              not (PNodeInfoEh(Node.Data)^.NodeType = nthProperty));
            if not (PNodeInfoEh(Node.Data)^.NodeType = nthProperty) then
              AddComponents(Node, TComponent(PNodeInfoEh(Node.Data)^.Instance),
                PNodeInfoEh(Node.Data)^.Path + PTerm);
          end;
          if (Node.Count > 0)
            then Node := Node.Item[0]
    ...
    if AnsiUpperCase(Token) = '<P>' then
    begin
      if PNodeInfoEh(Node.Data)^.NodeType = nthPropNode then
        if (Node.Count > 0)
          then Node := Node.Item[0]
          else raise Exception.Create('Can not expand path - "' + Path + '"');
...
begin
  if not (PNodeInfoEh(N.Data)^.NodeType = nthProperty) or
    (PNodeInfoEh(N.Data)^.Checked = 0) then Exit;
//  TreeView1.Items.BeginUpdate;
  try
    N.ImageIndex := 0;
    N.SelectedIndex := 0;
    PNodeInfoEh(N.Data)^.Checked := 0;
...
begin
  for i := 0 to TreeView2.Items.Count-1 do
    if AnsiCompareText(Path,
      PNodeInfoEh(TTreeNode(TreeView2.Items[i].Data).Data)^.Path) = 0 then
    begin
  ...
  begin
    ...
    for i := 0 to TreeView2.Items.Count-1 do
      if (TreeView2.Items[i].Parent = nil) and
          CompareNodeInfo(PNodeInfoEh(TTreeNode(TreeView2.Items[i].Data).Data),
                          PNodeInfoEh((TTreeNode(NList[NList.Count-1]).Data))) then
      begin
        N := TreeView2.Items[i];
        Break;
      end;
...
{$ENDIF}
    ...
    begin
      NC := FindChildNodeInfo(N, NList[i]);
      if NC <> nil then
        N := NC
      else
      begin
...
begin
  N := N.Parent;
  while (N <> nil) and (PNodeInfoEh(N.Data)^.NodeType = nthProperty) do
  begin
    N.ImageIndex := 2;
    N.SelectedIndex := 2;
    PNodeInfoEh(N.Data)^.Checked := 2;
    N := N.Parent;
  end;
end;
...
begin
  ...
  if not HaveCheckedChilds(N) then
    RemovePropertyNode(N);
  if (PNodeInfoEh(N.Data)^.NodeType = nthProperty) then
...
var
  i: Integer;
begin
  if PNodeInfoEh(N.Data)^.Checked <> 0 then
    for i := 0 to N.Count-1 do
    begin
      ResetChildNodes(N.Item[i]);
      if PNodeInfoEh(N.Item[i].Data)^.Checked <> 0 then
      begin
        N.Item[i].ImageIndex := 0;
        N.Item[i].SelectedIndex := 0;
        PNodeInfoEh(N.Item[i].Data)^.Checked := 0;
...
begin
  Result := False;
  for i := 0 to N.Count-1 do
    if (PNodeInfoEh(N.Item[i].Data)^.NodeType = nthProperty) then
    begin
      if PNodeInfoEh(N.Item[i].Data)^.Checked <> 0 then
      begin
        Result := True;
    ...
    end else if (PNodeInfoEh(N.Item[i].Data)^.NodeType = nthPropNode) then
    begin
      Result := HaveCheckedChilds(N.Item[i]);
      if Result then Exit;
    end else if PNodeInfoEh(N.Item[i].Data)^.Instance <> nil then
    begin
...
function TPropStorageEditEhForm.AddSortedChildObject(Parent: TTreeNode;
  const S: string; Data: PNodeInfoEh): TTreeNode;
var
  Node: TTreeNode;
begin
  if Parent = nil
  ...
  while Node <> nil do
  begin
    if CompareNodeData(PNodeInfoEh(Node.Data), Data) > 0 then
    begin
{$IFDEF EH_LIB_CLX} // Clx BUG of InsertObject
...
function TPropStorageEditEhForm.CompareNodeData(Data1, Data2: PNodeInfoEh): Integer;
begin
...
begin
  Result := CompareNodeData(PNodeInfoEh(Node1.Data), PNodeInfoEh(Node2.Data));
end;
...
var
  NC: TTreeNode;
begin
  NC := TreeView1.Items.AddChildObjectFirst(N, 'VoidProperty', Pointer(0));
  NC.Data := CreateNodeInfo(nil, 'VoidProperty', 'VoidProperty', nthProperty, True);
end;

function TPropStorageEditEhForm.GetObjectPropList(AObject: TObject; var ObjPropCount: Integer): PPropList;
var
...
begin
  ...
  begin
    ...
    begin
      SubO := TObject(GetOrdProp(O, PropList^[i]));
      if Assigned(SubO) then
      begin
        PNodeInfoEh(NC.Data)^.Instance := SubO;
...
begin
  ...
  for i := 0 to TreeView2.Items.Count-1 do
    if TreeView2.Items[i].Count = 0 then
      PropList.Add(PNodeInfoEh(TTreeNode(TreeView2.Items[i].Data).Data).Path);
end;
...
begin
  ...
  if (TreeView1.Selected <> nil) and (TreeView1.Selected.Data <> nil) then
    Edit2.Text :=  PNodeInfoEh(TreeView1.Selected.Data).Path;
end;
...
begin
  spAddProp.Enabled := (TreeView1.Selected <> nil) and
                       ...
                       (PNodeInfoEh(TreeView1.Selected.Data).NodeType = nthProperty) and
                       (PNodeInfoEh(TreeView1.Selected.Data).Checked <> 1);
...
begin
  ...
  begin
    ...
    if (DownNode <> nil) and (PNodeInfoEh(TTreeNode(DownNode.Data).Data).NodeType = nthProperty) then
...
begin
  if PNodeInfoEh(Node1.Data).NodeType = PNodeInfoEh(Node2.Data).NodeType then
    Compare := CompareText(PNodeInfoEh(Node1.Data).Name, PNodeInfoEh(Node2.Data).Name)
  else if PNodeInfoEh(Node1.Data).NodeType = nthProperty then
    Compare := -1
  else
    Compare := 1;
end;
...
{$ENDIF}
    ...
    try
      ...
      AddProperties(Node, PNodeInfoEh(Node.Data)^.Instance,
        ...
        not (PNodeInfoEh(Node.Data)^.NodeType = nthProperty));
      if not (PNodeInfoEh(Node.Data)^.NodeType = nthProperty) then
        AddComponents(Node, TComponent(PNodeInfoEh(Node.Data)^.Instance), PNodeInfoEh(Node.Data)^.Path + PTerm);
//      TreeView1.AlphaSort;
    finally
      Screen.Cursor := OldCursor;
    end;
...
begin
  ...
  begin
    ...
    cbPredifinedProps.Items.AddObject(PreProp.Caprion, PreProp);
  end;
end;

{ TPredifinedPropsEh }

function TPredifinedPropsEh.Caprion: String;
begin
end;
...
function TPredifinedActiveControlEh.Caprion: String;
begin
  ...
  for i := 0 to cbPredifinedProps.Items.Count-1 do
    if cbPredifinedProps.Items.Objects[i] <> nil then
      cbPredifinedProps.Checked[i] :=
        TPredifinedPropsEh(cbPredifinedProps.Items.Objects[i]).
          PropertyAdded(nil, PNodeInfoEh(TTreeNode(DN.Data).Data)^.Path )
end;
...
begin
  for i := 0 to cbPredifinedProps.Items.Count-1 do
    if cbPredifinedProps.Items.Objects[i] <> nil then
      cbPredifinedProps.Checked[i] :=
        TPredifinedPropsEh(cbPredifinedProps.Items.Objects[i]).
          PropertyDeleted(nil, PNodeInfoEh(TTreeNode(DN.Data).Data)^.Path)
end;
...
function TPredifinedPosPropertiesEh.Caprion: String;
begin
  Result := 'Form Pos';
end;
...
function TPredifinedSizePropertiesEh.Caprion: String;
begin
  Result := 'Form Size';
end;
...
function TPredifinedStatePropertiesEh.Caprion: String;
begin
  Result := 'Form State';
end;
