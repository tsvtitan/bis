...
type
  ...
  public
    ...
    function Add(Watch: TBisStaticWatch): Boolean; reintroduce; overload;
    function Add(const Name: String; Value: Variant): TBisStaticWatch; reintroduce; overload;
    function Add(const Name: String; OnGetValue: TBisWatchOnGetValueEvent): TBisDynamicWatch; reintroduce; overload;
...
begin
  Result:=false;
  if not Find(Watch) then begin
    if not Assigned(Find(Watch.Name)) then
...
begin
  ...
  if not Assigned(Find(Name)) then begin
    Result:=TBisStaticWatch.Create(Name);
    Result.Value:=Value;
...
begin
  Result:=nil;
  if not Assigned(Find(Name)) then begin
    Result:=TBisDynamicWatch.Create(Name);
