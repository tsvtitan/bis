...
const
  ...
  const
    ...
    const
      c100: array['0'..'9'] of string = ('', 'сто ', 'двести ', 'триста ', 'четыреста ', 'пятьсот ', 'шестьсот ', 'семьсот ', 'восемьсот ', 'девятьсот ');
      ...
      c2: array[False..True, '0'..'9'] of string = (('нулевой', 'первый ', 'второй ', 'третий ', 'четвертый ', 'пятый ', 'шестой ', 'седьмой ', 'восьмой ', 'девятый '),
                                                    ('нулевая', 'первая ', 'вторая ', 'третья ', 'четвертая ', 'пятая ', 'шестая ', 'седьмая ', 'восемая ', 'девятая '));
    begin {Num000toStr}
      if not counter then
        ...
        Result := c100[s[1]] + iif((s[2] = '1') and (s[3] > '0'),
                                   c11[s[3]] + iff(first,'надцат'+iff(woman,'ая ','ый '),'надцать '),
...
var
  ...
  AsCounter: Boolean;
const
  whole: array['0'..'9'] of string[2] = ('ых', 'ая', 'ых', 'ых', 'ых', 'ых', 'ых', 'ых', 'ых', 'ых');
  ruble: array['0'..'9'] of string[2] = ('ей', 'ь', 'я', 'я', 'я', 'ей', 'ей', 'ей', 'ей', 'ей');
...
begin
  ...
  case WordType of
    0,1,7,8: S:=V1;
    2: S:=V1+' цел'+whole[ending(O1)]+' '+SysUtils.Trim(O2)+' дробн'+whole[ending(O2)];
    3: S:=V1+' цел'+whole[ending(O1)]+' '+V2+' дробн'+whole[ending(O2)];
    4: S:=V1+' рубл'+ruble[ending(O1)];
    5: S:=V1+' рубл'+ruble[ending(O1)]+' '+SysUtils.Trim(O2)+' копе'+Kopeek[ending(O2)];
    6: S:=V1+' рубл'+ruble[ending(O1)]+' '+V2+' копе'+kopeek[ending(O2)];
  else
    S:=V1;
  end;
  S:=SysUtils.Trim(S);
