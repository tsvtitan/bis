  ...
  if Assigned(DataSet) and DataSet.Active then begin
    ...
    try
      ...
      for i:=0 to Count-1 do begin
        ...
        Flag:=Flag and (Assigned(FName) or Assigned(FDecs) or
        ...
        if Flag then begin
          ...
          if Assigned(FVisible) then
            FVisible.AsInteger:=Integer(Param.Visible);
...
begin
  for i:=0 to Count-1 do begin
    if Items[i].Enabled and Items[i].Connected then begin
      try
        Items[i].CancelAll;
      except
        On E: Exception do
