...
begin
  ...
  FResultQueue:='очередь';
  FResultActive:='отправка';
  FResultSuccess:='успешно';
  FResultFail:='ошибка';
...
begin
  if FInProcess and Assigned(FThread) then begin
    ...
    if not Suspended then
      FThread.Suspend;
    CanClose:=ShowQuestion('Отправка не закончена. Выйти из программы?',mbNo)=mrYes;
    ...
    try
      ...
      if Fails.Count>0 then begin
        ...
        if Trim(FMessage)='' then begin
          FFilters.Delimiter:=',';
          Fails.Delimiter:=',';
          Mes:=FormatEx('Формат файла(ов) %s не поддерживатся.'+#13#10+
                        'Поддерживаются только следующие форматы: %s',[Trim(Fails.DelimitedText),Trim(FFilters.DelimitedText)]);
        end;
        ShowWarning(Mes);
      end;
    finally
...
var
  ...
  SecFull: Integer;
begin
  LabelFile.Caption:=FormatEx('Файлов: %d / %d',[Current,AllCount]);
  LabelFile.Update;
  ...
  LabelSize.Caption:=FormatEx('Размер: %s / %s Mb',[FormatFloat('#0.00',NewSize),FormatFloat('#0.00',NewAllSize)]);
  LabelSize.Update;
  ...
  LabelTime.Caption:=FormatEx('Время: %s / %s',[FormatDateTime('h:nn:ss',ATime),FormatDateTime('h:nn:ss',NewTime)]);
  LabelTime.Update;
  ...
  LabelSpeed.Caption:=FormatEx('Скорость: %s Mb/s',[FormatFloat('#0.00',NewSpeed)]);
  LabelSpeed.Update;
