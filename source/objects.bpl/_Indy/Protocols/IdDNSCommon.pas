...
const
  IdDNSServerVersion = 'Indy DNSServer 2004071501'; {do not localize}
  cRCodeNoError   = 0;
  cRCodeFormatErr = 1;
  cRCodeServerErr = 2;
  cRCodeNameErr   = 3;
...
function IPv6AAAAToDNSStr(IPv6Address : String): TIdBytes;
...
begin                         { DomainNameToDNSStr }
  ...
  begin
    aPos := Pos ( '.', ADomain );  {do not localize}
    if aPos = 0 then
    begin
      aPos := Length(ADomain) + 1;
    end; //if aPos = 0 then
    BufStr := Copy(ADomain, 1, aPos - 1);
    Delete(ADomain, 1, aPos);
...
begin
  ...
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_CName)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes(Self.TTL));
  AppendBytes(Result, ToBytes(SmallInt(Length(RRData))));
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_HINFO)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes(Self.TTL));
  AppendBytes(Result, ToBytes(integer(Length(RRData))));
  AppendBytes(Result, RRData);
end;
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_MB)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
end;
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt((TypeCode_MG))));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  RRData := DomainNameToDNSStr((Self.ErrorHandle_Mail));

  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_MINFO)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_MR)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_MX)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_NS)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_PTR)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  AppendBytes(RRData, ToBytes(StrToInt(Self.Serial)));
  AppendBytes(RRData, ToBytes(StrToInt(Self.Refresh)));
  AppendBytes(RRData, ToBytes(StrToInt(Self.Retry)));
  AppendBytes(RRData, ToBytes(StrToInt(Self.Expire)));
  AppendBytes(RRData, ToBytes(StrToInt(Self.Minimum)));

  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_SOA)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_A)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
  ...
  Result := DomainNameToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt(TypeCode_AAAA)));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes(Self.TTL));
  AppendBytes(Result, ToBytes(SmallInt(16)));
  AppendBytes(Result, RRData);
end;
...
begin
  ...
  RRData := ToBytes(Self.TXT); //TODO ???

  Result := NormalStrToDNSStr((QName));
  AppendBytes(Result, ToBytes(SmallInt((TypeCode_TXT))));
  AppendBytes(Result, ToBytes(SmallInt(Class_IN)));
  AppendBytes(Result, ToBytes((Self.TTL)));
  AppendBytes(Result, ToBytes((Length(RRData))));
  AppendBytes(Result, RRData);
...
begin
   if Target = NewString then Result := Source
   else begin
        FixingString := Source;
